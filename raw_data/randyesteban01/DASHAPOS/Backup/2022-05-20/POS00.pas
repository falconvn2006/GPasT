...
uses
  ...
  uJSON, OCXFISLib_TLB, vmaxFiscal, iFiscal, Tfhkaif, cxControls,
...
type
  ...
  TfrmMain = class(TForm)
    ...
    procedure QTicketAfterOpen(DataSet: TDataSet);
  ...
  public
    ...
    SeleccionCliente, SeleccionEmpresa : integer;
...
uses POS01, POS03, POS04, POS05, POS06, POS07, POS08, POS09, POS10, POS11,
  ...
  POS24, POS25, POS22, PVENTA185, POS27;
...
begin
  ...
  if Trim(MimeEncodeString(dm.QEmpresaemp_rnc.AsString)) <> dm.QEmpresacode_rnc.AsString then
    ...
    begin //[1]
      ...
      dm.Query1.SQL.Add('puerto_display, Termica, clave_empaque, clave_temporal, Verifica_Precio from Cajas_IP');
...
begin
  ...
  Clear;
    ...
    OPOSScanner1.Open('ZEBRA_SCANNER') ;
    OPOSScanner1.ClaimDevice(1000);
    OPOSScanner1.DeviceEnabled:=True;
...
begin
  ...
  dm.Query1.Close;
  dm.Query1.SQL.Clear;
  dm.Query1.SQL.Add('select * from productos where emp_codigo = :paremp and pro_roriginal like '+QuotedStr('%'+edproducto.Text+'%'));
...
//fin serie
        ...
        if Trim(frmCombinado.edtarjeta.Text) <> '0' then
        begin
          LPagado := LPagado + ',TAR';
          QFormaPago.Append;
          QFormaPagocaja.Value       := StrToInt(edcaja.Caption);
          ...
          QFormaPagoticket.Value     := QTicketticket.Value;
          QFormaPagoforma.Value      := 'TAR';
          QFormaPagopagado.Value     := StrToFloat(frmCombinado.edtarjeta.Text);
          if cliente > 0 then QFormaPagocliente.Value    := cliente;
          QFormaPagodevuelta.Value   := 0;
          QFormaPagocredito.Value    := 'False';
        ...
        if Trim(frmCombinado.edcheque.Text) <> '0' then
        begin
          LPagado := LPagado + ',CHE';
          QFormaPago.Append;
          QFormaPagocaja.Value       := StrToInt(edcaja.Caption);
          ...
          QFormaPagoticket.Value     := QTicketticket.Value;
          QFormaPagoforma.Value      := 'CHE';
          QFormaPagopagado.Value     := StrToFloat(frmCombinado.edcheque.Text);
        ...
        if Trim(frmCombinado.edCredito.Text) <> '0' then
        begin
          LPagado := LPagado + ',CRE';
          QFormaPago.Append;
          QFormaPagocaja.Value       := StrToInt(edcaja.Caption);
          ...
          QFormaPagopagado.Value     := StrToFloat(frmCombinado.edCredito.Text);
          QFormaPagodevuelta.Value   := 0;
          ...
          Query1.Parameters.parambyname('total').Value   := StrToFloat(frmCombinado.edCredito.Text);
          Query1.Parameters.parambyname('emp').Value     := empresa;
          ...
          Query1.Parameters.parambyname('cliente').Value  := cliente;
          Query1.Parameters.parambyname('monto').Value    := StrToFloat(frmCombinado.edCredito.Text);
          ...
          begin
            ...
            Query1.Parameters.ParamByName('fec').DataType := ftdate;
            Query1.Parameters.ParamByName('val').Value    := StrToFloat(frmCombinado.edCredito.Text);
            Query1.ExecSQL;
          end;
          frmClientes.Release;
        end;

        if Trim(frmCombinado.edBonoClub.Text) <> '0' then
        begin
          LPagado := 'BOC';
          QFormaPago.Append;
          QFormaPagocaja.Value       := StrToInt(edcaja.Caption);
          ...
          QFormaPagopagado.Value     := StrToFloat(frmCombinado.edBonoClub.Text);
          QFormaPagodevuelta.Value   := 0;
          QFormaPagocredito.Value    := 'False';
          QFormaPagodocumento.Value  := '';
          QFormaPago.Post;
        ...
        if Trim(frmCombinado.edBonoOtro.Text) <> '0' then
        begin
          LPagado := 'OBO';
          QFormaPago.Append;
          QFormaPagocaja.Value       := StrToInt(edcaja.Caption);
          ...
          QFormaPagopagado.Value     := StrToFloat(frmCombinado.edBonoOtro.Text);
          QFormaPagodevuelta.Value   := 0;
          QFormaPagocredito.Value    := 'False';
          QFormaPagodocumento.Value  := '';
          QFormaPago.Post;
        ...
        if Trim(frmCombinado.edrecibido.Text) <> '0' then
        begin
          LPagado := ',EFE';
          QFormaPago.Append;
          QFormaPagocaja.Value       := StrToInt(edcaja.Caption);
          ...
          if cliente > 0 then QFormaPagocliente.Value    := cliente;
          QFormaPagoforma.Value      := 'EFE';
          QFormaPagopagado.Value     := StrToFloat(frmCombinado.edrecibido.Text);
          if MDevuelta < QFormaPagopagado.Value then
          begin
            QFormaPagodevuelta.Value := MDevuelta;
            MDevuelta := 0;
        ...
        begin
          ...
          Query1.Parameters.ParamByName('tarjeta').Value := tarjetaclub;
          Query1.Parameters.parambyname('monto').Value   := QTickettotal.Value -
                                (StrToFloat(frmCombinado.edBonoOtro.Text) +StrToFloat(frmCombinado.edBonoClub.Text));
          Query1.ExecSQL;
        end;
        ...
        if not ImprimeTicketFiscal(Impresora) then
        begin
        if UpperCase(pregunta) = 'S' then
        if MessageDlg('Desea imprimir el ticket?', mtConfirmation, [mbyes, mbno], 0) = mryes then
        ImpTicket;
        ...
        Application.CreateForm(tfrmDevuelta, frmDevuelta);
        frmDevuelta.recibido := (StrToFloat(frmCombinado.edrecibido.Text) + StrToFloat(frmCombinado.edtarjeta.Text) +
                                StrToFloat(frmCombinado.edcheque.Text) + StrToFloat(frmCombinado.edBonoClub.Text) +
                                StrToFloat(frmCombinado.edBonoOtro.Text) + StrToFloat(frmCombinado.edCredito.Text));
        frmDevuelta.devuelta := (StrToFloat(frmCombinado.edrecibido.Text) + StrToFloat(frmCombinado.edtarjeta.Text) +
                                StrToFloat(frmCombinado.edcheque.Text) + StrToFloat(frmCombinado.edBonoClub.Text) +
                                StrToFloat(frmCombinado.edBonoOtro.Text) + StrToFloat(frmCombinado.edCredito.Text)) - QTickettotal.Value;

        frmDevuelta.lbdevuelta.Caption := Format('%n',[(StrToFloat(frmCombinado.edrecibido.Text) + StrToFloat(frmCombinado.edtarjeta.Text) +
                                                        StrToFloat(frmCombinado.edcheque.Text) + StrToFloat(frmCombinado.edBonoClub.Text) +
                                                        StrToFloat(frmCombinado.edBonoOtro.Text) + StrToFloat(frmCombinado.edCredito.Text)) -
                                                        QTickettotal.Value]);
        frmDevuelta.ShowModal;
...
begin
  ...
  begin
    ...
    frmBuscaProducto.QProductos.SQL.Add('p.pro_codigo, p.pro_roriginal, p.pro_precio3, p.pro_precio4');
...
begin
      ...
      writeln(arch, 'Fecha   .: '+DateToStr(DM.getFechaServidor)+' Factura: '+formatfloat('0000000000',QTicketticket.value));
      writeln(arch, 'Caja   ..: '+formatfloat('000',strtofloat(edCaja.Caption))+'        Hora ..: '+timetostr(time));
      ...
      begin
        ...
        if ImpItbis = 'True' then begin
          FillChar(s2, 16-length(copy(trim(QDetalledescripcion.value),1,16)),' ');
          end
        else begin
        ...
        s4 := '';
        FillChar(s4, 8-length(trim(FORMAT('%n',[QDetalleCalcItbis.value * QDetallecantidad.Value]))),' ');
        //FillChar(s4, 8-length(trim(FORMAT('%n',[QDetalleitbis.value])+'%')),' ');

        s5 := '';
        FillChar(s5, 5-length(format('%n',[qDetalleprecio.value])),' ');
        ...
        begin
          ...
          writeln(arch, s+format('%n',[QDetallecantidad.value])+
                        ' '+s5+format('%n',[qDetalleprecio.value])+' '+copy(trim(QDetalledescripcion.value),1,10)+s2+s4+FORMAT('%n',[QDetalleCalcItbis.value * QDetallecantidad.Value])+s1+
                        format('%n',[QDetalleValor.value]));
          if Length(QDetalledescripcion.value)>11 then
          writeln(arch,'            '+copy(trim(QDetalledescripcion.value),11,28));

        end
        else begin
          writeln(arch, s+format('%n',[QDetallecantidad.value])+' '+
                        s5+format('%n',[qDetalleprecio.value])+
                        lbitbis+' '+copy(trim(QDetalledescripcion.value),1,16)+s2+s1+format('%n',[QDetalleValor.value]));
         if Length(QDetalledescripcion.value)>16 then
          writeln(arch,'            '+copy(trim(QDetalledescripcion.value),16,28));
...
//-----------------------------------
    ...
    PrecioClie := Query1.fieldbyname('cli_precio').AsString;
     ...
     PrecioClie := Query1.fieldbyname('cli_Precio').AsString
      ...
      QTicketrnc.Value           :=  Query1.fieldbyname('cli_rnc').Value;
...
procedure TfrmMain.QTicketAfterOpen(DataSet: TDataSet);
begin
QDetalle.Close;
QDetalle.Open;
end;
...
if qImpCardNet.RecordCount > 0 then begin
  ...
  begin
    ...
    readln(puertopeqCardNet, puerto);
  end
  else
    puerto := 'PRN';
  ...
  rewrite(archCardNet);
  writeln(archCardNet, 'type .\tverifone.txt > '+puerto);
  ...
  writeln(archCardNet,'');
  writeln(archCardNet,'');
  writeln(archCardNet,'Monto RD$       : '+qImpCardNet.FieldByName('montosinitbis').text);
  writeln(archCardNet,'Tax   RD$       : '+qImpCardNet.FieldByName('monto_itbis').text);
  writeln(archCardNet,'Total RD$       : '+qImpCardNet.FieldByName('monto').text);
  writeln(archCardNet,'');
  writeln(archCardNet,'');
  ...
  writeln(archCardNet,'Fecha            : '+copy(b['Transaction']['DataTime'].getStr,1,10));
 ...
 //Ticket Copia
  ...
  rewrite(archCardNet);
  writeln(archCardNet, 'type .\tverifoneCopia.txt > '+puerto);
  closefile(archCardNet);
  ...
  writeln(archCardNet,'No de referencia: '+b['Transaction']['RetrievalReference'].getStr);
  writeln(archCardNet,'No Autorizacion : '+b['Transaction']['AuthorizationNumber'].getStr);
  writeln(archCardNet,'Fecha            : '+copy(b['Transaction']['DataTime'].getStr,1,12));
  writeln(archCardNet,'Hora             : '+trim(copy(b['Transaction']['DataTime'].getStr,12,20)));
  writeln(archCardNet,'');
  writeln(archCardNet,'');
