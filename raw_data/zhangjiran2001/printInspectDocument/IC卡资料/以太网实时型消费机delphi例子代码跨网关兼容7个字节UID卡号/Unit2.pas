...
type
  ...
  public
    ...
    strmydevno:string;//设备硬件序列号，全球唯一
...
begin
  //读卡器地址
  EditIp1.Text := strmyip1;
  EditIp2.Text := strmyip2;
  EditIp3.Text := strmyip3;
  EditIp4.Text := strmyip4;

  //子网掩码
  EditIp5.Text := strsubnetmask1;
  EditIp6.Text := strsubnetmask2;
  EditIp7.Text := strsubnetmask3;
  EditIp8.Text := strsubnetmask4;

  //网关地址
  EditIp13.Text := strgateip1;
  EditIp14.Text := strgateip2;
  EditIp15.Text := strgateip3;
  EditIp16.Text := strgateip4;

  //网关MAC
  EditMac7.Text := strgatemac1;
  EditMac8.Text := strgatemac2;
  EditMac9.Text := strgatemac3;
  EditMac10.Text := strgatemac4;
  EditMac11.Text := strgatemac5;
  EditMac12.Text := strgatemac6;

  //远程电脑地址
  EditIp9.Text := strremoteip1;
  EditIp10.Text := strremoteip2;
  EditIp11.Text := strremoteip3;
  EditIp12.Text := strremoteip4;

  //远程电脑MAC
  EditMac1.Text := strremotemac1;
  EditMac2.Text := strremotemac2;
  EditMac3.Text := strremotemac3;
  EditMac4.Text := strremotemac4;
  EditMac5.Text := strremotemac5;
  EditMac6.Text := strremotemac6;

  if(isautogetgatemac1) then
  begin
  ...
  //端口
  Edit14.Text := strmyport;

  Edit15.Text := strmydevno; //硬件序列号
...
begin
   ...
   strmyip1 := EditIp1.Text;

   i := StrToInt(strmyip1);

   if (Trim(EditIp1.Text) = '255') and (Trim(EditIp2.Text) = '255') and (Trim(EditIp3.Text) = '255') and (Trim(EditIp4.Text) = '255') then
   begin//广播式

   end
   else
   begin
     if((i < 0) or (i > 223)) then
     begin
        ShowMessage('请指定一个介于1 和 223之间的数值');
        EditIp1.SelectAll;
        EditIp1.SetFocus;
        Exit;
     end;
   ...
   strmyip2 := EditIp2.Text;
   strmyip3 := EditIp3.Text;
   strmyip4 := EditIp4.Text;
