...
type
  TControleConsultaChequesRecebidos = class(TControleConsulta)
    ...
    CdsBaixaChequeSomaTotal: TAggregateField;
    CdsBaixaChequeQtd: TAggregateField;
  ...
  private
    ...
    function MarcadoParaBaixar(id: Integer; situacao: string): Boolean;
    procedure DesmarcaMarcadosParaBaixar;
...
begin
  //  inherited;
 DesmarcaMarcadosParaBaixar;

  bJSName := (BotaoBaixarCheque).JSName;
  UniSession.AddJS(bJSName + '.setBadgeText(' + bJSName + '.getBadgeText() -'+ IntToStr(QtdChequeBaixa) +');');
...
begin
  ...
  begin
    ControleConsultaChequesRecebidosBaixa.Abrir;
    ControleConsultaChequesRecebidosBaixa.ShowModal(procedure(Sender: TComponent; Result: Integer)
    begin
     if ControleConsultaChequesRecebidosBaixa.Depositado then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        QtdChequeBaixa := QtdChequeBaixa +1;
        bJSName := (BotaoBaixarCheque).JSName;
        UniSession.AddJS(bJSName + '.setBadgeText(' + bJSName + '.getBadgeText() + 1);');


        MarcadoParaBaixar(CdsConsultaID.AsInteger,'S');
        CdsConsulta.Refresh;
      end
      else
      begin
        ...
        MarcadoParaBaixar(CdsConsultaID.AsInteger,'N');
        CdsConsulta.Refresh;
      end;
    end;
  end;
...
Var
  DtInicial, DtFinal,bJSName : String;
begin
  CdsConsulta.Params.ParamByName('login_id').Value   :=ControleMainModule.FUsuarioId;
  DesmarcaMarcadosParaBaixar;
//  inherited;
...
procedure TControleConsultaChequesRecebidos.UniEdit1KeyPress(Sender: TObject;
  var Key: Char);
var
  bJSName : string;
begin
...
function TControleConsultaChequesRecebidos.MarcadoParaBaixar(id:Integer;situacao:string):Boolean;
begin
  result := False;
  // Update
  ...
  begin
    ...
    begin
      ...
      QryAx1.SQL.Clear;
      QryAx1.SQL.Text :=
            'INSERT INTO temp_baixa_cheque ('
          + '       cheque_id,'
          + '       usuario_id) '
          + 'VALUES ('
          + '       :cheque_id,'
          + '       :usuario_id)';
      QryAx1.Parameters.ParamByName('cheque_id').Value   :=id;
    ...
    begin
      ...
      QryAx1.SQL.Clear;
      QryAx1.SQL.Text :=
                      'DELETE FROM temp_baixa_cheque WHERE cheque_id =:cheque_id and usuario_id =:usuario_id';
      QryAx1.Parameters.ParamByName('cheque_id').Value   :=id;
      QryAx1.Parameters.ParamByName('usuario_id').Value    :=FUsuarioId;
...
procedure TControleConsultaChequesRecebidos.DesmarcaMarcadosParaBaixar;
begin
  with ControleMainModule do
  begin
    ADOConnection.BeginTrans;
    ...
    QryAx1.SQL.Clear;
    QryAx1.SQL.Text :=
                   'DELETE FROM temp_baixa_cheque WHERE usuario_id =:usuario_id';
    QryAx1.Parameters.ParamByName('usuario_id').Value    :=FUsuarioId;
    try
      // Tenta salvar os dados
      QryAx1.ExecSQL;
