...
uses
  ...
  uniSweetAlert, FireDAC.Phys.FBDef, FireDAC.Phys.FB, FireDAC.Moni.Base,
  FireDAC.Moni.FlatFile, FireDAC.Phys.IBBase, uniLabel, uniButton, uniEdit,
  uniPanel, uniBasicGrid, uniDBGrid, Vcl.Controls, Vcl.Forms;
...
type
  ...
  TControleMainModule = class(TUniGUIMainModule)
    ...
    FDConnectionIntegracao: TFDConnection;
  ...
  public
    { Public declarations }
    cnpj_cpf_cliente_integracao: string;
    MensageiroSweetAlert : TUniSweetAlert;
    PLiquidaMultiploReceber: String;
    PLiquidaMultiploPagar: String;
    ...
    DirBytes : integer;
    function ProcuraUsuarioPermissaoDelTitulo(password: string): String;
    function ApagaTituloReceber(TituloId: Integer): Boolean;
    function ConectaBancoIntegracao: Boolean;
...
//    function ConfereTituloGeraBoleto(Tipo_Titulo_id: Integer): String;
    ...
    function VerificaCaixaAberto(id: Integer): Boolean;
...
//    function PlanoSistemaHDControle(plano: String): double;
    ...
    Function GravandoTituloReceber(CondicoesPagamentoId: Integer;
                                                   ...
                                                   NumeroReferencia: string): Integer;
...
//                                          Schema_Alvo: string);
    ...
    function SelecionaClienteGestor(vcpf_cnpj: string): Boolean;
    function CadastraCliente(cpf_cnpj: string;
                             cliente_id: string): Boolean;
    function CadastraClienteDoFirebird(ClienteFB: TFDQuery): Boolean;
    procedure ReabreCaixa(CaixaId: Integer);
    function VerificaCaixaFechadoDoDia(UsuarioId: Integer;
                                                       data: TDateTime): integer;
    procedure FechaCaixaDiasAnteriores(CaixaId: Integer);
  end;
...
uses
  ...
  System.NetEncoding, System.StrUtils, System.IniFiles;
...
procedure TControleMainModule.FDMoniFlatFileClientLink1Output(
...
begin
  ...
  QryAx1.SQL.Add('       situacao                = :situacao,');
  ...
  QryAx1.Parameters.ParamByName('situacao').Value               := situacao;
...
begin
  ...
  QryAx2.Parameters.ParamByName('titulo_pagamento_id').Value  := titulo_pagamento_id;
...
procedure TControleMainModule.FechaCaixaDiasAnteriores(CaixaId: Integer);
begin
  // Inicia a transação
  ADOConnection.BeginTrans;
  ...
  QryAx1.SQL.Text :=
    ...
    + '   SET data_fechamento = SYSDATE'
    + ' WHERE usuario_id = :id'
    + ' AND TRUNC(data_abertura) < TRUNC(SYSDATE)'
    + ' AND data_fechamento is null';
  QryAx1.Parameters.ParamByName('id').Value := CaixaId;
//  QryAx1.SQL.SaveToFile('teste.txt');
...
procedure TControleMainModule.ReabreCaixa(CaixaId: Integer);
begin
...
function TControleMainModule.VerificaCaixaAberto(id: Integer): Boolean;// retorna o id do caixa conectado, se for 0 é caixa novo
begin
  Result := False;
  ...
  QryAx2.SQL.Add('SELECT id');
  QryAx2.SQL.Add('  FROM caixa');
  QryAx2.SQL.Add(' WHERE id = '+ IntToStr(id) +'');
  QryAx2.SQL.Add('   AND data_fechamento is null');
  CdsAx2.Open;
...
function TControleMainModule.VerificaCaixaFechadoDoDia(UsuarioId: Integer;
                                                       data: TDateTime): integer;// retorna o id do caixa conectado, se for 0 é caixa novo
begin
  Result := 0;
  ...
  QryAx2.SQL.Add('  FROM caixa');
  QryAx2.SQL.Add(' WHERE usuario_id = '+ IntToStr(UsuarioId) +'');
  QryAx2.SQL.Add('   and trunc(data_fechamento) is not null');
  QryAx2.SQL.Add('   and trunc(data_abertura)');
  QryAx2.SQL.Add('   between to_date(''' + Trim(DateToStr(Data)) + ''', ''dd/mm/yyyy'')');
  QryAx2.SQL.Add('   and to_date(''' + Trim(DateToStr(Data)) + ''', ''dd/mm/yyyy'')');
//  QryAx2.SQL.SaveToFile('1.txt');
  CdsAx2.Open;
...
begin
  try
    ...
    begin
      ...
      MensageiroSweetAlerta('Atenção','Falha ao gerar PDF, tente novamente!');
  except
...
begin
  Try
    ...
    QryAx1.SQL.Text :=  'SELECT p.cpf_cnpj,'
                       ...
                       +'   		   FROM '+ FSchemaPdve +'.TITULO_PAGAMENTO tp'
                       +' 		     LEFT JOIN '+ FSchemaPdve +'.FORMA_PAGAMENTO fp'
                       +'     	     ON fp.ID = tp.FORMA_PAGAMENTO_ID'
                       +'  	      WHERE	TITULO_ID = t.id'
                       +'    	      AND fp.GERA_NOVO_TITULO = ''S'') valor_total,'
                       +'	    	T.id numero_titulo,'
                       +'		    pv.cliente_id'
                       +'  FROM '+ FSchemaPdve +'.PEDIDO_VENDA pv'
                       +'  LEFT JOIN '+ FSchemaPdve +'.PEDIDO_VENDA_ITEM pvi'
                       +'    ON	pvi.PEDIDO_VENDA_ID = pv.ID'
                       +'  LEFT JOIN '+ FSchemaPdve +'.CLIENTE c'
                       +'    ON c.ID = pv.CLIENTE_ID'
                       +'  LEFT JOIN '+ FSchemaPdve +'.PESSOA p'
                       +'    ON	p.ID = c.ID'
                       +'  LEFT JOIN '+ FSchemaPdve +'.TITULO t'
...
function TControleMainModule.SelecionaClienteGestor(vcpf_cnpj: string): Boolean;
begin
  Result := False;
  Try
    CdsAx1.Close;
    ...
    QryAx1.SQL.Text :=   'SELECT id,'
                        +'       razao_social,'
                        +'       cpf_cnpj'
                        +'  FROM '+ FSchemaPdve +'.PESSOA pes'
                        +' WHERE pes.cpf_cnpj  = :cpf_cnpj';
    QryAx1.Parameters.ParamByName('cpf_cnpj').Value := vcpf_cnpj;
    CdsAx1.Open;
  Finally
...
begin
  Try
    ...
    Percet_Dia := Juros / 30;//DiasPorMes(ano, mes);
    ...
    Result.ValorAtualizado := Novo_Valor - Desconto;
  end;
end;
...
function TControleMainModule.ConectaBancoIntegracao :Boolean;
var
  vDllFirebird,
  vName : String;
...
begin
  ...
  if Integracao.UserName  = '' then
    Exit;

  FDPhysFBDriverLink1.VendorLib := Integracao.VendorLib;

  FDConnectionIntegracao.Connected := False;

  with FDConnectionIntegracao do
  begin
...
Function TControleMainModule.LerIniIntegracao: TIntegracao;
var
  Config : TiniFile;
begin
  ...
  Except
    raise Exception.Create('Não foi possível ler o config.ini da integração.');
    Exit;
  end;
end;

function TControleMainModule.CadastraCliente(cpf_cnpj: string;
                                             cliente_id: string): Boolean;
var
  CadastroId, CadastroEndereco_id: integer;
begin
  inherited;
  ...
  CadastroId := GeraId('pessoa');
  // Insert
  QryAx1.SQL.Text :=   'INSERT INTO'
                       +'             pessoa ('
                       +'                     id,'
                       +'                     tipo,'
                       +'                     razao_social,'
  ...
  // Insert
  QryAx2.SQL.Text :=   'INSERT INTO'
                      +'    cliente(id) VALUES('+'''' + IntToStr(CadastroId) +''')';
  ...
  CadastroEndereco_id := GeraId('pessoa_endereco');

  // Insert
  QryAx3.SQL.Text :=   'INSERT INTO'
                       +'     pessoa_endereco ('
                       +'                     id,'
                       +'                     pessoa_id,'
                       +'                     tipo,'
                       ...
                       +'                     celular,'
                       +'                     email'
                       +'              FROM   '+ FSchemaPdve +'.pessoa_endereco'
...
function TControleMainModule.CadastraClienteDoFirebird(ClienteFB: TFDQuery): Boolean;
var
  CadastroId, CadastroEndereco_id: integer;
begin
  inherited;
  ...
  QryAx1.SQL.Text :=   'INSERT INTO'
                       ...
                       +'                     '+ IntToStr(CadastroId) +','
                       +'                     ''F'','
                       +'                     '''+ ClienteFB.FieldByName('nome').AsString +''','
                       +'                     '''+ ClienteFB.FieldByName('nome').AsString +''','
                       +'                     '''+ ControleFuncoes.RemoveMascara(Trim(ClienteFB.FieldByName('cnpjcpf').AsString)) +''','
                       +'                     '''+ ControleFuncoes.RemoveMascara(Trim(ClienteFB.FieldByName('inscrg').AsString)) +''','
                       +'                     null,'
                       +'                     null,'
                       +'                     null,'
                       +'                     null,'
                       +'                     null,'
                       +'                     null,'
                       +'                     '''+ ClienteFB.FieldByName('mae').AsString +''','
                       +'                     '''+ ClienteFB.FieldByName('pai').AsString +''','
                       +'                     null,'
                       +'                     null,'
                       +'                     null)';
  ...
  QryAx3.SQL.Text :=   'INSERT INTO'
                       ...
                       +'                     '+ IntToStr(CadastroId) +','
                       +'                     ''GE'','
                       +'                     '''+ ClienteFB.FieldByName('endereco').AsString +''','
                       +'                     null,'
                       +'                     null,'
                       +'                     null,'
                       +'                     '''+ ClienteFB.FieldByName('cep').AsString +''','
                       +'                     '''+ ClienteFB.FieldByName('bairro').AsString +''','
                       +'                     null,'
                       +'                     null,'
                       +'                     null,'
                       +'                     null,'
                       +'                     '''+ ClienteFB.FieldByName('celular').AsString +''','
                       +'                     '''+ ClienteFB.FieldByName('email').AsString +''')';
...
function TControleMainModule.ApagaTituloReceber(TituloId : Integer):Boolean;
begin
  Result:= False;

  //update nos titulos se existir
  // Inicia a transação
  if ADOConnection.InTransaction = False then
  ADOConnection.BeginTrans;
  ...
  QryAx3.Parameters.Clear;
  QryAx3.SQL.Clear;
  QryAx3.SQL.Text := 'DELETE FROM titulo_receber WHERE id = :TituloId';
  QryAx3.Parameters.ParamByName('TituloId').Value := TituloId;

  try
    // Tenta apagar o registro
    QryAx3.ExecSQL; //titulo_receber
  except
    on E: Exception do
    begin
      ADOConnection.RollbackTrans;
...
function TControleMainModule.ProcuraUsuarioPermissaoDelTitulo(password : string):String;
begin
  Result := '';
  with ControleMainModule do
  begin
      CdsAx4.Close;
      QryAx4.SQL.Clear;
      QryAx4.SQL.Text := '  SELECT login, senha, menu_permite_del_titulo_botao'
                        +'  FROM    usuario.usuario u'
                        +'       INNER JOIN'
                        +'          usuario.usuario_permissao_controle upc'
                        +'       ON upc.usuario_id = u.id'
                        +'  WHERE u.senha = :password'
                        +'  AND u.schema = :SchemaLogin';

     QryAx4.Parameters.ParamByName('password').Value := Trim(password);
     QryAx4.Parameters.ParamByName('SchemaLogin').Value := UpperCase(Trim(FSchema));
     CdsAx4.Open;

     if CdsAx4.RecordCount >0 then
       Result := CdsAx4.FieldByName('menu_permite_del_titulo_botao').AsString;;
  end;
