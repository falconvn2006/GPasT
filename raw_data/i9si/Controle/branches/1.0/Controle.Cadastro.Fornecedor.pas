unit Controle.Cadastro.Fornecedor;
...
type
  TControleCadastroFornecedor = class(TControleCadastro)
...
function ControleCadastroFornecedor: TControleCadastroFornecedor;
...
function ControleCadastroFornecedor: TControleCadastroFornecedor;
begin
  Result := TControleCadastroFornecedor(ControleMainModule.GetFormInstance(TControleCadastroFornecedor));
end;

{ TControleCadastrofornecedor }

function TControleCadastroFornecedor.Abrir(Id: Integer): Boolean;
begin
  Result := False;
...
procedure TControleCadastroFornecedor.BotaoCEPCobrancaClick(Sender: TObject);
begin
  inherited;
  try
...
procedure TControleCadastroFornecedor.BotaoCEPComercialClick(Sender: TObject);
begin
  inherited;
  try
     ACBrCEP1.BuscarPorCEP(DBEditCepGeral.Text);
...
procedure TControleCadastroFornecedor.BotaoCEPGeralClick(Sender: TObject);
var
  CidadeId: Integer;
...
procedure TControleCadastroFornecedor.BotaoGeraCPFClick(Sender: TObject);
begin
  inherited;
...
procedure TControleCadastroFornecedor.ButtonPesquisaCidadeGeralClick(
  Sender: TObject);
var
  Tipo: String;
begin
...
procedure TControleCadastroFornecedor.DbComboTipoChange(Sender: TObject);
begin
  inherited;
  // Configura os componentes de acordo com o tipo de pessoa
  ConfiguraTipoPessoa;
...
procedure TControleCadastroFornecedor.DBEditCepGeralExit(Sender: TObject);
begin
  inherited;
  BotaoCEPGeral.Click;
end;

procedure TControleCadastroFornecedor.DBEdtCpfCnpjExit(Sender: TObject);
begin
  inherited;
  with ControleMainModule do
  begin
...
function TControleCadastroFornecedor.Editar(Id: Integer): Boolean;
begin
...
function TControleCadastroFornecedor.Novo(): Boolean;
begin
  Result := False;
...
function TControleCadastroFornecedor.Salvar: Boolean;
Var
...
{    if DBEditEmailGeral.Text <> '' then
  ...
  // Passo 3 - Salva os dados do fornecedor
  if Erro = '' then
  with ControleMainModule do
  begin
    QryAx1.Parameters.Clear;
    ...
    begin
      // O id do fornecedor será igual ao id da pessoa (ver modelo E/R)
      CadastroId := PessoaId;
      // Insert
      QryAx1.SQL.Add('INSERT INTO fornecedor (');
      QryAx1.SQL.Add('       id,');
    ...
    begin
      // Update
      QryAx1.SQL.Add('UPDATE fornecedor');
...
procedure TControleCadastroFornecedor.UniFormShow(Sender: TObject);
begin
  inherited;
...
function TControleCadastroFornecedor.Descartar: Boolean;
var
  Fechar: Boolean;
begin
  inherited;
...
procedure TControleCadastroFornecedor.ConfiguraTipoPessoa;
begin
  if Copy(DBComboTipo.Text, 1, 1) = 'F' then
  begin
    // Pessoa Física
