...
uses
  Controle.Main.Module, Controle.Main,  uniGUIApplication;
...
//            ControleMain.LbPlano.Caption := 'Plano: ' + ControleMainModule.FPlano;
          ...
          //////////////////////////////////////////////////////////////////////////////
          // Juros
          if ControleMainModule.CdsParametros.Locate('campo','VALOR_JUROS',[])= True then
            ControleMainModule.percentual_juros := StrToFloatDef(ControleMainModule.CdsParametros.FieldByName('valor').AsString,0)
          else
            ControleMainModule.percentual_juros := 0;

          // Multa
          if ControleMainModule.CdsParametros.Locate('campo','VALOR_MULTA',[])= True then
            ControleMainModule.percentual_multa := StrToFloatDef(ControleMainModule.CdsParametros.FieldByName('valor').AsString,0)
          else
            ControleMainModule.percentual_multa := 0;

          // Se gera cada parcela dos titulos como o valor total (repete o valor entre parcelas , tag = 0), ou rateia o valor entre as
          // parcelas (tag = 1)
          if ControleMainModule.CdsParametros.Locate('campo','TIPO_GERACAO_TITULOS',[])= True then
            ControleMainModule.tipo_geracao_titulos := ControleMainModule.CdsParametros.FieldByName('valor').AsString
          else
            ControleMainModule.tipo_geracao_titulos := 'N';

          if ControleMainModule.CdsParametros.Locate('campo','NEGOCIAR_TITULO_VARIOS_CLIENTES',[])= True then
            ControleMainModule.negociar_titulo_varios_clientes := ControleMainModule.CdsParametros.FieldByName('valor').AsString
          else
            ControleMainModule.negociar_titulo_varios_clientes := 'N';
          //////////////////////////////////////////////////////////////////////////////
        end;
      end
      else
