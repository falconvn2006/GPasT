unit Controle.Cadastro.Representante;
...
uses
  ...
  uniDBEdit, uniMultiItem, uniComboBox, uniDBComboBox, ACBrValidador,
  uniSweetAlert;

type
  TControleCadastroRepresentante = class(TControleCadastro)
    UniLabel1: TUniLabel;
...
function ControleCadastroRepresentante: TControleCadastroRepresentante;
...
function ControleCadastroRepresentante: TControleCadastroRepresentante;
begin
  Result := TControleCadastroRepresentante(ControleMainModule.GetFormInstance(TControleCadastroRepresentante));
end;

{ TControleCadastroRepresentante }

function TControleCadastroRepresentante.Abrir(Id: Integer): Boolean;
begin
  Result := False;
...
procedure TControleCadastroRepresentante.BotaoCEPCobrancaClick(Sender: TObject);
begin
  inherited;
 try
...
procedure TControleCadastroRepresentante.BotaoCEPComercialClick(
  Sender: TObject);
begin
  inherited;
  try
...
procedure TControleCadastroRepresentante.BotaoCEPGeralClick(Sender: TObject);
var
  CidadeId: Integer;
...
procedure TControleCadastroRepresentante.BotaoGeraCPFClick(Sender: TObject);
begin
  inherited;
  if DbComboTipo.ItemIndex = 0 then
    DBEdtCpfCnpj.Text := ControleFuncoes.GeraCPF(False)
  else
    DBEdtCpfCnpj.Text := ControleFuncoes.GeraCNPJ(False);

  DBEdtNome.SetFocus;
end;

procedure TControleCadastroRepresentante.ButtonPesquisaCidadeGeralClick(
  Sender: TObject);
var
  Tipo: String;
begin
  ...
  with ControleMainModule, ControleConsultaModalCidade do
  begin
    // Identifica o tipo de endereço que está sendo editado
    if Pos('COMERCIAL', UpperCase(TUniSpeedButton(Sender).Name)) > 0 then
      Tipo := 'comercial'
...
procedure TControleCadastroRepresentante.ConfiguraTipoPessoa;
begin
...
procedure TControleCadastroRepresentante.DbComboTipoChange(Sender: TObject);
begin
  inherited;
  // Configura os componentes de acordo com o tipo de pessoa
  ConfiguraTipoPessoa;
...
procedure TControleCadastroRepresentante.DBEditCepGeralExit(Sender: TObject);
begin
  inherited;
  BotaoCEPGeral.Click;
end;

procedure TControleCadastroRepresentante.DBEdtCpfCnpjExit(Sender: TObject);
begin
  inherited;
  with ControleMainModule do
  begin
...
function TControleCadastroRepresentante.Descartar: Boolean;
var
  Fechar: Boolean;
begin
  inherited;
...
function TControleCadastroRepresentante.Editar(Id: Integer): Boolean;
begin
  Result := False;
...
function TControleCadastroRepresentante.Novo(): Boolean;
begin
  Result := False;
...
function TControleCadastroRepresentante.Salvar: Boolean;
Var
...
begin
  ...
  begin
    ...
    if (CdsCadastro.FieldByName('pessoa_id').AsInteger = 0) and
      (Trim(ControleFuncoes.RemoveMascara(DBEdtCpfCnpj.Text)) <> '00000000000') and
      (Trim(ControleFuncoes.RemoveMascara(DBEdtCpfCnpj.Text)) <> '00000000000000') then
    with ControleMainModule do
    begin
      // Pesquisa o cpf/cnpj
      CdsAx1.Close;
      ...
      begin
        if Copy(DBComboTipo.Text, 1, 1) = 'F' then
        begin
           ControleMainModule.MensageiroSweetAlerta('Atenção','O CPF informado já existe em outro cadastro (Vendedor, Fornecedor, etc..).' +
                                               'Utilize o botão de pesquisa, ao lado do campo CPF,' +
                                               'para carregar os dados do cadastro existente.');
        end
        else
        begin
           ControleMainModule.MensageiroSweetAlerta('Atenção','O CNPJ informado já existe em outro cadastro (Vendedor, Fornecedor, etc..).' +
  ...
  // Passo 3 - Salva os dados do representante
  if Erro = '' then
  with ControleMainModule do
  begin
    QryAx1.Parameters.Clear;
    ...
    begin
      // O id do representante será igual ao id da pessoa (ver modelo E/R)
      CadastroId := PessoaId;
      // Insert
      QryAx1.SQL.Add('INSERT INTO representante (');
      QryAx1.SQL.Add('       id,');
    ...
    begin
      // Update
      QryAx1.SQL.Add('UPDATE representante');
...
procedure TControleCadastroRepresentante.UniFormShow(Sender: TObject);
begin
  inherited;
