...
type
  TNNActivationFunction = (LogisticSigmoid, HyperbolicTangent, Hybrid);
...
type
  ...
  public
    ...
    constructor Create(pInputLayerSize, pHiddenLayerSize,
      pOutputLayerSize: integer);
...
constructor TNNMLP.Create(pInputLayerSize, pHiddenLayerSize,
  pOutputLayerSize: integer);
begin
  Randomize;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (RenormalizeOutput(learn, o)) then
          finish := False;
        OutputLayerDestData[o] :=
          (LearningOutputData[learn, o] - OutputLayerNormalizeMin[o]) /
          (OutputLayerNormalizeMax[o] - OutputLayerNormalizeMin[o]);
      end;
      Think();
      PropagateError();
      AdjustWeights();
    ...
    begin
      finish :=
        finish and ((OutputLayerErrorData[o] >= -LearningMinErr) and
        (OutputLayerErrorData[o] <= LearningMinErr));
      if (not finish) then
        break;
    end;
    if (finish) then
...
begin
  ...
  begin
    ...
    for i := 0 to InputLayerSize - 1 do
    begin
      HiddenLayerSumData[h] :=
        HiddenLayerSumData[h] + ((HiddenLayerWeightData[h, i] * InputLayerNormalizedData[i]) +
        HiddenLayerBiasData[h, i]);
    end;
    case ActivationFunction of
      LogisticSigmoid:
        HiddenLayerOutputData[h] :=
          1.0 / (1.0 + exp(-HiddenLayerSumData[h]));
      HyperbolicTangent, Hybrid:
        HiddenLayerOutputData[h] :=
          1.716 * tanh(0.667 * HiddenLayerSumData[h]);
    end;
  end;
  ...
  begin
    ...
    for h := 0 to HiddenLayerSize - 1 do
    begin
      OutputLayerSumData[o] :=
        OutputLayerSumData[o] + ((OutputLayerWeightData[o, h] * HiddenLayerOutputData[h]) +
        OutputLayerBiasData[o, h]);
    end;
    case ActivationFunction of
      LogisticSigmoid, Hybrid:
        OutputLayerNormalizedData[o] :=
          1.0 / (1.0 + exp(-OutputLayerSumData[o]));
      HyperbolicTangent:
        OutputLayerNormalizedData[o] :=
          1.716 * tanh(0.667 * OutputLayerSumData[o]);
    end;
  end;
  DenormalizeOutput;
end;
...
begin
  for i := 0 to InputLayerSize - 1 do
  begin
    InputLayerNormalizedData[i] :=
      (InputLayerData[i] - InputLayerNormalizeMin[i]) /
      (InputLayerNormalizeMax[i] - InputLayerNormalizeMin[i]);
    if ActivationFunction <> LogisticSigmoid then
      InputLayerNormalizedData[i] :=
        InputLayerNormalizedData[i] * 2 - 1;
  end;
end;
...
begin
  ...
  begin
    ...
    if ActivationFunction = HyperbolicTangent then
      n := (n + 1) / 2;
    OutputLayerData[o] :=
      (n * (OutputLayerNormalizeMax[o] - OutputLayerNormalizeMin[o])) +
      OutputLayerNormalizeMin[o];
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      NewMax :=
        LearningInputData[learn, i] * LearningNormalizeFactorMax;
      if NewMax = 0 then
        NewMax := 1;
      renormalize := True;
    end;
    ...
    begin
      NewMin :=
        LearningInputData[learn, i] * LearningNormalizeFactorMin;
      renormalize := True;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      NewMax :=
        LearningOutputData[learn, o] * LearningNormalizeFactorMax;
      if NewMax = 0 then
        NewMax := 1;
      renormalize := True;
    end
    ...
    begin
      NewMin :=
        LearningOutputData[learn, o] * LearningNormalizeFactorMin;
      renormalize := True;
    end;
  end;
...
begin
  ...
  begin
    case ActivationFunction of
      LogisticSigmoid, Hybrid:
        ddx :=
          OutputLayerNormalizedData[o] * (1.0 - OutputLayerNormalizedData[o]);
      // logistic sigmoid curve derivate
      HyperbolicTangent:
        ddx :=
          0.388695 * (1.716 - OutputLayerNormalizedData[o]) *
          (1.716 + OutputLayerNormalizedData[o]);
      // hiperbolic tangent curve derivate
    end;
    OutputLayerErrorData[o] :=
      ddx * (OutputLayerDestData[o] - OutputLayerNormalizedData[o]);
  end;
  for h := 0 to HiddenLayerSize - 1 do
  begin
    case ActivationFunction of
      LogisticSigmoid:
        ddx :=
          HiddenLayerOutputData[h] * (1.0 - HiddenLayerOutputData[h]);
      // logistic sigmoid curve derivate
      HyperbolicTangent, Hybrid:
        ddx :=
          0.388695 * (1.716 - HiddenLayerOutputData[h]) * (1.716 + HiddenLayerOutputData[h]);
      // hiperbolic tangent curve derivate
    end;
    HiddenLayerErrorData[h] := 0;
    for o := 0 to OutputLayerSize - 1 do
    begin
      HiddenLayerErrorData[h] :=
        HiddenLayerErrorData[h] + (ddx * (OutputLayerErrorData[o] *
        OutputLayerWeightData[o, h]));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    for i := 0 to InputLayerSize - 1 do
    begin
      HiddenLayerWeightData[h, i] :=
        HiddenLayerWeightData[h, i] + (adj * InputLayerNormalizedData[i]);
      HiddenLayerBiasData[h, i] :=
        HiddenLayerBiasData[h, i] + adj;
    end;
  end;
  for o := 0 to OutputLayerSize - 1 do
  begin
    ...
    begin
      OutputLayerWeightData[o, h] :=
        OutputLayerWeightData[o, h] + (adj * HiddenLayerOutputData[h]);
      OutputLayerBiasData[o, h] :=
        OutputLayerBiasData[o, h] + adj;
    end;
  end;
end;
