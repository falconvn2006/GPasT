...
constructor TLotoIni.Create;
begin
   inherited Create;
   Load;
end;

procedure TLotoIni.Load;
var oIni : TINIFile;
begin
   Slash := PathDelim;
   ...
   {$if defined(windows)}
        ...
        AppPath := IncludeTrailingPathDelimiter(GetUserDir) + 'GLotofacil';
        WGetPath := 'wget.exe';
   {$else}
          TempPath := '/tmp';
          AppPath := IncludeTrailingPathDelimiter(GetUserDir) + '.GLotofacil';
   ...
   {$ifend}
   DataPath := AppPath;
   DataFileName := 'glotofacil.csv';
   ImportMode := '0';
   ImportURL := 'http://www1.caixa.gov.br/loterias/_arquivos/loterias/D_lotfac.zip';
   ImportJson := 'https://servicebus2.caixa.gov.br/portaldeloterias/api/lotofacil';
   ImportFileName := 'd_lotfac.htm';
   BetsMaxSearch := 10000;
   ...
   oIni := TINIFile.Create( IncludeTrailingPathDelimiter(AppPath) + 'glotofacil.ini' );
