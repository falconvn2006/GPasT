...
{ TACBrHTTP }
  ...
  public
    ...
    procedure HttpPostURLteste(URL: string; urldata : String);
...
var
  ...
  {$IFDEF UPDATE_SCREEN_CURSOR}
   OldCursor : TCursor ;
  {$ENDIF}
   CT, Location, HtmlHead, AUTH: String ;
...
begin
  ...
  try
    ...
    // DEBUG
//    HTTPSend.Document.SaveToFile( 'C:\Users\Dev11\Desktop\' );
//    HTTPSend.Document.SaveToFile( 'C:\temp\HttpSend.txt' );
    ...
    // DEBUG //
    RespHTTP.SaveToFile('c:\temp\HttpResp.json');
    //HTTPSend.Headers.SaveToFile('c:\temp\HeaderResp.txt');
    ...
    begin
      ...
      AUTH  := LowerCase( GetHeaderValue('Authorization:') );
...
procedure TACBrHTTP.HttpPostURLteste(URL: string; urldata : String);
var
    vURL: string;
    vParams: string;
...
begin
  vResponse := TMemoryStream.Create;
   
  try
  //  URL := 'http://posttestserver.com/post.php?dump&html';

    vParams :=urldata;
    HTTPSend.Headers.Text;
     HttpPostURL(URL, vParams, vResponse);
      vResponse.SaveToFile('c:\response.txt');
