...
procedure TTelephones.quitExecute(Sender: TObject);//выход
begin
  Telephones.Close;
end;

procedure TTelephones.aboutExecute(Sender: TObject); //о программе
begin
  AboutForm.ShowModal;
end;

procedure TTelephones.FindExecute(Sender: TObject);   //поиск полей по заданному тексту
var
  b:boolean;
  PtrTel:PTelList;
  i:word;
...
procedure TTelephones.FindNextExecute(Sender: TObject);//поиск полей по заданному тексту
begin
  //если ссылка на тему - переход к теме
...
begin
  //проверка установки фокуса на окна Part, Name, Tel
  if Names.Focused=True then
  begin
    if Tema.Items.Count=0 then NewElem.Enabled:=false
    else begin
      NewElem.OnExecute:=N3Click; //если окно Names в фокусе
    ...
    begin
      if Names.Items.Count=0 then NewElem.Enabled:=false
        ...
        NewElem.OnExecute:=N6Click; //если окно Teleph в фокусе
        NewElem.Enabled:=true;
      end
    end
    else begin
      NewElem.OnExecute:=N2Click; //если окно Tema или что-то другое в фокусе
      NewElem.Enabled:=true;
    end;
  end;
end;
...
begin
  ...
  begin
    if Names.Items.Count=0 then DelElem.Enabled:=false
      ...
      DelElem.OnExecute:=N4Click; //если окно Names в фокусе
    ...
    begin
      if Teleph.Items.Count=0 then DelElem.Enabled:=false
        ...
        DelElem.OnExecute:=N8Click; //если окно Teleph в фокусе
        DelElem.Enabled:=true;
      end
    end
    else begin
      if Tema.Items.Count=0 then DelElem.Enabled:=false
        ...
        DelElem.OnExecute:=N9Click; //если окно Tema или что-то другое в фокусе
        DelElem.Enabled:=true;
      end;
    end;
  end;
...
begin
  ...
  begin
    if Names.Items.Count=0 then EditElem.Enabled:=false
      ...
      EditElem.OnExecute:=N5Click; //если окно Names в фокусе
    ...
    begin
      if Teleph.Items.Count=0 then EditElem.Enabled:=false
        ...
        EditElem.OnExecute:=N7Click; //если окно Teleph в фокусе
        EditElem.Enabled:=true;
      end
    end
    else begin
      if Tema.Items.Count=0 then EditElem.Enabled:=false
        ...
        EditElem.OnExecute:=N10Click; //если окно Tema или что-то другое в фокусе
        EditElem.Enabled:=true;
      end;
    end;
  end;
  ...
  begin
    ...
    begin
     ...
     // CommentButton.Indeterminate:=true;
      ...
      Comments.OnExecute:=N1Click; //если окно Names в фокусе
    ...
    begin
      if Teleph.Items.Count=0 then Comments.Enabled:=false
        ...
        Comments.OnExecute:=N11Click; //если окно Teleph в фокусе
        Comments.Enabled:=true;
      end
    end
