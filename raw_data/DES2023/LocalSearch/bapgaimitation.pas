...
interface
 uses
  ...
  Classes;
 type
  PTBAPGAImitation = ^TBAPGAImitation;
  TBAPGAImitation = class(TSolver)
  private
   _order_vector                    :array of integer;
   _Task                          :TBAPImitation;
   _NewRecord                       :TBAPSolution;
   _NewSolution                     :TBAPSolution;
   _NewSolution_                     :TBAPSolution;
...
{LocalSearch*******************************************************************}
   ...
   Procedure LocalSearch_WhileGood(numberpopulation,numberindivid:integer);
   Procedure LocalSearch_LocalDescentWithWhileGood(numberpopulation,numberindivid:integer);
...
implementation
 uses
  ...
  FOrder;
{******************************************************************************}
 Constructor TBAPGAImitation.Create;
 begin
  inherited;
  _Task:=TBAPImitation.Create;
  _NewRecord:=TBAPSolution.Create;
  _NewSolution:=TBAPSolution.Create;
  _NowRecord:=TBAPSolution.Create;
  _NewSolution_:=TBAPSolution.Create;
 end;
{******************************************************************************}
 Destructor TBAPGAImitation.Destroy;
 begin
...
{******************************************************************************}
 Procedure TBAPGAImitation.GetMemory;
 var i,x:integer;
 begin
  ...
  _NewRecord.Size:=_Task.Size;
  _NewSolution.Size:=_Task.Size;
  _NowRecord.Size:=_Task.Size;
  _NewSolution_.Size:=_Task.Size;
...
{******************************************************************************}
 Procedure TBAPGAImitation.FreeMemory;
 var i:integer;
 begin
...
{******************************************************************************}
 Procedure TBAPGAImitation.Load(var f : TstringList);
 var
  i,j,t,cod  :integer;
  s,k    :string;
...
{******************************************************************************}
 Procedure TBAPGAImitation.FirstApproximation;
 var i,j:integer;
 begin
...
{Selection*********************************************************************}
 Procedure TBAPGAImitation.turnament_selection;
 var
  i,j,z,best:integer;
 begin
...
{Crossover*********************************************************************}
 Procedure TBAPGAImitation.one_point_crossover;
 var
  i,k,l:integer;
  p:double;
  s:String;
...
{Mutation**********************************************************************}
 Procedure TBAPGAImitation.Mutation1(p:integer;i:integer;p_mut:double);
 var
  j:integer;
  yy:double;
 begin
 ...
 {******************************************************************************}
  Procedure TBAPGAImitation.Mutation3(p:integer;i:integer;p_mut:double);
  var
   j:integer;
   new_val:integer;
   from_rnd:integer;
...
{LocalSearch*******************************************************************}
 procedure TBAPGAImitation.LocalSearch_Hamming(numberpopulation,indexindivid:integer);
 var
  i       :integer;
  p       :PTBAPSolution;
 begin
...
{Add***************************************************************************}
 ...
 {******************************************************************************}
  procedure TBAPGAImitation.LocalSearch_WhileGood(numberpopulation,numberindivid:integer);
  var
   p      :PTBAPSolution;
   i,j    :integer;
  begin
 ...
 {oder vector-------------------------------------------------------------------}
   RandomOrder(_order_vector);
 {------------------------------------------------------------------------------}
   for i:=0 to Task.Size_ do
   begin

    _NewSolution_.Assign(p^);
    _NewSolution.Assign(p^);
    j:=_order_vector[i];
 ...
 {while better go up------------------------------------------------------------}
    ...
    begin
     ...
     if p^.element[j]+1 > task.GetUpBoundary(j) then break;
     _NewSolution_.element[j]:=p^.element[j]+1;
     Task.SetValue(_NewSolution_);

     if _NewSolution_.getvalue <= p^.getvalue then break;
     p^.Assign(_NewSolution_);
    end;
    _NewSolution_.Assign(p^);


 {while better go down----------------------------------------------------------}
    p^.Assign(_NewSolution);
    while true do
    begin

     if p^.element[j]-1 < 0 then break;
     _NewSolution.element[j]:=p^.element[j]-1;
     Task.SetValue(_NewSolution);

     if _NewSolution.getvalue <= p^.getvalue then break;
     p^.Assign(_NewSolution);
    end;
 {------------------------------------------------------------------------------}

    if _NewSolution_.getvalue >= p^.getvalue then p^.Assign(_NewSolution_);
   end;
 {delete------------------------------------------------------------------------}
  end;
 {******************************************************************************}
  procedure TBAPGAImitation.LocalSearch_LocalDescentWithWhileGood(numberpopulation,numberindivid:integer);
  var
   p        :PTBAPSolution;
   k,i      :integer;
   s        :string;
 ...
 {help individ------------------------------------------------------------------}
   ...
   begin
 {find better individ-----------------------------------------------------------}
    LocalSearch_WhileGood(numberpopulation, numberindivid);
 {if no change then stop--------------------------------------------------------}
    if (p^.getvalue = _NewRecord.getvalue) and (p^.IsEqual(_NewRecord)=true) then break;
    _NewRecord.Assign(p^);
   end;
  end;
 {******************************************************************************
  procedure TBAPGAImitation.LocalSearch_LocalDescentWithHamming(numberpopulation,numberindivid:integer);
  var
   p        :PTSolution;
   k,i      :integer;
   s        :string;
 ...
 {if no change then stop--------------------------------------------------------}
    if p^.Equal(_NewRecord)=true then  break;
    _NewRecord.Assign(p^);
   end;
 {delete------------------------------------------------------------------------}
  end;
 ******************************************************************************}
{GA****************************************************************************}
 procedure TBAPGAImitation.MutationVariationForPopulation(
           popindex:integer;
           mindex:integer;
           mparam:double;
           LSindex:integer
...
{******************************************************************************}
 procedure TBAPGAImitation.MutationVariationForIndividual(
           popindex:integer;
           indindex:integer;
           mindex:integer;
           mparam:double;
 ...
 begin
     ...
     case LSindex of
      3:LocalSearch_WhileGood(popindex,indindex);
      5:LocalSearch_LocalDescentWithWhileGood(popindex,indindex);
      8:LocalSearch_Hamming(popindex,indindex);
     end;
  end;
{******************************************************************************}
 procedure TBAPGAImitation.Iteration;
 begin
...
{******************************************************************************}
 function TBAPGAImitation.EnabledLocalSearch:boolean;
 begin
...
{******************************************************************************}
 Function TBAPGAImitation.GetPopulationQuantity_:integer;
 begin
  GetPopulationQuantity_:=PopulationQuantity-1;
 end;
 {******************************************************************************}
Function TBAPGAImitation.GetNowRecord:TSolution;
begin
 GetNowRecord:=TSolution(_NowRecord);
end;
   {******************************************************************************}
  Function TBAPGAImitation.GetProblem:TProblem;
  begin
   GetProblem:= _Task;
  end;

  {******************************************************************************}
  Procedure TBAPGAImitation.SetProblem(x:TProblem);
  begin
   _Task:=TBAPImitation(x);
  end;

 {******************************************************************************}
 {******************************************************************************}
 procedure TBAPGAImitation.ShowDebug;
 var
 i:integer;
 s:string;
 begin
 ...
 {******************************************************************************}
 procedure TBAPGAImitation.ShowRecord;
  var
 i:integer;
 s:string;
  begin
  ...
  {******************************************************************************}
 procedure TBAPGAImitation.SaveDebug;
  var
 i:integer;
 f:TextFile;
    s:string;
  ...
  {******************************************************************************}
 procedure TBAPGAImitation.SaveRecord;
    var
    f:TextFile;
       s:string;
   begin
