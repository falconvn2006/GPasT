...
{******************************************************************************}
 Function TMachineLine.R1:TIndex;
 var
  i,minindex : TInt;
  minsize : TReal;
 begin
...
{******************************************************************************}
 Function TMachineLine.R1(ind:TIndex):TIndex;
 var
  i,minindex : TIndex;
  minsize : TReal;
 begin
  ...
  begin
   ...
   (CBuffer[i] = True) and 
   (i<>ind) and 
   (Buffer[i].Size <= minsize) and 
   (Buffer[i].PDMQuantity = 1) and 
   (Buffer[i].TDMQuantity = 1) 
   then
   begin
    minsize:=Buffer[i].Size;
    minindex:=i;
  ...
  if minindex = 0 then R1:= ind
  else R1:= minindex;
 end;
{******************************************************************************}
 Function TMachineLine.R1f:TIndex;
 var
  i,index : TInt;
 begin
  index:=0;
 ...
 {******************************************************************************}
 Procedure TMachineLine.AverageStock();
 var
  ihm: TInt;
  avg_stock,size        : TReal;

 begin
//  for ihm:=1 to BufferQuantity_ do
//  begin
    ihm:=4;
...
{******************************************************************************}
 function TMachineLine.GetAverageStockForBuffer(ind:TIndex):TReal;
  var
//  order1:TIntVector;
   b                 : Boolean;
   ihm,pos,i,j,k,t,tt,hh : TInt;
  ...
  begin
    ...
    GetAverageStockForBuffer:=avg_stock;
  end; 
