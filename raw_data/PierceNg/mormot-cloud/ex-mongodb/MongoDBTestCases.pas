...
const
  //
  MONGOSERVER = 'XXX CHANGE ME XXX';
  //MONGOSERVER = '10.0.2.2'; // e.g. from a VirtualBox VM
  MONGOPORT = 10255; // Change if necessary
...
const
  DB_NAME = 'mormot';
  USER_NAME = 'XXX CHANGE ME XXX';
  USER_PWD = 'XXX CHANGE ME XXX';
...
begin
  assert(fClient = nil);
  {$ifdef TESTMONGOAUTH}
  // Commented out because user has been pre-created
  ...
  {$endif TESTMONGOAUTH}
  fClient := TMongoClient.Create(MONGOSERVER, MONGOPORT, [mcoTls]);
...
begin
  fMongoClient := TMongoClient.Create(MONGOSERVER, MONGOPORT, [mcoTls]);
