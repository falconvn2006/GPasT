...
(* ¹¦ÄÜ¿ÉÑ¡ÏîÌõ¼þ±àÒë *)
{$DEFINE USEYxdStr}    // ÊÇ·ñÊ¹ÓÃYxdStrµ¥Ôª
{$DEFINE USERTTI}      // ÊÇ·ñÊ¹ÓÃRTTI¹¦ÄÜ
{$DEFINE USERegEx}     // ÊÇ·ñÊ¹ÓÃÕýÔò±í´ïÊ½ËÑË÷¹¦ÄÜ£¬D2010Ö®Ç°°æ±¾ÐèÒªÒýÓÃÏà¹Øµ¥Ôª
{$DEFINE USEBase64}    // ÊÇ·ñÊ¹ÓÃBase64µ¥Ôª
{.$DEFINE USEPool}      // ÊÇ·ñÊ¹ÓÃ³Ø (·þÎñ¶ËÊ¹ÓÃ½¨Òé¿ªÆô)

{$IFDEF USERTTI}
  {$DEFINE USEDBRTTI}    // ÊÇ·ñÊ¹ÓÃDataSetÐòÁÐ»¯¹¦ÄÜ£¬±ØÐëÏÈÆôÓÃUSERTTI
{$ENDIF}

(* Delphi °æ±¾¿ØÖÆÌõ¼þ±àÒë *)
//Delphi 2007
{$IFDEF VER185}
{$DEFINE JSON_SUPPORT}
{$ENDIF}                         
...
{$IFNDEF USEYxdStr}
type
  TTextEncoding = (teUnknown, {Î´ÖªµÄ±àÂë} teAuto,{×Ô¶¯¼ì²â} teAnsi, { Ansi±àÂë }
    teUnicode16LE, { Unicode LE ±àÂë } teUnicode16BE, { Unicode BE ±àÂë }
    teUTF8 { UTF8±àÂë } );
{$ENDIF}
...
type
  ...
  TJsonDateFormatStyle = (
    jdsNormal {´«Í³µÄÊä³ö·½Ê½£¬¸ù¾ÝJsonDateTimeFormatµÈÉèÖÃÀ´Êä³ö×Ö·û´®},
    jdsJsonTime {JSON±ê×¼Ê±¼ä´ÁÊä³ö·½Ê½}
  );
...
type
  ...
  public
    ...
    property AsVariant: Variant read GetAsVariant write SetAsVariant; // ½öÖ§³Ö³£ÓÃÀàÐÍ
    ...
    property AsStream: TStream read GetAsStream write SetAsStream; // ×¢Òâ£¬·µ»ØµÄÁ÷ÐèÒªÊÍ·Å
  ...
  private
    ...
    FItems: JSONList;
    FData: Pointer;
    FValue: PJSONValue; // FParent²»ÎªnilÊ±, FValue±Ø¶¨²»Îªnil
  ...
  public
    ...
    function GetStream(const Key: JSONString): TStream;  // ·µ»ØµÄÁ÷ÐèÒªÊÖ¶¯ÊÍ·Å
...
resourcestring
  ...
  SJsonParseError = '%dJsonParse%dError: %s '#13#10'ÐÐ: %s';
...
begin
  ...
  if Result > 64 then begin
    // ¿é´óÐ¡ÒÔ64×Ö½Ú¶ÔÆë£¬ÕâÑùµÄÖ´ÐÐÐ§ÂÊ×î¸ß
...
{$IFNDEF USEYxdStr}
//¼ÆËãµ±Ç°×Ö·ûµÄ³¤¶È
// GB18030,¼æÈÝGBKºÍGB2312
// µ¥×Ö½Ú£¬ÆäÖµ´Ó0µ½0x7F¡£
// Ë«×Ö½Ú£¬µÚÒ»¸ö×Ö½ÚµÄÖµ´Ó0x81µ½0xFE£¬µÚ¶þ¸ö×Ö½ÚµÄÖµ´Ó0x40µ½0xFE£¨²»°üÀ¨0x7F£©¡£
// ËÄ×Ö½Ú£¬µÚÒ»¸ö×Ö½ÚµÄÖµ´Ó0x81µ½0xFE£¬µÚ¶þ¸ö×Ö½ÚµÄÖµ´Ó0x30µ½0x39£¬µÚÈý¸ö×Ö½Ú´Ó0x81µ½0xFE£¬µÚËÄ¸ö×Ö½Ú´Ó0x30µ½0x39¡£
function CharSizeA(c: PAnsiChar): Integer;
begin
  {$IFDEF MSWINDOWS}
...
//    ps := Key;
//    lr := (KeyLen and $03);//¼ì²é³¤¶ÈÊÇ·ñÎª4µÄÕûÊý±¶
//    KeyLen := (KeyLen and $FFFFFFFC);//ÕûÊý³¤¶È
...
begin
  ...
  if s^='-' then begin
    ...
    if s^<>#0 then begin
      if not ParseNum(H) then begin //Ã»¸úÊ±¼äÖµ
        AResult:=ADate;
        Exit;
      end;
      if s^<>':' then begin
...
begin
  //Ìø¹ýÐÇÆÚ£¬Õâ¸ö¿ÉÒÔÖ±½ÓÍ¨¹ýÈÕÆÚ¼ÆËã³öÀ´£¬²»ÐèÒª
...
begin
  if p=nil then
    ...
    Result.Length:=l*6;
    {$ELSE}
    SetLength(Result, l*6);//UTF8Ã¿¸ö×Ö·û×î¶à6×Ö½Ú³¤,Ò»´ÎÐÔ·ÖÅä×ã¹»µÄ¿Õ¼ä
    {$ENDIF}
    if l>0 then begin
      ...
      while l>0 do begin
        c:=Cardinal(ps^);
        Inc(ps);
        if (c>=$D800) and (c<=$DFFF) then begin//Unicode À©Õ¹Çø×Ö·û
          c:=(c-$D800);
        ...
        Dec(l);
        if c=$0 then begin
          if JavaFormatUtf8 then begin//°´ÕÕJava¸ñÊ½±àÂë£¬½«#$0×Ö·û±àÂëÎª#$C080
  ...
  while Integer(ps)<Integer(pe) do begin
    if (ps^ and $80)<>0 then begin
      if (ps^ and $FC)=$FC then begin //4000000+
        ...
        raise Exception.Create(Format('ÎÞÐ§µÄUTF8×Ö·û:%d',[Integer(ps^)]));
    end else begin
      pd^ := ps^;
      Inc(ps);
      Inc(pd);
...
begin
  ...
  s := p;
  while p^ <> #0 do begin
    if p^ = AQuoter then begin //ÒýÓÃµÄÄÚÈÝ²»²ð·Ö
      Inc(p);
      while p^<>#0 do begin
        if p^=#$5C then begin
          Inc(p);
  ...
  if L >= 2 then begin
    ...
    if pWide^ = #$FEFF then
      ...
      if (pAnsi^ = $EF) and (PByte(IntPtr(pAnsi) + 1)^ = $BB) and
        ...
        Result := teUTF8
      else begin// ¼ì²â×Ö·ûÖÐÊÇ·ñÓÐ·ûºÏUFT-8±àÂë¹æÔòµÄ×Ö·û£¬11...
        b := false;
        Result := teUTF8;//¼ÙÉèÎÄ¼þÎªUTF8±àÂë£¬È»ºó¼ì²âÊÇ·ñÓÐ²»·ûºÏUTF-8±àÂëµÄÐòÁÐ
        I := 0;
        Dec(L, 2);
        while I<=L do begin
          if (pAnsi^ and $80) <> 0 then begin // ¸ßÎ»Îª1
            if IsUtf8Order(AUtf8CharSize) then begin
              if AUtf8CharSize>2 then//³öÏÖ´óÓÚ2¸ö×Ö½Ú³¤¶ÈµÄUTF8ÐòÁÐ£¬99%¾ÍÊÇUTF-8ÁË£¬²»ÔÙÅÐ¶Ï
                Break;
            ...
            if pAnsi^=0 then begin //00 xx (xx<128) ¸ßÎ»ÔÚÇ°£¬ÊÇBE±àÂë
              if PByte(IntPtr(pAnsi)+1)^<128 then begin
                Result := teUnicode16BE;
                Break;
              end;
            end else if PByte(IntPtr(pAnsi)+1)^=0 then begin//xx 00 µÍÎ»ÔÚÇ°£¬ÊÇLE±àÂë
              Result:=teUnicode16LE;
              Break;
            end;
            Inc(pAnsi);
...
begin
  ADate := Trunc(AValue);
  if SameValue(ADate, 0) then begin //DateÎª0£¬ÊÇÊ±¼ä
...
const
  ...
  CSBlobBase64: PJSONChar = '[BS]'; //Ê¹ÓÃBase64±àÂëBlobÊ±µÄÊ¶±ðÇ°×º
...
begin
  ...
  if (FParent = nil) and (FValue <> nil) and (FValue.FType = jdtUnknown) then
    FreeJsonValue(FValue); // JSONÅFValue
  inherited;
end;
...
////////////////////STS//////////////////////////
// Modified by STS 02.05.2019 09:22:23 Âîçâðàò êîë-âà îáúåêòîâ â Json
function JSONBase.GetCountObj: Integer;
begin
   Result := FItems.Count;
end;

//Modified by STS 13.03.2019 08:06:03  çàïÿòóþ ïðåîáðàçîâàòü â òî÷êó
...
//Óäàëåíèå èç ñòðîêè ñïåö ñèìâîëîâ 
...
// Modified by STS 02.05.2019 08:37:43  Âûáîðêà âñåõ çíà÷åíèé âñåõ êëþ÷åé - Key
    ...
    if ANode.FItems.Count > 0 then begin
      ...
      for I := 0 to ANode.FItems.Count - 1 do begin
       ...
       if (item.FName <> '') or (not IsArray) then begin
          ...
          try
              Y := Pos(Item.FName,Key);
           if Y > 0 then begin //óñëîâèÿ ïðîâåðêè ...
           case Item.FType of
                jdtString:
                begin
    ...
    if ANode.FItems.Count > 0 then begin
      ...
      for I := 0 to ANode.FItems.Count - 1 do begin
       ...
       if (item.FName <> '') or (not IsArray) then begin
          ...
          ABuilder.Cat(CharNameEnd, 2 + Ord(JsonNameAfterSpace));
          try
           //if item.GetString = '2680913' then MessageBox(0,PChar('ÒÅÑÒ'),PChar('ÒÅÑÒ'),64);
              Y := Pos(Item.FName,Key);
           if Y > 0 then begin
...
begin
  ...
  while Assigned(AParent) and (p^ <> #0) do begin
    ...
    if Length(AName) > 0 then begin       
      ...
      AIndex := -1;
      pn := PJSONChar(AName);
      if (pn[l - 1] = ']') then begin // ²éÕÒµÄÊÇÊý×é£¿
        repeat
          if pn[l] = '[' then begin
            ws := pn + l + 1;
            ParseInt(ws, AIndex);
...
begin
  ...
  if p^ = '"' then begin
    ...
    BuildJsonString(ABuilder, p);
    JSONObject(Self).put(FName, ABuilder);
  end else if ParseNumeric(p, ANum) then begin //Êý×Ö£¿
    {$IFDEF JSON_UNICODE}SkipSpaceW{$ELSE}SkipSpaceA{$ENDIF}(p);
    ...
    Inc(p,4);
    {$IFDEF JSON_UNICODE}SkipSpaceW{$ELSE}SkipSpaceA{$ENDIF}(p);
    JSONObject(Self).put(FName, NULL);  // ÍøÓÑ¡°ÖÐ¹úÖÆÔì¡±±¨¸æ 2014.08.01
  end else if p^ = '{' then begin
...
begin
  ...
  if p^ = '{' then begin
    ...
    while (p^<>#0) and (p^ <> '}') do begin
      ...
      //  Exit;
      //end;  // Ãû³ÆÔÊÐíÎª¿Õ by 2018.01.25
      Inc(p);
      {$IFDEF JSON_UNICODE}SkipSpaceW{$ELSE}SkipSpaceA{$ENDIF}(p);
      //½âÎöÖµ
...
begin
  //Modified by STS 07.03.2019 15:31:11 - Îáðàáîòêà îøèáîê ...
  if ACode<>0 then begin
    case ACode of
...
// Modified by STS 18.11.2019 11:12:16 - ïåðåäà÷à ðåçóëüòàòà
