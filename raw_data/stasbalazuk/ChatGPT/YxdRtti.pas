...
{$DEFINE USEIniSerialize}         // Ê¹ÓÃINIÐòÁÐ»¯Ä£¿é
{$DEFINE USEXmlSerialize}         // Ê¹ÓÃXMLÐòÁÐ»¯Ä£¿é
{$DEFINE USEJsonSerialize}        // Ê¹ÓÃJsonÐòÁÐ»¯Ä£¿é
{$DEFINE USEDataSet}              // ÊÇ·ñÊ¹ÓÃDataSetÐòÁÐ»¯¹¦ÄÜ
...
type
  ...
  /// ÐòÁÐ»¯ÀàÐÍ
  /// </summary>
  TSerializeType = (afXML,{XML¸ñÊ½} afIni,{iniÎÄ¼þ} afJson {json¸ñÊ½});
...
{$IFDEF USE_UNICODE}
//ÐÞÕýXE6ÖÐSystem.rttiÖÐTValue¶ÔtkSetÀàÐÍ´¦ÀíµÄBug
...
begin
  if not Assigned(AStream) then
    raise Exception.Create('²Î Stream Õ');
  Data := ToString;
...
class procedure TYxdSerialize.Serialize(Writer: TSerializeWriter; const Key: string;
  ...
  begin
    ...
    //Èç¹ûÊÇ´Ó½á¹¹Ìå£¬Ôò¼ÇÂ¼Æä³ÉÔ±£¬Èç¹ûÊÇ¶ÔÏó£¬ÔòÖ»¼ÇÂ¼Æä¹«¿ªµÄÊôÐÔ£¬ÌØÊâ´¦ÀíTStringsºÍTCollection
    ...
    try
      for J := 0 to ACount - 1 do begin
        ...
        case APropList[J].PropType^.Kind of
          ...
          tkRecord, tkArray, tkDynArray: //¼ÇÂ¼¡¢Êý×é¡¢¶¯Ì¬Êý×éÊôÐÔÏµÍ³Ò²²»±£´æ£¬Ò²Ã»Ìá¹©ËùÓÐÌ«ºÃµÄ½Ó¿Ú
...
begin
  ...
  try
    if (not Assigned(Meta)) then begin // Ã»ÓÃMetaÊý¾Ý£¬´ÓÊý¾Ý×Ö¶ÎÖÐ»ñÈ¡
      ...
      for J := 0 To ItemObject.Count - 1 do begin
        Item := ItemObject[J];
        if Item.FType = jdtNull then begin
          if Length(Item.FName) > 0 then  // ´ÓÏÂÒ»Ìõ¼ÇÂ¼ÖÐ²éÕÒ
    ...
    try
      ...
      raise Exception.CreateFmt('json½«×Ö¶Î(%s)¸³Öµ¸øÊý¾Ý¼¯·¢ÉúÒì³££¡', [FldName]);
    end;
  finally
    ...
    for J := 0 to aIn.Count - 1 do begin
      ...
      if AProp <> nil then begin
        case AProp.PropType^.Kind of
          ...
          tkRecord, tkArray, tkDynArray://tkArray,tkDynArrayÀàÐÍµÄÊôÐÔÃ»¼û¹ý,tkRecord´æÒÉ
