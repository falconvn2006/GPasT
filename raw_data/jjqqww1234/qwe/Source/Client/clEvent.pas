...
begin
  if dsurface <> nil then
    if BoBlend then
      DrawBlend(backsurface, ax + px, ay + py, dsurface, 1)
    else
      backsurface.Draw(ax + px, ay + py, dsurface.ClientRect, dsurface, True);
end;
  ...
  case EventType of
    ...
    ET_PILESTONES: begin
      ...
      if EventParam > 5 then
        EventParam := 5;
      dsurface := FrmMain.WEffectImg.GetCachedImage(STONEFRAGMENTBASE +
        (EventParam - 1), px, py);
    end;
    ET_HOLYCURTAIN: begin
      dsurface := FrmMain.WMagic.GetCachedImage(HOLYCURTAINBASE +
        (curframe mod 10), px, py);
      BoBlend  := True;
      light    := 1;
    end;
    ET_FIRE: begin
      dsurface := FrmMain.WMagic.GetCachedImage(FIREBURNBASE +
        ((curframe div 2) mod 6), px, py);
      BoBlend  := True;
      light    := 1;
    end;
    ET_METEOR: begin
      dsurface := FrmMain.WMagic2.GetCachedImage(1601 +
        ((curframe) mod 9), px, py);
      BoBlend  := True;
      light    := 1;
    end;
    ET_SCULPEICE: begin
...
begin
  for i := 0 to EventList.Count - 1 do
    if (EventList[i] = evn) or (TClEvent(EventList[i]).ServerId = evn.ServerId) then
    begin
      evn.Free;
      Result := nil;
      exit;
