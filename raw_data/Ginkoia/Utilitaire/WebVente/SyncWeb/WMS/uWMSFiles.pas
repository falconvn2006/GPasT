...
type
  ...
  private
    ...
    procedure InitHeaderOL;
  protected
    FHeaderOL: TExportHeaderOL;

    FVersions: Array of TFileLastVersion;
    FCurrentVersionLame, FCurrentVersionBase: integer;
    FInitMode: boolean;
    FCurrentFileName: string;

    procedure startWrite(AInitMode: boolean; ACurrentVersionBase, ACurrentVersionLame: integer);
    ...
    procedure setVersion(AType: TSendFileLastVersionType; ANewVersion: integer);
  ...
  public
    constructor Create(Const AConnection: TMyConnection; Const AFileName: string;
                       ...
                       AUniqueField: string = ''; AExcludeExportFields: string = ''); overload;

    destructor Destroy;

    procedure UpdateVersions(ABaseVersion, ALameVersion: integer; AAssId: integer);

    function Execute(APath: string; AInitMode: boolean; ACurrentVersionBase,
      ACurrentVersionLame: integer): boolean;

    function addVersion(AMagId: integer; AId: integer; AVersion: integer;
...
function TCustomFTPSendFile.addVersion(AMagId: integer; AId: integer;
  AVersion: integer; ADate: TDateTime): boolean;
var
  index: integer;
begin
...
constructor TCustomFTPSendFile.Create(Const AConnection: TMyConnection; Const AFileName: string;
  Const AMonitoringAppType: TMonitoringAppType; AUniqueField: string; AExcludeExportFields: string);
begin
...
function TCustomFTPSendFile.Execute(APath: string; AInitMode: boolean;
  ACurrentVersionBase, ACurrentVersionLame: integer): boolean;
begin
  try
    if CanExecute(AInitMode) then
    begin
...
begin
  ...
  begin
    if Assigned(AQuery.Params.FindParam('LAST_VERSION')) then
      AQuery.Params.ParamByName('LAST_VERSION').AsInteger := getTableVersion;
    if Assigned(AQuery.Params.FindParam('LAST_VERSION_LAME')) then
      AQuery.Params.ParamByName('LAST_VERSION_LAME').AsInteger := getLameVersion;

    if Assigned(AQuery.Params.FindParam('CURRENT_VERSION')) then
      AQuery.Params.ParamByName('CURRENT_VERSION').AsInteger := FCurrentVersionBase;
    if Assigned(AQuery.Params.FindParam('CURRENT_VERSION_LAME')) then
      AQuery.Params.ParamByName('CURRENT_VERSION_LAME').AsInteger := FCurrentVersionLame;

    if Assigned(AQuery.Params.FindParam('DEBUT_PLAGE_BASE')) then
      AQuery.Params.ParamByName('DEBUT_PLAGE_BASE').AsInteger := Dm_Common.PlageBase.Debut;
    if Assigned(AQuery.Params.FindParam('FIN_PLAGE_BASE')) then
      AQuery.Params.ParamByName('FIN_PLAGE_BASE').AsInteger := Dm_Common.PlageBase.Fin;
  end;
end;
...
begin
  if FInitMode then
    FCurrentFileName := 'INIT_' + FDefaultFileName + '.TXT'
  else
    FCurrentFileName := Format('%s_%s.TXT', [FDefaultFileName, FormatDateTime('yyyymmdd_hhnnss', Now())]);
end;
...
procedure TCustomFTPSendFile.startWrite(AInitMode: boolean; ACurrentVersionBase, ACurrentVersionLame: integer);
begin
...
procedure TCustomFTPSendFile.UpdateVersions(ABaseVersion,
  ALameVersion: integer; AAssId: integer);
var
  i: integer;
  query: TMyQuery;
...
begin
  ...
  begin
    ...
    case FVersions[i].lvType of
      sflvtTable:
      begin
        query.Params.ParamByName('version').AsInteger := ABaseVersion;
        BaseVersionUpdated := True;
        setVersion(sflvtTable, ABaseVersion);
      end;
      sflvtLame:
      begin
        query.Params.ParamByName('version').AsInteger := ALameVersion;
  ...
  begin
    ...
    query.Params.ParamByName('LASTVERSION').AsInteger  := ABaseVersion;
  ...
  begin
    ...
    query.Params.ParamByName('ACTIF').AsInteger        := 1;
    query.Params.ParamByName('DATEEXPORT').AsDateTime  := Now();
    query.Params.ParamByName('LASTVERSION').AsInteger  := ALameVersion;
