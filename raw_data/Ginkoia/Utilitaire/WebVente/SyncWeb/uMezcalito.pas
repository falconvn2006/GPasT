...
begin
  ...
  try
    ...
    Dm_Common.FTP.Address            := FTPCfg.Host;
    Dm_Common.FTP.Username        := FTPCfg.User;
    Dm_Common.FTP.Password        := FTPCfg.Psw;
    Dm_Common.FTP.Port            := FTPCfg.Port;
    Dm_Common.FTP.PassiveMode         := True;
    Dm_Common.FTP.SocketTimeout  := Dm_Common.MyIniParams.iConnect; // (3s)
  ...
  try
    ...
    try
      ...
      begin
        ...
        bDel := not Dm_Common.FTP.FileExists(sFile);
        iTry := 0;
        ...
        begin
          try
            Dm_Common.FTP.UploadFile(FTPCfg.SourcePath + sFile, sFile, ftmOverwrite);
            bSend := True;
          except
          end; // try
          if not(bSend) then
