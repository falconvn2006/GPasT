...
// fonction permettant de créer un zip d'un fichier
function ZipFile(ASourceFile, AZipName : String) : Boolean;
// Fonction permettant de créer un Zip d'un dossier
function ZipFolder(ASourceFolder, AZipName, AWillcards: String): Boolean;
// Fonction permettant de dézipper un fichier zip
...
BEGIN
  TRY
    ...
    BEGIN
      ...
      //on ne vérifie le fichier que si on LOG
      //a la fin : si pas de fichier => pas d'envoi de mail
...
begin
  ...
  try
    Arch    := CreateOutArchive(CLSID_CFormatZip);
    Arch.AddFiles(ASourceFolder, '', AWillcards, False);
    Arch.SaveToFile(AZipName);
    Arch    := nil;
    Result  := True;
  Except on E:Exception do
    raise Exception.Create('ZipFolder -> ' + E.Message);
  end;
end;
