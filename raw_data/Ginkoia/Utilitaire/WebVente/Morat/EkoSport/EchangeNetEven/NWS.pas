...
TYPE
  ...
  PUBLIC
    ...
    CONSTRUCTOR Create(Owner: TComponent; AURL, ALogin, APassword: STRING); OVERLOAD;
    DESTRUCTOR Destroy; OVERRIDE;
...
FUNCTION CreerNWSServer(AURL, ALogin, APassword: STRING): TGestionNetEven;
...
CONSTRUCTOR TGestionNetEven.Create(Owner: TComponent; AURL, ALogin, APassword: STRING);
BEGIN
...
BEGIN
  ...
  TRY
    ...
    rCdes.DatePaymentFrom.AsDateTime := 0; // Une date ancienne
    rCdes.DatePaymentTo.AsDateTime := Now + 1; // Demain
  ...
  TRY
    ...
    TRY
      ...
      CASE aConnect.TestConnectionResult OF
        ...
        NonConformantAuthorization: Result := 'NCA';
        NonConformantFormat: Result := 'NCF';
        Rejected: Result := 'Reject';
        UnexpectedInformation: Result := 'Unexp';
        Error: Result := 'Error';
      ELSE Result := 'Else';
      END; // CASE
    EXCEPT
...
FUNCTION CreerNWSServer(AURL, ALogin, APassword: STRING): TGestionNetEven;
BEGIN
  Result := TGestionNetEven.Create(NIL, AURL, ALogin, APassword);
END;
