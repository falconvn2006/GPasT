...
TYPE
  TFrm_Main = CLASS(TForm)
    ...
    Lab_TransferTimeout: TRzLabel;
    Lab_ReadTimeout: TRzLabel;
    Chp_ConnectTimeout: TRzEdit;
    Chp_TransferTimeout: TRzEdit;
    Chp_ReadTimeout: TRzEdit;
...
BEGIN
  ...
  BEGIN
    ...
    iTransfer                 := MyIniFile.ReadInteger('TIMEOUT', 'Transfer', 3000);
    Chp_TransferTimeout.Text  := IntToStr(iTransfer);
    iRead                     := MyIniFile.ReadInteger('TIMEOUT', 'Read', 3000);
    Chp_ReadTimeout.Text      := IntToStr(iRead);
  END;
END;
...
BEGIN
  ...
  MyIniFile.WriteInteger('TIMEOUT', 'Transfer', StrToInt(Chp_TransferTimeout.Text));
  MyIniFile.WriteInteger('TIMEOUT', 'Read', StrToInt(Chp_ReadTimeout.Text));
...
BEGIN
  ...
  try
    ...
    on e : exception do
      LogAction('Exception dans "TFrm_Main.Tim_DoTraitementTimer" : ' + e.Message, 3);
  end;
...
BEGIN
  ...
  BEGIN
    ...
    WHILE NOT Que_GetSites.Eof DO
    BEGIN
      LogAction('********** ' + Que_GetSitesASS_NOM.AsString +
        ' **********', 3);
        ...
        BEGIN
          ...
          TRY
            if (Que_GetSites.RecNo <> Que_GetSites.RecordCount) then
              DoSend(Chp_TimerDeb.Time, Chp_TimerFin.Time, False)
            else
              DoSend(Chp_TimerDeb.Time, Chp_TimerFin.Time, True);
          EXCEPT
            ON E: Exception DO
            BEGIN
