...
uses
  ...
  RzCmboBx, RzBtnEdt, IFC_DM, DB, DBCtrls, RzDBCmbo, AdvDBLookupComboBox,
  ...
  cxDBLookupComboBox, IFC_Type;

type
  TMode = (mAdd, mEdit);

  Tfrm_CfgLignes = class(TForm)
    Pan_Btn: TRzPanel;
...
function ShowForm(AMode : TMode; ADOS_ID : Integer = 0) : Integer;
begin
  frm_CfgLignes := Tfrm_CfgLignes.Create(Nil);
  With frm_CfgLignes do
  try
    Mode := AMode;
    DOS_ID := ADOS_ID;
    dtp_Du.Date := Now;
    ...
    case Mode of
      mAdd: begin
        Caption  := 'Ajouter une base';
        DM_IFC.cds_LstMag.Close;
        DM_IFC.cds_LstMag.Open;
        DM_IFC.cds_LstMag.EmptyDataSet;
      end;
      mEdit: begin
        Caption := 'Modifier une base';
        With DM_IFC,Que_DOSSIERS do
        begin
          ...
          begin
            ...
            edt_IDMDC.Text    := FieldByName('DOS_IDMDC').AsString;
            edt_GrpBI.Text    := FieldByName('DOS_CODEGROUPEBI').AsString;
    ...
    frm_CfgLignes.Release;
    DM_IFC.iboDbCheck.Disconnect;
  end;
end;

procedure Tfrm_CfgLignes.AdvGlowButton1Click(Sender: TObject);
begin
  ConnectToDb(edt_BasePath.Text);
end;

function Tfrm_CfgLignes.CheckData: Boolean;
begin
  Result := False;

  if Trim(edt_IDMDC.Text) = '' then
  begin
    ShowMessage('Veuillez saisir un ID');
    edt_IDMDC.SetFocus;
    Exit;
  end;
...
function Tfrm_CfgLignes.ConnectToDb(sBasePath: String) : Boolean;
var
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FieldByName('Numero').AsString := Que_LstMag.FieldByName('MAG_CODEADH').AsString; //  GetGenParamInfo(12,10,Que_LstMag.FieldByName('MAG_ID').AsInteger);
          ...
          if Que_lstUsr.Locate('USR_ID',FieldByName('USR_ID').AsInteger,[]) then
            ...
            if Que_lstUsr.Locate('USR_USERNAME','Nosymag',[loCaseInsensitive]) then
            begin
              FieldByName('USR_ID').AsInteger := Que_lstUsr.FieldByName('USR_ID').AsInteger;
              FieldByName('USR_NOM').AsString := Que_lstUsr.FieldByName('USR_USERNAME').AsString;
            end;
          end;
...
procedure Tfrm_CfgLignes.cxGrid1DBTableView1USR_NOMPropertiesCloseUp(
  Sender: TObject);
begin
  With DM_IFC do
  begin
...
procedure Tfrm_CfgLignes.Ds_DomaineDataChange(Sender: TObject;
  Field: TField);
begin
  With DM_IFC do
  begin
...
procedure Tfrm_CfgLignes.edt_BasePathButtonClick(Sender: TObject);
begin
  With DM_IFC do
...
procedure Tfrm_CfgLignes.lku_MagCloseUp(Sender: TObject);
begin
  With DM_IFC do
  begin
...
procedure Tfrm_CfgLignes.Nbt_CancelClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure Tfrm_CfgLignes.Nbt_PostClick(Sender: TObject);
var
  PRMID_Groupe,
  PRMID_Num : Integer;
  sCode, sNum : String;
...
begin
        ...
        With DM_IFC do
          ...
          begin
            ...
            begin
              ...
              sNum := '';
              if Que_LstMag.Locate('MAG_ID',VarArrayOf([FieldByName('MAG_ID').AsInteger]),[loCaseInsensitive]) then
                sNum  := Que_LstMag.FieldByName('MAG_CODEADH').AsString; // GetGenParamInfo(12,10,FieldByName('MAG_ID').AsInteger);
 ...
 //               SetGenParamInfo(12,10,FieldByName('MAG_ID').AsInteger,FieldByName('Numero').AsString);
                  UpDateMagADH(FieldByName('MAG_ID').AsInteger, FieldByName('Numero').AsString);
              end;
        ...
        begin
            ...
            case Mode of
              mAdd: begin
                ...
                SQL.Add('DOS_MAILMODE, DOS_MAILSENDTO, DOS_IDMDC, DOS_CODEGROUPEBI)');
                ...
                SQL.Add(' :PDOSINIT, :PDOSDEBUT, :PDOSFIN, :PDOSLIVRAISON, :PDOSMAILMODE, :PDOSMAILSENDTO, :PDOSIDMDC,');
                SQL.Add(' :PDOSCODEGROUPEBI)');
                ParamCheck := True;
...
//                  ParamByName('PDOSACTIVE').AsInteger := 0;
              ...
              mEdit: begin
                ...
                SQL.Add(' DOS_IDMDC = :PDOSIDMDC, DOS_CODEGROUPEBI = :PDOSCODEGROUPEBI');
...
//            ParamByName('PDOSVILLE').AsString            := Que_LstMag.FieldByName('Ville').AsString;
            ...
            ParamByName('PDOSIDMDC').AsString := edt_IDMDC.Text;
            ParamByName('PDOSCODEGROUPEBI').AsString := edt_GrpBI.Text;
            ExecSQL;
