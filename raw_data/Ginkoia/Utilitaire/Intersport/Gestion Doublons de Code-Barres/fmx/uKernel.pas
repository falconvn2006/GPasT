...
type
  ...
  public type
    TProcessKind = (pkCreate, pkComplete);
    TShop = class(TObject)   
      ID: Integer;
      Name: string;
  ...
  public
    ...
    procedure Execute(const ProcessKind: TProcessKind);
...
procedure TBarCodeManager.Execute(const ProcessKind: TProcessKind);
begin
  case ProcessKind of
    pkCreate: ExecuteCreate;
    pkComplete: ExecuteComplete;
  end;
end;
              ...
              begin
                try
                  ...
                  FFDQuery.Open('select sum(stc_qte) "Quantity" ' +
                                ...
                                'group by cbi_cb ');
                  if FFDQuery.IsEmpty then
                    StreamWriter.WriteLine('%s;%s;%s;%s;-1', [
                      Items[0],
                      Items[1],
                      State,
                      &Type
  ...
  try
    ...
    try
      ...
      try
        ...
        if BarCodesQuantity.Count = 0 then {skip when there is no items}
          Exit;
        StringList := TStringList.Create;
        try
          StringList.StrictDelimiter := True;
