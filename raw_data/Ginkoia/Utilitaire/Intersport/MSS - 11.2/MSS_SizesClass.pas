...
//      IMP_REFSTR := {FPLXTAILLEGF.ClientDataSet.FieldByName('SRID').AsString + '/' +} FPLXTAILLEGF.FieldByName('Idx').AsString;
    ...
    Except on E:Exception do
      FErrLogs.Add('PLXTAILLESGF - ' + FPLXTAILLEGF.FieldByName('Idx').AsString + ' / ' + FPLXTAILLEGF.FieldByName('Denotation').AsString + ' : ' + E.Message);
    End;
...
begin
  Result := -1;
  Try
//    IMP_REFSTR := {FPLXTAILLEGF.ClientDataSet.FieldByName('SRID').AsString + '/' +} FPLXTAILLEGF.ClientDataSet.FieldByName('Idx').AsString;
