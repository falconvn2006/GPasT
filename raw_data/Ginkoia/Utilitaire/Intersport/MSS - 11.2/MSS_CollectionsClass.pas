...
begin
  ...
  begin
    try
      // Vérification que la colelction n'existe pas encore
      EXE_ID := 0;
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('Select COL_ID, COL_NOM, COL_ACTIVE, COL_DTDEB, COL_DTFIN from ARTCOLLECTION');
        ...
        if ADoMaj then
          ...
          begin
            ...
            if (IsActive <> FCds.FieldByName('Active').AsInteger) or
               ...
               (CompareDate(DateTo, FCds.FieldByName('DateTo').AsDateTime) <> EqualsValue)
            then
            begin
              Close;
              SQL.Clear;
...
begin
   // Est ce que la collection est dans GENIMPORT
      Result := IsInGenImport(CIMPNUM,FCds.FieldByName('Code').AsString);
      if (Result  = -1) then
      begin
         // Si non alors on vérifie qu'elle n'existe pas encore dans la table
         With FIboQuery do
         begin
           ...
           SQL.Add('  join K on K_Id = COL_ID and K_Enabled = 1');
           SQL.Add('Where Upper(COL_NOM) = :PCOLNOM');
           ParamCheck := True;
           ParamByName('PCOLNOM').AsString := UpperCase(FCds.FieldByName('Denotation').AsString);
           Open;
           ...
           begin
             ...
             Close;
             SQL.Clear;
             SQL.Add('Insert Into ARTCOLLECTION(COL_ID, COL_NOM, COL_NOVISIBLE)');
             SQL.Add('Values(:PCOLID,:PCOLNOM,:PCOLNOVISIBLE)');
             ParamCheck := True;
           ...
           SetGenImport(Result,CIMPNUM,FCds.FieldByName('Code').AsString);
         end;
      end
      else begin
        // Est ce que la collection doit être mise à jour
        ...
        begin
          ...
          if (RecordCount > 0) and (CompareText(FieldByName('COL_NOM').AsString,FCds.FieldByName('Denotation').AsString) <> 0) then
          begin
            Close;
            SQL.Clear;
            SQL.Add('Update ARTCOLLECTION set');
