...
uses DBClient, MidasLib, Db, Classes, ZipMstr19, SysUtils, xmldom, XMLIntf,
     msxmldom, XMLDoc, Variants, DateUtils, IBODataset, Forms, ComCtrls;
...
begin
  ...
  begin
    ...
    case PRM_INTEGER of
     -1: raise Exception.Create('Configuration incorrecte -> (' + IntToStr(APRM_TYPE) + '/' + IntToStr(APRM_CODE) + ') paramètre inéxistant');
      0: begin
        if bZeroInError then
          raise Exception.Create('Configuration incorrecte -> (' + IntToStr(APRM_TYPE) + '/' + IntToStr(APRM_CODE) + ') valeur 0 ');
      end;
    end;
  end;
