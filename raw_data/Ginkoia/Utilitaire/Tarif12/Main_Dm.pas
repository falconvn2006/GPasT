...
BEGIN
    ...
    BEGIN
        ...
        IF trim ( ch ) <> '' THEN str_base.strings.add ( ch ) ;
        FOR i := 0 TO 9 DO
        BEGIN
        ...
        FOR i := 0 TO str_Nombases.strings.count - 1 DO
        BEGIN
            ch := UpperCase ( iniCtrl.readString ( 'NOMBASES', 'ITEM' + intToStr ( i ) , '' ) ) ;
            IF trim ( ch ) <> '' THEN
                str_NomBases.Strings[i] := ch;
        END;

        def := UpperCase ( iniCtrl.readString ( 'NOMPOSTE', 'POSTE', '*' ) ) ;
        if def <> '*' Then str_Postes.strings.add(ch);
        FOR i := 0 TO str_Nombases.strings.count - 1 DO
        BEGIN
            ...
            Begin
                ...
                if Def = '*' Then def := ch;
            End
            Else str_Postes.strings.add (def)
        END;
        FOR i := 0 TO str_Postes.strings.count - 1 DO
              if str_Postes.strings[i] = '*' Then str_Postes.strings[i] := def;

        def := UpperCase ( iniCtrl.readString ( 'NOMMAGS', 'MAG', '*' ) ) ;
        if def <> '*' Then str_Mags.strings.add(def);
        FOR i := 0 TO str_Nombases.strings.count - 1 DO
        BEGIN
            ...
            Begin
                ...
                if Def = '*' Then def := ch;
            End
            Else  str_Mags.strings.add (def)
        END;
        FOR i := 0 TO str_Mags.strings.count - 1 DO
              if str_Mags.strings[i] = '*' Then str_Mags.strings[i] := def;


        UserName := iniCtrl.readString ( 'DATABASE', 'USERNAME', 'SYSDBA' ) ;
        PassWord := iniCtrl.readString ( 'DATABASE', 'PASSWORD', 'masterkey' ) ;
