...
type
  ...
  TJournaliser      = procedure(const AMessage: String; const ANiveau: TNiveau = NivDetail) of object;
...
begin
  try
    ...
    if HandleSCM = 0 then
      raise EServiceInfos.Create('Erreur lors de la connexion au gestionnaire de services.');
    ...
    // Si le service n'existe pas
    if HandleService = 0 then
      raise EServiceInfos.Create(Format('Le service "%s" n’existe pas.', [AService]));
    ...
    if ErrorCode <> ERROR_INSUFFICIENT_BUFFER then
      raise EServiceInfos.Create(Format('Erreur lors de la récupération de la configuration du service "%s". %s',
