...
procedure SinCos(const Theta: Single; var Sin, Cos: Single) register; overload;
procedure SinCos(const Theta: Double; var Sin, Cos: Double) register; overload;
procedure SinCos(const Theta: Extended; var Sin, Cos: Extended) register; overload;
...
function LnXP1(const X: Single): Single; overload;
function LnXP1(const X: Double): Double; overload; { Ln(X + 1), accurate for X near zero }
...
function IntPower(const Base: Single; const Exponent: Integer): Single register; overload;
function IntPower(const Base: Double; const Exponent: Integer): Double register; overload;
function IntPower(const Base: Extended; const Exponent: Integer): Extended register; overload;
...
{ Ldexp: returns X*2**P }
function Ldexp(const X: Single; const P: Integer): Single register; overload;
function Ldexp(const X: Double; const P: Integer): Double register; overload;
function Ldexp(const X: Extended; const P: Integer): Extended register; overload;
...
function Sum(const Data: array of Single): Single register; overload;
function Sum(const Data: array of Double): Double register; overload;
function Sum(const Data: array of Extended): Extended register; overload;
...
procedure SumsAndSquares(const Data: array of Single;
  var Sum, SumOfSquares: Extended) register; overload;
...
@@ValidRange:
  fstcw   word ptr [esp]
  fldcw   word ptr [esi].CW8087X    {Use GOT}
  lea     eax, [eax+eax*4]
...
begin
  Result := ((PExtendedRec(@AValue)^.Exponent and $7FFF)  = $7FFF) and
...
begin
  Result := ((PExtendedRec(@AValue)^.Exponent and $7FFF) = $7FFF) and
            ((PExtendedRec(@AValue)^.Mantissa and $FFFFFFFFFFFFFFFF) = $8000000000000000);
end;
...
begin
  if (PExtendedRec(@AValue)^.Mantissa and $FFFFFFFFFFFFFFFF) = $0000000000000000 then
    Result := ZeroValue
  else
    if (PExtendedRec(@AValue)^.Exponent and $8000) = $8000 then
      Result := NegativeValue
    else
      Result := PositiveValue;
end;
...
asm  // IN: EAX = ptr to Data, EDX = High(Data) = Count - 1
     ...
     // 5 clocks per loop, 4 items per loop = 1.2 clocks per item
       ...
       MOV      EBX, EDX    // count * sizeof(Extended) = count * 10 = count * 8 + count * 2
       SHL      EDX, 3
       SHL      EBX, 1
       ADD      EDX, EBX
