//$Log:
// 1    Utilitaires1.0         09/08/2013 09:53:54    Thierry Fleisch 
//$
//$NoKeywords$
//
...
TYPE
  ...
  PRIVATE
    // a Garder
    FFlagSavePrinterSetup_RapRV: Boolean;
    FPatRapport : String;
    FTOTWINNER, Forig, FMagasinID : Integer;
    ...
    Funi,  FMM, FSocieteID, FPosteID, FTvtID, FMtaid: Integer;
    ...
    FNM, FNP, FPatBase, FIniApp, FPatLect, FPatApp, FPatHelp, FPatEAI, FPatEAIexe : STRING;
    FPatErreurXML: string;    // repertoire de sauvegarde des fichiers XML reçu en erreur 
    FPseudoTO, FTOWEB: Integer;
  ...
  PUBLIC
    ...
    PROCEDURE DlgMemoRv(VAR Chp: TdxDbMemoRv; Titre: STRING; GotoNextOnOk: Boolean = True);
...
VAR
  ...
  CardSVSConfig : TcardSVS; // Record de configuration des cartes SVS
...
//  eSendexLib_TLB,
//  ActiveX
//  , GinkoiaEAICommon, ReferencementResStr,
//  PostFocusUnit;
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN //Rechereche sur toute la table des articles
      ...
      Qry.Close;
      Qry.SQL.Clear;
      Qry.SQL.Add('SELECT DISTINCT arl_ID,arl_nom,arl_chrono,arl_calid,cal_nom,' +
    ...
    Qry.Close;
    Qry.SQL.Clear;
    Qry.SQL.Add('SELECT DISTINCT ARL_ID,CBI_CLTID,ARL_NOM,arl_chrono,arl_calid,cal_nom,' +
      'tgf_nom,arl_virtuel,cal_reglage,cal_flagass,arl_arlid,cal_tcaid FROM LOCARTICLE');
  ...
  Qry.Close;
  Qry.SQL.Clear;
  Qry.SQL.Add('SELECT DISTINCT arl_ID,arl_nom,arl_chrono,cal_nom,' +
