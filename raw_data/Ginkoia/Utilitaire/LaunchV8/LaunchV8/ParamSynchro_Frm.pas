...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, AlgolStdFrm, ExtCtrls, Db, dxmdaset, IBCustomDataSet, IBQuery, DBCtrls,
  StdCtrls, RzLabel, RzPanelRv, RzPanel, LMDControl, LMDBaseControl, LMDBaseGraphicButton, LMDCustomSpeedButton, LMDSpeedButton, Buttons, uLog,
...
begin
  ...
  begin
    try
      ...
      Log.Log('ParamSynchro_Frm', 'ExecuteParamSynchro', 'Log', 'Que_ParamSynchro (avant).', logDebug, True, 0, ltLocal);
      Que_ParamSynchro.close;
      Que_ParamSynchro.open;
      Log.Log('ParamSynchro_Frm', 'ExecuteParamSynchro', 'Log', 'Que_ParamSynchro (après).', logDebug, True, 0, ltLocal);
      ...
      begin
        ...
        //la base actuelle
        Log.Log('ParamSynchro_Frm', 'ExecuteParamSynchro', 'Log', 'IBQue_Base (avant).', logDebug, True, 0, ltLocal);
        IBQue_Base.Open;
        Log.Log('ParamSynchro_Frm', 'ExecuteParamSynchro', 'Log', 'IBQue_Base (après).', logDebug, True, 0, ltLocal);

        //liste des bases
        Log.Log('ParamSynchro_Frm', 'ExecuteParamSynchro', 'Log', 'Que_Bases (avant).', logDebug, True, 0, ltLocal);
        Que_Bases.open;
        Log.Log('ParamSynchro_Frm', 'ExecuteParamSynchro', 'Log', 'Que_Bases (après).', logDebug, True, 0, ltLocal);
...
begin
  try
    ...
    begin
      ...
      IF NOT IBQue_UpdateParamNb.Transaction.InTransaction THEN
        IBQue_UpdateParamNb.Transaction.StartTransaction;
      try
        Log.Log('ParamSynchro_Frm', 'updateParametres', 'Log', 'IBQue_UpdateParamNb (avant).', logDebug, True, 0, ltLocal);
        ...
        Log.Log('ParamSynchro_Frm', 'updateParametres', 'Log', 'IBQue_UpdateParamNb (après).', logDebug, True, 0, ltLocal);
        Log.Log('ParamSynchro_Frm', 'updateParametres', 'Log', 'IBQue_UpdateK (avant).', logDebug, True, 0, ltLocal);

        //mettre à jour K_version pour param base portable
        IBQue_UpdateK.paramByName('id').value := prmid;
        IBQue_UpdateK.ExecSQL;

        Log.Log('ParamSynchro_Frm', 'updateParametres', 'Log', 'IBQue_UpdateK (après).', logDebug, True, 0, ltLocal);
        IBQue_UpdateParamNb.Transaction.commit;
      except
        Showmessage('Erreur, transaction annulée');
        IBQue_UpdateParamServeur.Transaction.Rollback;
      end;
      ...
      begin
        ...
        IF NOT IBQue_UpdateParamNb.Transaction.InTransaction THEN
          IBQue_UpdateParamNb.Transaction.StartTransaction;
        try
          Log.Log('ParamSynchro_Frm', 'updateParametres', 'Log', 'IBQue_UpdateParamServeur (avant).', logDebug, True, 0, ltLocal);
          ...
          Log.Log('ParamSynchro_Frm', 'updateParametres', 'Log', 'IBQue_UpdateParamServeur (après).', logDebug, True, 0, ltLocal);
          Log.Log('ParamSynchro_Frm', 'updateParametres', 'Log', 'Que_ParamServeur (avant).', logDebug, True, 0, ltLocal);
          ...
          Log.Log('ParamSynchro_Frm', 'updateParametres', 'Log', 'Que_ParamServeur (après).', logDebug, True, 0, ltLocal);
          Log.Log('ParamSynchro_Frm', 'updateParametres', 'Log', 'IBQue_UpdateK (avant).', logDebug, True, 0, ltLocal);
