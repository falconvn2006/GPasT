...
uses
  SysUtils, Classes, DB, IBCustomDataSet, IBQuery, IBDatabase, LaunchMain_DM, IBUpdateSQL, IBSQL, Dialogs, uLog;
...
type
  TDm_Param = class(TDataModule)
    ...
    IBQue_GenReplicationREP_EXEFINREPLIC: TIBStringField;
  ...
  public
    ...
    function GetREPEXEFINREPLIC : String;
    ...
    function SetREPEXEFINREPLIC(AFilename : string) : Boolean;
...
begin
  ...
  try
    ...
    IBQue_GenReplicationREP_ID.AsInteger := Dm_LaunchMain.MainNewK('GENREPLICATION');
    ...
    IBQue_GenReplicationREP_EXEFINREPLIC.AsString := '';
...
begin
  ...
  id := Dm_LaunchMain.MainNewK('GENPROVIDERS');
...
begin
  ...
  id := Dm_LaunchMain.MainNewK('GENSUBSCRIBERS');
...
begin
  ...
  IBQue_WebHStartPRM_ID.AsInteger       := Dm_LaunchMain.MainNewK('GENPARAM');
  IBQue_WebHStartPRM_INTEGER.AsInteger  := aBaseId;
  IBQue_WebHStart.Post;
  IBQue_WebHStart.Edit;

  IBQue_WebHEnd.Append;
  IBQue_WebHEndPRM_ID.AsInteger       := Dm_LaunchMain.MainNewK('GENPARAM');
  IBQue_WebHEndPRM_INTEGER.AsInteger  := aBaseId;
  IBQue_WebHEnd.Post;
  IBQue_WebHEnd.Edit;

  IBQue_WebIntervale.Append;
  IBQue_WebIntervalePRM_ID.AsInteger      := Dm_LaunchMain.MainNewK('GENPARAM');
...
begin
  try
    ...
    begin
      Dm_LaunchMain.MainDeleteK(IBQue_ProviderTGLR_ID.AsInteger);  //Suppression des lignes  GenLiairepli
      IBQue_ProviderT.Next;
    end;
    Dm_LaunchMain.MainDeleteK(id);    //Suppression enregistrement GenReplication
...
begin
  try
    ...
    begin
      ...
      Dm_LaunchMain.MainDeleteK(aProId);
...
begin
  try
    ...
    begin
      ...
      Dm_LaunchMain.MainDeleteK(aSubId);
...
begin
  try
    Dm_LaunchMain.MainDeleteK(IBQue_WebHStartPRM_ID.AsInteger);
    Dm_LaunchMain.MainDeleteK(IBQue_WebHEndPRM_ID.AsInteger);
    Dm_LaunchMain.MainDeleteK(IBQue_WebIntervalePRM_ID.AsInteger);
  except on e:Exception do
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          IBSql_Req.Close;
          IBSql_Req.ParamByName('GLR_ID').AsInteger := Dm_LaunchMain.MainNewK('GENLIAIREPLI');
    ...
    begin
      ...
      begin
        Dm_LaunchMain.MainDeleteK(IBQue_ProviderTGLR_ID.AsInteger);
        IBQue_ProviderT.Next;
      end;
    end;
  end
  ...
  begin
    ...
    begin
      Dm_LaunchMain.MainDeleteK(aGlrId);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Dm_LaunchMain.MainDeleteK(IBQue_SubscriptionTGLR_ID.AsInteger);
...
function TDm_Param.GetREPEXEFINREPLIC: String;
begin
  Result := IBQue_GenReplicationREP_EXEFINREPLIC.AsString;
end;
...
begin
  ...
  IBSql_Req.SQL.Add('INSERT INTO GENREPLICATION (REP_ID, REP_LAUID, REP_PING, '+
                    ...
                    'REP_PLACEBASE, REP_JOUR, REP_EXEFINREPLIC) VALUES (:REP_ID, :REP_LAUID, '+
                    ':REP_PING, :REP_PUSH, :REP_PULL, :REP_USER, :REP_PWD, '+
                    ':REP_ORDRE, :REP_URLLOCAL, :REP_URLDISTANT, :REP_PLACEEAI, '+
                    ':REP_PLACEBASE, :REP_JOUR, :REP_EXEFINREPLIC)');
  ...
  IBSql_Req.ParamByName('REP_EXEFINREPLIC').AsString := IBQue_GenReplicationREP_EXEFINREPLIC.AsString;
  IBSql_Req.ExecQuery;
end;
...
function TDm_Param.SetREPEXEFINREPLIC(AFilename: string): Boolean;
begin
  Result := False;
  try
    IBQue_GenReplicationREP_EXEFINREPLIC.AsString := AFilename;
    Result := True;
  except on e:Exception do
    begin
      ShowMessage('Erreur lors de l''affectation de l''exécutable. (' + e.Message + ')');
      Result := False;
    end;
  end;
...
begin
  ...
  try
    ...
    begin
      ShowMessage('Erreur lors de l''affectation du chemin de la base. (' + e.Message + ')');
      Result := False;
    end;
  end;
end;
