...
type TSock = class; // Forward Declared For Event Types
   ...
   private
      ...
      FLocalPortName: string; // Local Port Name Or Well-Known Number, Defaults To 1 (=FPortName) For Backward Compatibility
      ...
      FFreeOnClose: Boolean; // Free after closure of socket? (Non-blocking, auto-accepted sockets!)
   ...
   public
      ...
      function SendBuf(var Value; Size: Integer): Boolean;
      function ReceiveBuf(var Value; Count: Integer): integer;
      ...
      function SendDatagramBuf(var Value; Size: Integer; HostName: string): Boolean;
      function ReceiveDatagram(var HostName: string): string;
      function ReceiveDatagramBuf(var HostName: string; var Size: Integer): Pointer;
...
begin
   ...
   begin
      ...
      if FLocalPortName = '-1' then
         LAddrIn.SIn_Port := PortLookup(FPortName) // Default behaviour for backward compatibility
      else
         LAddrIn.SIn_Port := PortLookup(FLocalPortName);
      LAddrIn.SIn_Addr.S_Addr := HToNL(INADDR_ANY); // No HostLookup(...) Because INADDR_ANY Is A Windows Constant
      ...
      begin
         ...
         Res := WinSock.Bind(FSocket, LAddrIn, SizeOf(LAddrIn)) + WinSock.Connect(FSocket, FSockAddrIn, SizeOf(TSockAddrIn));
         if FBlocking then
         begin
            if Res = 0 then
            begin
...
function TSock.SendBuf(var Value; Size: Integer): Boolean;
var Remain: Integer;
   Pt: pointer;
begin
   Result := True;
   ...
   begin
      ...
      begin
         ...
         while Remain > 0 do
         begin
            Remain := WinSock.Send(FSocket, pt^, size, 0);
            ...
            begin
               ...
               if Remain > 0 then
               begin
                  integer(pt) := integer(pt) + (size - Remain);
                  size := Remain;
               end;
         ...
         raise ESockException.Create('Que en blocking');
   end
   else
      SendDatagramBuf(Value, size, FHostName);
end;
...
function TSock.ReceiveBuf(var Value; Count: Integer): integer;
var Res: Integer;
   FDSet: PFDSet;
   TV: PTimeVal;
   Err: Integer;
...
begin
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            begin
               ...
               if (not (Err - WSABASEERR in [WSAEINTR - WSABASEERR, WSAEINPROGRESS - WSABASEERR, WSAEOPNOTSUPP - WSABASEERR, WSAEWOULDBLOCK - WSABASEERR, WSAEMSGSIZE - WSABASEERR])) then
               begin
         ...
         if (Length(FInBuffer) > Count) then
         begin
            Move(pointer(FInBuffer)^, value, count);
            Delete(FInBuffer, 1, Count);
            result := count;
         end
         ELSE if (Length(FInBuffer) = Count) then
         begin
            Move(pointer(FInBuffer)^, value, count);
            FInBuffer:= '';
            result := count;
      ...
      raise ESockException.Create('Pas de datagram');
end;
...
begin
   ...
   begin
      ...
      begin
         ...
         raise ESockException.Create('Socket Error while Receiving Datagram:' + IntToStr(WSAGetLastError));
   end;
   Dispose(FDSet);
   Dispose(TV);
end;
...
begin
   ...
   if WinSock.GetHostname(CharHostname, SizeOf(CharHostname)) = 0 then
      ...
      raise ESockException.Create('GetLocalHostname - Could Not Retrieve Hostname');
end;
...
procedure Register;
begin
   RegisterComponents('iPRO', [TSock]);
end;

function TSock.ReceiveDatagramBuf(var HostName: string; var Size: Integer): Pointer;
var Res: Integer;
   FDSet: PFDSet;
   TV: PTimeVal;
   FLen: Integer;
...
function TSock.SendDatagramBuf(var Value; Size: Integer;
...
begin
   ...
   SetHostName(HostName);
   if Size <> 0 then
      WinSock.SendTo(FSocket, Value, size, 0, FSockAddrIn, SizeOf(TSockAddrIn));
end;
