unit uRepriseCCIllicadoDBUtils;
...
type
  ...
  TRepriseCCIllicadoDBUtils = class
  private
  ...
  public
    ...
    function ModuleCCIllicadoEnabled(AMagId: Integer): boolean;
    function ListMagasins(AWithCCIllicadoEnabled: boolean = False): TMagasinArray;
...
var
  FRepriseCCIllicadoDBUtils: TRepriseCCIllicadoDBUtils;
...
{ TRepriseCCIllicadoDBUtils }

function TRepriseCCIllicadoDBUtils.getConnection: TMyConnection;
begin
...
function TRepriseCCIllicadoDBUtils.getTransaction: TMyTransaction;
begin
...
function TRepriseCCIllicadoDBUtils.getURL(AMagId: integer): string;
begin
  result := getGenParamValueString(13, 115, AMagId);
  if RightStr(result,1)<>'/' then
    result := result + '/';
end;

function TRepriseCCIllicadoDBUtils.getGUID(AMagId: integer): string;
begin
  result := getGenParamValueString(13, 125, AMagId);
end;


function TRepriseCCIllicadoDBUtils.ListMagasins(AWithCCIllicadoEnabled: boolean = False): TMagasinArray;
var
  query: TMyQuery;
begin
  ...
  try
    ...
    begin
      if not AWithCCIllicadoEnabled or (AWithCCIllicadoEnabled
        and ModuleCCIllicadoEnabled(query.FieldByName('MAG_ID').AsInteger)) then
      begin
...
function TRepriseCCIllicadoDBUtils.ModuleCCIllicadoEnabled(
...
begin
  ...
  query.ParamByName('MODULENAME').AsString := 'CARTECADEAU_GLOBALPOS_ILLICADO';
...
procedure TRepriseCCIllicadoDBUtils.UpdateCKDO(ACKDOID: integer; aWSValue: Integer);
var
  query: TMyQuery;
begin
  try
...
procedure TRepriseCCIllicadoDBUtils.UpdateCKDOWithoutTicket();
var
...
begin
  ...
  try
    ...
    query := GetNewQuery(getConnection, getTransaction,
      ...
      'AND KDO_TKEID <> 0 AND KDO_CARDTYPE = 4');
    query.Open;
...
procedure TRepriseCCIllicadoDBUtils.closeConnection;
begin
...
constructor TRepriseCCIllicadoDBUtils.Create(ADataBaseFile: string);
begin
  inherited Create;
...
procedure TRepriseCCIllicadoDBUtils.AddLog(ALogMessage: string;  ALogLevel: TLogLevel);
var
  tmplog: TLogItem;
begin
...
procedure TRepriseCCIllicadoDBUtils.CreateCKDO(ACKDORIGIN: TDataSet; AcodeRetour: Integer);
var
  query: TMyQuery;
  NewID: Integer;
  sql: String;
...
procedure TRepriseCCIllicadoDBUtils.SetOnLog(const Value: TLogEvent);
begin
  FOnLog := Value;
end;

function TRepriseCCIllicadoDBUtils.getGenBaseBasCodeTiersValue(AMagId: integer): string;
var
  query: TMyQuery;
  vVersion : string;
begin
...
function TRepriseCCIllicadoDBUtils.getGenParamValue(AType, ACode, AMagId: integer;
...
function TRepriseCCIllicadoDBUtils.getGenParamValueInteger(AType, ACode, AMagId: integer): integer;
begin
  Result := getGenParamValue(AType, ACode, AMagId, gpvtInteger);
end;

function TRepriseCCIllicadoDBUtils.getGenParamValueString(AType, ACode, AMagId: integer): string;
begin
  Result := getGenParamValue(AType, ACode, AMagId, gpvtString);
end;

function TRepriseCCIllicadoDBUtils.getDisconnectCKDOQuery(AMAGId: integer; ListForMails: Boolean = False): TDataSet;
var
  sql: string;
  query: TMyQuery;
begin
...
function TRepriseCCIllicadoDBUtils.getCKDOKO(): TDataSet;
var
  sql: string;
  query: TMyQuery;
begin
...
function TRepriseCCIllicadoDBUtils.NewK(ATableName: string): integer;
var
...
procedure TRepriseCCIllicadoDBUtils.UpdateK(AId: integer; AType: integer);
var
  storedproc: TMyStoredProc;
begin
