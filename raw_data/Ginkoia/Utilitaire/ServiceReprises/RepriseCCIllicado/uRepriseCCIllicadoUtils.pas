unit uRepriseCCIllicadoUtils;

interface

uses
  Data.DB,
  uRepriseCCIllicadoDBUtils, WSCartesCadeauxGlobalPOS, ulog, System.Classes,
  IdSMTP, IdExplicitTLSClientServerBase, IdSSLOpenSSL, IdMessage, uRepriseCCIllicadoUtils_RS;

type
  TRCCCanContinueEvent = procedure(Sender: TObject; var ACanContinue: Boolean) of object;

  TRepriseCCIllicadoUtils = class
  private
...
{ TRepriseCCIllicadoUtils }

procedure TRepriseCCIllicadoUtils.AddLog(ALogMessage: string; ALogLevel: TLogLevel; AMagId: Integer);
var
  tmplog: TLogItem;
begin
...
procedure TRepriseCCIllicadoUtils.CancelCKD(ACKDO: TDataSet; AMagId: integer);
var
...
begin
  ...
  ACardDetail := FwsGlobalPOS.SetOperationCarteCadeau(GPAnnulation);
   ...
   begin
     ...
     AddLog('Le serveur Illicado ne répond pas', logWarning);
   end
   else
...
procedure TRepriseCCIllicadoUtils.SendMail(aMessage: string; AMagId: Integer);
var
  vEmails: string;
  SMTP: TIdSMTP;
...
begin
  ...
  vEmails := Trim(FRCCDBUtils.getGenParamValueString(13,126, AMagId));

  if vEmails = '' then
  begin
    // on sort et on log
    AddLog('Pas d''email renseigné pour l''envoi des erreurs de reprise Illicado (GENPARAM 13/126)', logError);
    Exit
  end;
...
procedure TRepriseCCIllicadoUtils.MailFailedCKDO(ACKDO: TDataSet; AMagId: integer);
var
  vMessage: String;
begin
  AddLog(Format('Envoi du mail pour le KDO_ID : %d (CodeBarre : %s)', [ACKDO.FieldByName('KDO_ID').AsInteger, ACKDO.FieldByName('KDO_NUMCARTE').AsString]), logInfo);
...
procedure TRepriseCCIllicadoUtils.MailCKDOKO(ACKDO: TDataSet; AMagId: integer);
var
  vMessage: String;
  MAGID: Integer;
begin
...
function TRepriseCCIllicadoUtils.CanContinue: Boolean;
begin
  Result := True;
  ...
  if not Result then
    AddLog('La reprise de cartes cadeaux Illicado à reçu l''ordre de s''arreter', logWarning);
end;

procedure TRepriseCCIllicadoUtils.closeConnection;
begin
  FRCCDBUtils.closeConnection;
end;

constructor TRepriseCCIllicadoUtils.Create(ADataBaseFile: string);
begin
...
destructor TRepriseCCIllicadoUtils.Destroy;
begin
...
procedure TRepriseCCIllicadoUtils.RecoverBaseCKDOs;
var
  i: integer;
  count: integer;
...
begin
  ...
  try
    // on traite d'abord toutes les cartes cadeaux Illicado qui ont un tke_id qui ne correspond à rien (par ex plantage de caisse)
...
procedure TRepriseCCIllicadoUtils.RecoverCKDO(ACKDO: TDataSet; ABasCodeTiers: string;
  AMagId: integer);
var
  vType: String;
begin
...
procedure TRepriseCCIllicadoUtils.RecoverMagCKDOs(AMagId: integer);
var
  i: integer;
...
begin
  ...
  if not FRCCDBUtils.ModuleCCIllicadoEnabled(AMagId) then
  begin
    AddLog('Le module "CARTECADEAU_GLOBALPOS_ILLICADO" n''est pas activé');
  end
  else
  begin
...
procedure TRepriseCCIllicadoUtils.SetOnLog(const Value: TLogEvent);
begin
  FOnLog := Value;
