...
begin
  ...
  begin
    ...
    //N'affiche pas la fenêtre de progression si on pointe sur une autre fenêtre
    if @Callback <> nil then
      ShowProgress := FALSE;

    try
      //Indique que l'on compresse
      s7cmd := 'a';

      //Option de compression au format zip
      if TypeZip then
        s7cmd := s7cmd + ' -tzip';

      //Ajoute le nom du fichier de sortie à la command 7Zip
      s7cmd := s7cmd + ' "' + BaseDirectory + ArchiveFilename + '" "' + RemoveQuotes( flist[ 0 ] ) + '"';

      //Ajoute la liste des fichiers à compresser
      ...
      //Indique le taux de compression
      s7cmd := s7cmd + ' -mx' + IntToStr( CompressionLevel );
      if RecurseFolders then
        s7Cmd := s7cmd + ' -r';

      //Ajout le mot de passe dans la ligne de commande 7Zip
      if Password <> '' then
        s7Cmd := s7Cmd + ' -p' + Password;

      if Not CreateSolidArchive then
        s7cmd := s7cmd + ' -ms=off';

      //Ajout l'affiche de la fenêtre de progression dans la ligne de commande 7Zip
