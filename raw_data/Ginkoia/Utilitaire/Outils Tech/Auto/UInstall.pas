...
// modifié par RD
 ...
 Tlogfile.Addline(Datetimetostr(Now)+' création des raccourcis');  
 if Serveur <> '' then
  begin
  machine.CreerRaccourcie('Ginkoia', '\\' + Serveur + '\GINKOIA\BATCH\GINKOIA.BAT', 'Programme Ginkoia', FDirGinkoia, FDirGinkoia + 'GINKOIA.EXE');
  machine.CreerRaccourcie('Caisse', '\\' + Serveur + '\GINKOIA\BATCH\CAISSE.BAT', 'Programme caisse de Ginkoia', FDirGinkoia, FDirGinkoia + 'CAISSEGINKOIA.EXE');
  end else
  begin
   machine.CreerRaccourcie('Ginkoia', IncludeTrailingBackslash(FdirGinkoia)+'BATCH\GINKOIA.BAT', 'Programme Ginkoia', FDirGinkoia, FDirGinkoia + 'GINKOIA.EXE');
   machine.CreerRaccourcie('Caisse',  IncludeTrailingBackslash(FdirGinkoia)+'BATCH\CAISSE.BAT', 'Programme caisse de Ginkoia', FDirGinkoia, FDirGinkoia + 'CAISSEGINKOIA.EXE');
  end;
...
begin
  ...
  Tlogfile.Addline (Datetimetostr(Now)+' Création des fichiers ini');
  ForceDirectories(DirGinkoia) ; // modifié par RD
  Tsl := TstringList.create;
  // transformation du chemin de la base                          
//  s := serveur + ':' + FDirBase + 'TEST.IB'; //  disk + ':\GINKOIA\DATA\
   if serveur <> '' then
   begin
    s := serveur+':'+IncludeTrailingBackslash(Dirginkoia)+'Data\TEST.IB' ; // modifié par RD
   end else
   begin
     s :=DirGinkoia+'Data\TEST.IB'
   end;

  //Base := serveur + ':' + FDirBase + 'GINKOIA.IB'; // disk + ':\GINKOIA\DATA\GINKOIA.IB'
  if Serveur <> '' then
  begin
   Base := serveur+':'+IncludeTrailingBackslash(DirGinkoia)+'Data\GINKOIA.IB' ;
  end else
  begin
   Base := DirGinkoia+'Data\GINKOIA.IB' ;
  end;
  ...
  Tlogfile.Addline(Datetimetostr(Now)+' ouverture du Query');
  qry.Open;
  ...
  qry.close;
  Tlogfile.Addline(Datetimetostr(Now)+' fermeture du Query');  
  Ferme;
  TSl.Add('ITEM1=Base TEST');
  TSl.SaveToFile(FDirGinkoia +  'GINKOIA.INI');
  Tlogfile.Addline(Datetimetostr(Now)+' Ginkoia.ini sauvegardé');
  TSl.SaveToFile(FDirGinkoia + 'Caisseginkoia.ini');
  Tlogfile.Addline(Datetimetostr(Now)+' Caisseginkoia.ini sauvegardé');  
  tsl.free;
end;
...
var
  ...
  dwResult: Cardinal;
begin
  Tlogfile.Addline (Datetimetostr(Now)+' Mise à jour environnement');
  ...
  Except
    ON E: Exception do
       Tlogfile.Addline (Datetimetostr(Now)+' Erreur  '+e.message) ;
  End;
end;
