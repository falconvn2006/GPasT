...
type
  ...
  TJSON = class
  private
    class function BoolToJSON(aBool: Boolean): string; static;
  public
    ...
    class function ObjectToJSON(aObject: TObject ; aDefaultClass : TClass; aJSONOptions : TJSONOptions): string; overload;
    class function ObjectToJSON(aObject : TPersistent) : string ; overload;
                    ...
                    begin
                      case currentProp^.PropType^.Kind of
                        tkClass : begin
                                    ...
                                    oObj := TPersistent(GetObjectProp(aObject, currentProp, TPersistent)) ;
                                    if not Assigned(oObj)
                                      then oObj := TPersistent(cClass.NewInstance) ;
...
//------------------------------------------------------------------------------
class function TJSON.ObjectToJSON(aObject: TObject ; aDefaultClass : TClass; aJSONOptions : TJSONOptions): string;
begin
  Result := ObjectToJSON(TPersistent(aObject));
end;
        ...
        case PropInfo^.PropType^.Kind of
          ...
          tkFloat :
            if PropInfo^.PropType^.Name = 'TDateTime' then
              ...
              Result := Result + StringReplace(FloatToStr(GetFloatProp(aObject, PropInfo)), FormatSettings.DecimalSeparator, '.', []);
