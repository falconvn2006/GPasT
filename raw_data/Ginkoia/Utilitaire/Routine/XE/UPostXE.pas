...
begin
  ...
  begin
    try
      ...
      tpd.add('action', 'QRY');
      tpd.add('qry1', qry);
      if tpd.PostWaitTimeOut(clt, 'http://' + URL_SERVEUR_YELLIS + '/admin/base.php', 30000, Body) then
      begin
        S := BODY.DataString;
...
begin
  ...
  begin
    try
      ...
      tpd.add('action', 'SEL');
      tpd.add('sel', qry);
      if tpd.PostWaitTimeOut(clt, 'http://' + URL_SERVEUR_YELLIS + '/admin/base.php', 30000, Body, Memo) then
      begin
        S := BODY.DataString;
        if pos('<HTML>', Uppercase(S)) > 0 then
        begin
