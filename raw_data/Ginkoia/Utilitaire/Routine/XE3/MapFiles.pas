...
type
  ...
  TMapStream = class(TStream)
   private
     ...
     procedure SetSize(NewSize: cardinal); {$IFDEF VER130} reintroduce; {$ENDIF}
   protected
     {$IFOPT D+}
...
begin
  if (fMutexHandle = 0) then
      {$IF CompilerVersion >= 16.0}
      fMutexHandle := Winapi.Windows.CreateMutex(nil, False, PChar(fMutexName));
      {$ELSE}
...
begin
  {$IF CompilerVersion >= 16.0}
  WriteStr(System.SysUtils.Format(Fmt, Args));
  {$ELSE}
...
begin
  {$IF CompilerVersion >= 16.0}
  WriteLine(System.SysUtils.Format(Fmt, Args));
  {$ELSE}
