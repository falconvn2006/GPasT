...
uses
  ...
  ShellApi, TlHelp32, IdSMTP, IdMessage, Registry,{IdAttachment, IdAttachmentFile,}
  DBClient, Controls, Mask, ExtCtrls, TypInfo, midasLib, Variants,
  IdText, IdFTP, IdFTPCommon, IdExplicitTLSClientServerBase, IdAllFTPListParsers,
  IdBaseComponent, IdComponent, IdRawBase, IdRawClient, IdIcmpClient,

  cxGrid, cxGridExportLink, cxGridDBBandedTableView, cxGridBandedTableView,
  cxGridDBTableView, cxGridTableView, cxStyles;
...
function GetGmtFromSystemTime: TDateTime;
...
function VarToDateTimeDef(Const V: Variant; Const ADefault: TDateTime = 0): TDateTime;
function StrToFloatExt(Const S: String): Extended;
...
{ Encryptage }
function EncryptDecryptChaine(Const S: String): String;
...
{ DevExpress }
function AddBand(Const ADBBandedTableView: TcxGridDBBandedTableView; Const ACaption: string;
                 Const AWidth, AIndex: integer; Const IsUnique: Boolean = True): TCxGridBand;
function AddCol(Const ADBBandedTableView: TcxGridDBBandedTableView;
                Const AFieldName, ACaption: string; Const AWidth: integer = 0): TcxGridDBBandedColumn; overload;
function AddCol(Const ADBTableView: TcxGridDBTableView;
                Const AFieldName, ACaption: string; Const AWidth: integer = 0): TcxGridDBColumn; overload;
...
{===============================================================================
 Fonction     : GetGmtFromSystemTime
 Description  : permet de calculer l'heure GMT à partir des informations systeme.
===============================================================================}
function GetGmtFromSystemTime: TDateTime;
var
  vRegistry: TRegistry;
const
  cRootK = HKEY_LOCAL_MACHINE;
  cKName = 'SYSTEM\CurrentControlSet\Control\TimeZoneInformation';
begin
  vRegistry:= TRegistry.Create(KEY_READ);
  try
    vRegistry.RootKey:= cRootK;
    if vRegistry.OpenKey(cKName, False) then
      Result:= ((Now * 96) + vRegistry.ReadInteger('ActiveTimeBias') div 15) / 96;
  finally
    FreeAndNil(vRegistry);
  end;
end;
...
begin
  ...
  if Separe then
    begin
      Idx:= Pos(FormatSettings.DecimalSeparator, Result);
      if Idx <> 0 then
        begin
...
{===============================================================================
 ...
 Description  : permet la conversion d'un String en DateTime en vérifiant si la
                ...
                - Si "S" est vide le result renvoie le "Default".
===============================================================================}
...
{===============================================================================
 Fonction     : Entrelacement
 Description  : permet d'entrelacer deux chaines pour des besoins de cryptage.
===============================================================================}
function Entrelacement(Const S1, S2: String): String;
var
  i, vPos: integer;
...
{===============================================================================
 Fonction     : VarToDateTimeDef
 Description  : permet la conversion d'un variant en TDateTime et si le paramètre
                V est null alors il prendra la valeur "ADefault".
===============================================================================}
function VarToDateTimeDef(Const V: Variant; Const ADefault: TDateTime = 0): TDateTime;
begin
  Result:= ADefault;
...
{===============================================================================
 Fonction     : StrToFloatExt
 Description  : permet la conversion d'un string en floatant en controlant la
                cohérence du "DecimalSeparator" et de le remplacer si besoin est.
                Si "S" est vide la fonction retourne 0.
===============================================================================}
function StrToFloatExt(Const S: String): Extended;
var
  i: integer;
  Buffer, vOldDecimalSep: String;
...
begin
  ...
  vSysCharSet:= ['0'..'9', '-'];
  Buffer:= S;
  if Pos(FormatSettings.DecimalSeparator, Buffer) = 0 then
    begin
      for i:= 1 to Length(Buffer) do
        begin
          if not (CharInSet(Buffer[i], vSysCharSet))  then
            begin
              vOldDecimalSep:= Buffer[i];
              Break;
            end;
        end;
      Buffer:= StringReplace(Buffer, vOldDecimalSep, FormatSettings.DecimalSeparator, []);
    end;
...
var
  vSysCharSet: TSysCharSet;
begin
  vSysCharSet:= ['0'..'9', '-', #27, #13, #8, FormatSettings.DecimalSeparator];
  if not AllowValueNeg then
    vSysCharSet:= vSysCharSet - ['-'];
  if not (CharInSet(Key, vSysCharSet)) then
    Key:= #0;
end;
...
{=============================================================================
 Function     : AddBand
 Description  : Permet d'ajouter une band dans dans une cxGrid.
==============================================================================}
function AddBand(Const ADBBandedTableView: TcxGridDBBandedTableView; Const ACaption: string;
  Const AWidth, AIndex: integer; Const IsUnique: Boolean = True): TCxGridBand;
var
...
begin
  ...
  if IsUnique then
    begin
      for i:= 0 to ADBBandedTableView.Bands.Count - 1 do
        begin
          if ADBBandedTableView.Bands.Items[i].Caption = ACaption then
            begin
              vExist:= True;
              Result:= ADBBandedTableView.Bands.Items[i];
              Break;
            end;
        end;
    end;
  if not vExist then
    begin
      Result                   := ADBBandedTableView.Bands.Add;
      Result.Caption           := ACaption;
      Result.Index             := AIndex;
      Result.Width             := AWidth;
      Result.Options.Moving    := False;
      Result.Options.Sizing    := True;
    end;
end;

{=============================================================================
 Function     : AddCol
 Description  : Permet d'ajouter une colonne dans dans une cxGrid.
                Deux implémentations : une avec une bande et l'autre sans
==============================================================================}
function AddCol(Const ADBTableView: TcxGridDBTableView;
  Const AFieldName, ACaption: string; Const AWidth: integer = 0): TcxGridDBColumn; overload;
var
...
begin
  Result:= nil;
  vExist:= False;
  for i:= 0 to ADBTableView.ColumnCount - 1 do
    begin
      if ADBTableView.Columns[i].DataBinding.FieldName = AFieldName then
        begin
          vExist:= True;
          Break;
        end;
  ...
  if not vExist then
    begin
      Result                       := ADBTableView.CreateColumn;
      Result.DataBinding.FieldName := AFieldName;
      Result.Caption               := ACaption;
      Result.Editing               := False;
      Result.Options.HorzSizing    := True;
      Result.Options.Moving        := False;
...
begin
  Result:= nil;
  vExist:= False;
  for i:= 0 to ADBBandedTableView.ColumnCount - 1 do
    begin
      if ADBBandedTableView.Columns[i].DataBinding.FieldName = AFieldName then
        begin
          vExist:= True;
          Break;
        end;
  ...
  if not vExist then
    begin
      Result                       := ADBBandedTableView.CreateColumn;
      Result.DataBinding.FieldName := AFieldName;
      Result.Caption               := ACaption;
      Result.Editing               := False;
      Result.Options.HorzSizing    := True;
...
begin
  ...
  for i:= 1 to Length(S) do
    begin
      if not (CharInSet(S[i], vSysCharSet)) then
        begin
          Result:= False;
          Break;
        end;
...
{===============================================================================
 Procedure    : EncryptDecryptChaine
 Description  : permet le cryptage et le decryptage d'une chaine.
===============================================================================}
function EncryptDecryptChaine(Const S: String): String;
var
  i: integer;
...
begin
  Buffer:= '';
  for i:= 1 to Length(S) do
    Buffer:= Buffer + Char(Byte(S[i]) xor 128);
  Result:= Buffer;
end;
