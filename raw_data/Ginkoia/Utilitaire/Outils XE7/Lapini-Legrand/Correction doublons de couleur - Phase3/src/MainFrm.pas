...
type
  Tfrm_Main = class(TForm)
    ...
    dtp_DateLimite: TDateTimePicker;
    lbl_DateLimite: TLabel;
...
begin
  ...
  begin
    ...
    begin
      ...
      case IndexText(Param, ['AUTO', 'BASE', 'FILE', 'MAGID', 'DATEMAX']) of
        0 : FAuto := true;
        ...
        4 : dtp_DateLimite.Date := StrToDateDef(Copy(ParamStr(i), Pos('=', ParamStr(i)) +1, Length(ParamStr(i))), dtp_DateLimite.Date);
      end;
    end;
...
function Tfrm_Main.ChangeCouleurMouvement(FileBDD : string; ListeArticle : TListArticles; ListeMagasins : TListMagasins) : TResTraitement;
  ...
  procedure UpdateCouleurTable(Query : TFDQuery; IdMag, DateMvt, TableEntete, IdEnete, TableLigne, IdLien, ArtLigne, CouLigne : string; ListeMagasins : string; idart, idoldcou, idnewcou : integer; DateMax : TDate);
  begin
    ...
    begin
      Query.SQL.Text := 'update ' + TableLigne + ' '
                      ...
                      + '  and ' + DateMvt + ' <= ' + QuotedStr(FormatDateTime('yyyy-mm-dd', DateMax)) + ' '
...
//            UpdateCouleurTable(Query, 'IMA_MAGOID', 'TRANSFERTIM', 'IMA_ID', 'TRANSFERTIML', 'IML_IMAID', 'IML_ARTID', 'IML_COUID', StringMagasins, IdArt, IdOldCou, IdNewCou);
            // Tables d'agregats ...
            UpdateCouleurTable(Query, 'MVT_MAGID', 'MVT_DATE', '', '', 'AGRMOUVEMENT', '', 'MVT_ARTID', 'MVT_COUID', StringMagasins, IdArt, IdOldCou, IdNewCou, dtp_DateLimite.Date);
            // Triggers differe !
...
//            UpdateCouleurTable(Query, 'IMA_MAGOID', 'TRANSFERTIM', 'IMA_ID', 'TRANSFERTIML', 'IML_IMAID', 'IML_ARTID', 'IML_COUID', StringMagasins, IdArt, IdCouTampon, IdNewCou);
            // Tables d'agregats ...
            UpdateCouleurTable(Query, 'MVT_MAGID', 'MVT_DATE', '', '', 'AGRMOUVEMENT', '', 'MVT_ARTID', 'MVT_COUID', StringMagasins, IdArt, IdCouTampon, IdNewCou, dtp_DateLimite.Date);
            // Triggers differe !
...
begin
  ...
  try
    ...
    lbl_DateLimite.Enabled := Enabled;
    dtp_DateLimite.Enabled := Enabled;
