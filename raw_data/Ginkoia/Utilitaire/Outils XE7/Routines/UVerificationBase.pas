...
type
  ...
  public
    ...
    function FillFromDatabase(Server, DataBase, Login, Password : string; Port : integer) : boolean;
...
function TDataBaseVerif.FillFromDatabase(Server, DataBase, Login, Password : string; Port : integer) : boolean;

  function GetTypeFromDB(Query : TFDQuery) : string;
  begin
    case Query.FieldByName('field_type').AsInteger of
      ...
      35 : result := 'timestamp';
      37 : result := 'varchar(' + Query.FieldByName('character_length').AsString + ')';
      40 : result := 'cstring';
      261 : result := 'blob';
      else result := 'unknown';
    end;
...
begin
  ...
  try
    ...
    try
      ...
      // recup du viveau des generateur
      ...
      try
        ...
        try
          ...
          begin
            ...
            tmpProcedure.Md5Source := MD5.HashStringAsHex(Trim(QueryList.FieldByName('source').AsString));
...
//              tmpChamp.Domain_Type.FDefault := '';
        ...
        try
          ...
          while not QueryList.Eof do
          begin
            FListeTriggers.Add(Trim(QueryList.FieldByName('name').AsString), MD5.HashStringAsHex(Trim(QueryList.FieldByName('source').AsString)));
            QueryList.Next();
          end;
        finally
          QueryList.Close();
