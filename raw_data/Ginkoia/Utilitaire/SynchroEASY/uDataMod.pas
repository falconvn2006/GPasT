...
type
  ...
  public
    ...
    function Controle_SYMDS_READY(const ABaseDonnees: TFileName):boolean;
...
function TDataMod.Controle_SYMDS_READY(const ABaseDonnees: TFileName):boolean;
...
begin
  ...
  try
    try
      // Controle de la présence de SYM_DATA
      vquery.Close();
      vquery.SQL.Clear;
      vquery.SQL.Add('SELECT *              ');
      vquery.SQL.Add(' FROM RDB$RELATIONS   ');
      vquery.SQL.Add(' WHERE RDB$SYSTEM_FLAG=0 AND RDB$RELATION_NAME=''SYM_DATA'' ');
      vQuery.Open();
      if vquery.IsEmpty then
         exit;

      // Controle de la présence de GEN_SYM_DATA_DATA_ID
      vquery.Close();
      vquery.SQL.Clear;
      vquery.SQL.Add('SELECT *                ');
      vquery.SQL.Add('  FROM RDB$GENERATORS   ');
      vquery.SQL.Add(' WHERE RDB$GENERATOR_NAME=''GEN_SYM_DATA_DATA_ID''  ');
      vQuery.Open();
      if vquery.IsEmpty then
         exit;

      // Controle de la présence de TRG_SYM_DATA_DATA_ID
      vquery.Close();
      vquery.SQL.Clear;
      vquery.SQL.Add('SELECT *              ');
      vquery.SQL.Add(' FROM RDB$TRIGGERS    ');
      vquery.SQL.Add(' WHERE RDB$TRIGGER_NAME=''TRG_SYM_DATA_DATA_ID''  ');
      vQuery.Open();
      if vquery.IsEmpty then
         exit;
      ...
      if vquery.IsEmpty
        then exit
          ...
          begin
            if vquery.Fields[0].AsInteger<>44 then exit;
          end;
    ...
    Except On E:Exception
      do
        begin
            result:=false;
           // Si ce n'est pas présent..... il faut faire une pause de 3s
           // Pause(3);
        end;
    end;
  finally
