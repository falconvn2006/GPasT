...
USes System.Win.Registry, WinSvc, Types, SysUtils, ShellAPI, Windows;
...
function ServiceRestart(sMachine, sService: string): Boolean;
function ServiceGetStatus(sMachine, sService: PChar): word;
function InfoService(aServiceName:string):string;
...
function ServiceRestart(sMachine, sService: string): Boolean;
var
  StopOk, StartOk: Boolean;
begin

  if ServiceStop(sMachine, sService) then
  begin
    if (ServiceStart(sMachine, sService)) then
      Result := True
    else
      Result := False;
  end
