...
interface
  ...
  uses SysUtils, Windows, Forms, Shlobj, TlHelp32, WinSvc, Shellapi, StdCtrls,
       Controls, Consts, Graphics, System.StrUtils;
  ...
  function GetEnumServiceStatus(Scm: SC_HANDLE; ServiceType, ServiceState: DWORD; var Count: DWORD; var Status: PEnumServiceStatus): Boolean;
  function GetServicesNameByPath(aPath: String): String;
...
function GetEnumServiceStatus(Scm: SC_HANDLE; ServiceType, ServiceState: DWORD; var Count: DWORD; var Status: PEnumServiceStatusW): Boolean;
var
  ResumeHandle, Size: DWORD;
begin
  ResumeHandle := 0;
  Size := 16384 + SizeOf(TEnumServiceStatus);
  Status := AllocMem(Size);

  Result := EnumServicesStatus(Scm,
  ServiceType,
  ServiceState,
  Status^,
  Size,
  ...
  begin
    ...
    Result := EnumServicesStatus(Scm, ServiceType, ServiceState, Status^,
...
var
  Scm: SC_HANDLE;
  I, Count: DWORD;
  SvcList, SvcEntry: PEnumServiceStatus;
  vServicePath, test: String;

  procedure ProcessService;
  var
    Service: SC_HANDLE;
    vConfigService: LPQUERY_SERVICE_CONFIG;
    nSize, nBytesNeeded: DWORD;
  begin
    Service := OpenService(Scm, SvcEntry.lpServiceName, SERVICE_QUERY_CONFIG);
    if Service <> 0 then
    begin
      QueryServiceConfig(Service, nil, 0, nSize);
      vConfigService := AllocMem(nSize);
      try
        if QueryServiceConfig(Service, vConfigService, nSize, nBytesNeeded) Then
          vServicePath := vConfigService^.lpBinaryPathName;

        if (ReplaceStr(LowerCase(vServicePath), '/', '\') = ReplaceStr(LowerCase(aPath), '/', '\')) then
          Result := SvcEntry^.lpServiceName;

      finally
        Dispose(vConfigService);
      end;
...
begin
  ...
  if Scm <> 0 then
  begin
    if GetEnumServiceStatus(Scm, SERVICE_TYPE_ALL, SERVICE_STATE_ALL, Count, SvcList) then
    begin
      SvcEntry := SvcList;

      for I := 0 to Count - 1 do
      begin
//        test := SvcEntry^.lpServiceName;
//        if LowerCase(LeftStr(SvcEntry^.lpServiceName, 3)) = 'gin' then
//          test := SvcEntry^.lpServiceName;
