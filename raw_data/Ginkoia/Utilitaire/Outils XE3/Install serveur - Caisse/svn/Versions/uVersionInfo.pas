...
begin
  ...
  begin
    ...
    try
      ...
      try
        if not GetFileVersionInfo(PChar(Fichier), Dummy, InfoSize, Info)
          then RaiseLastOSError();
        if not VerQueryValue(Info, '\', Pointer(VerValue), InfoDataSize)
          then RaiseLastOSError();
        with VerValue^ do
        begin
          if Precision > 0 then
      ...
      try
        ...
        begin
          ...
          if VerQueryValue(Info, @InfoType[1], InfoData, InfoDataSize) then
            InfoValue := StrPas(PChar(InfoData))
          else
            InfoValue := '';
          Result.Add(FileInfos[i] + '=' + InfoValue);
        end;
      finally
        FreeMem(Info, InfoSize);
      end;
      ...
      try
        ...
        begin
          ...
          if VerQueryValue(Info, @InfoType[1], InfoData, InfoDataSize) then
            ...
            InfoValue := '';
          case i of
            1 : Result.FileDescription := InfoValue;
            2 : Result.CompanyName := InfoValue;
            3 : Result.FileVersion := InfoValue;
            4 : Result.InternalName := InfoValue;
            5 : Result.LegalCopyright := InfoValue;
            6 : Result.OriginalFileName := InfoValue;
            7 : Result.ProductName := InfoValue;
            8 : Result.ProductVersion := InfoValue;
          end;
        end;
      finally
        FreeMem(Info, InfoSize);
