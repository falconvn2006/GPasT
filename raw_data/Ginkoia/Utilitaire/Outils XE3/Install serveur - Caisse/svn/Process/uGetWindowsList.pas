...
function FindProcessByCmd(Commande : string) : TProcessInfo;
...
// recherche multiple
function FindMultiProcess(ProcessName : string) : TList<TProcessInfo>;
function FindMultiProcessByCmd(Commande : string) : TList<TProcessInfo>;
// recherche a la con pour la gestion des ALG
function FindWindowsByClassName(WndName, ClsName : string) : TWindowsInfo;
  ...
  try
    if GetProcessList(ProcessListe) = 0 then
      for i := 0 to ProcessListe.Count -1 do
        if SameText(ProcessListe[i].Name, ProcessName) then
        begin
...
begin
  ...
  try
    if GetProcessList(ProcessListe) = 0 then
      for i := 0 to ProcessListe.Count -1 do
        if SameText(ProcessListe[i].Cmd, Commande) then
        begin
          Result := ProcessListe[i];
          Break;
        end;
...
var
  ...
  i : integer;
begin
  Result := TList<TProcessInfo>.Create();
  ProcessListe := nil;
  try
    if GetProcessList(ProcessListe) = 0 then
      for i := 0 to ProcessListe.Count -1 do
        if SameText(ProcessListe[i].Name, ProcessName) then
          Result.Add(ProcessListe[i]);
  finally
    FreeAndNil(ProcessListe);
  end;
end;
...
begin
  ...
  WindowsListe := nil;
  try
    if GetWindowsList(WindowsListe) = 0 then
      for i := 0 to WindowsListe.Count -1 do
      begin
        if SameText(WindowsListe[i].Name, WndName) and
           SameText(WindowsListe[i].Classe, ClsName) then
        begin
          Result := WindowsListe[i];
          Break;
        end;
      end;
  finally
