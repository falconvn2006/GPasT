...
constructor TOptimizeDB.Create(aIBFile:TFileName;
    // aOptions        : string;  on va directement lire les options dans la base ?
    aStatusCallBack : TStatusMessageCall;
    aLogCallBack    : TLogMessageCall;
    aEvent:TNotifyEvent=nil);
begin
...
procedure TOptimizeDB.GetBasId();
var
  vQuery: TFDQuery;
begin
    ...
    try
      ...
      begin
        FBASID := vQuery.FieldByName('BAS_ID').Asinteger;
      end
      else
        raise Exception.Create('Impossible de récupérer le BAS_ID dans la BDD');
      vQuery.Close;
    finally
      vQuery.DisposeOf;
    end;
...
procedure TOptimizeDB.Etape_Get_Options();
var vQuery:TFDQuery;
    i:integer;
begin
    FStatus := 'Récupération des Options d''Optimisation';
    if Assigned(FStatusProc) then Synchronize(StatusCallBack);
    FLog := 'Récupération des Options d''Optimisation';
    ...
    try
      ...
      vQuery.ParamByName('BASID').AsInteger := FBASID;
      vQuery.Open();
      If (vQuery.RecordCount = 1) then
        begin
          FOptions := vQuery.FieldByName('PRM_STRING').Asstring;
        end;
      // Petite protection
      if Length(FOptions)<4 then FOptions:='0000';
      vQuery.Close;
    finally
      vQuery.DisposeOf;
    end;
