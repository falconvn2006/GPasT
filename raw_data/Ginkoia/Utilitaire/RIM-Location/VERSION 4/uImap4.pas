...
uses IdIMAP4, IdExplicitTLSClientServerBase, IdSSL, IdSSLOpenSSL, IdMessageCollection,
     SysUtils, Classes, StdCtrls, ComCtrls, IdComponent, Forms, IdLogEvent, Progbr3d;
...
Type
  ...
  published
    ...
    property PGStatus3D : ProgressBar3D read FPGStatus3D write FPGStatus3D;
  End;
...
procedure IMAP4Class.IdLogEventReceived(ASender: TComponent; const AText,
  AData: string);
begin
  if CurrentMailBoxcount = 0 then
    Exit;

  if (FPGStatus <> nil) then
    FPGStatus.Position := FIdMessageCollection.Count * 100 Div CurrentMailBoxcount;
  if (FPGStatus3D <> nil) then
    FPGStatus3D.Progress := FIdMessageCollection.Count * 100 Div CurrentMailBoxcount;
...
begin
  ...
  if FPGStatus3D <> nil then
    FPGStatus3D.Progress := FIdMessageCollection.Count * 100 Div FWorkCountMax;
  Application.ProcessMessages;
end;
