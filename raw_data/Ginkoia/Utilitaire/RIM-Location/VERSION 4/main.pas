...
uses
  ...
  Ulog,Inifiles,DateUtils, dbugintf, Diagnostic_frm, ReservationTypeSkiset_Defs;

type
  TFmainRim = class(TForm)
    Pan_Fond: TRzPanel;
    Pan_Btn: TRzPanel;
    Gge_A: ProgressBar3D;
    Pan_Text: TRzPanel;
...
var
  ...
  cpt_noMAJ : Integer = 0;  // comptage des résa déjà existante mais non mise à jour
  cpt_total : Integer = 0;  // comptage total des résa traitées
  ...
  bAPI : Boolean;
  vDiagnostic : TResaSkisetDiagnostic;     // classe utilisée pour renseigner la fenêtre de diagnostic
...
procedure TFmainRim.trclog(smsg:string); //PDB
var
  i:integer;
  sprem,si:string;
        ...
        begin
          ...
          begin
            try
              ...
              if Assigned(vDiagnostic) then
              begin
                // G = Erreur fonctionnelle mais non archivable
                if sprem='G' then
                begin
                  FDiagnostic.MemD_Reservations.Append;
                  FDiagnostic.MemD_Reservations.FieldByName('Centrale').AsString := vDiagnostic.Centrale + ' (' + vDiagnostic.CodeAdherent + ')';
                end
                else
                begin
                  //On vérifie s'il n'y a pas déjà une ligne pour ce couple centrale+résa
                  if FDiagnostic.MemD_Reservations.Locate('Centrale;NumResa',VarArrayOf([vDiagnostic.Centrale, vDiagnostic.NumResa]),[loCaseInsensitive]) then
                  begin
                    //Oui on edit
                ...
                begin
                  // Info Centrale
                  if sprem = 'A' then
                    FDiagnostic.MemD_Reservations.FieldByName('Centrale').AsString := vDiagnostic.Centrale
                  else
                    FDiagnostic.MemD_Reservations.FieldByName('Centrale').AsString := vDiagnostic.Centrale + ' ('+ vDiagnostic.CodeAdherent +')';

                  // Info Date
                  FDiagnostic.MemD_Reservations.FieldByName('Date').AsDateTime := vDiagnostic.DateResa;

                  // Info NumResa
                  if vDiagnostic.NumResa <> '' then
                    FDiagnostic.MemD_Reservations.FieldByName('NumResa').AsString := vDiagnostic.NumResa
                  else
                    FDiagnostic.MemD_Reservations.FieldByName('NumResa').AsString := '<Non disponible>';

                  // Info Client
                  if (vDiagnostic.Client <> '') and (vDiagnostic.Client <> '<Non disponible>') then
                    FDiagnostic.MemD_Reservations.FieldByName('Client').AsString := vDiagnostic.Client
                  else
                    FDiagnostic.MemD_Reservations.FieldByName('Client').AsString := '<Non disponible>';
                end;

                //Ne rien afficher si date nil/0
                if FDiagnostic.MemD_Reservations.FieldByName('Date').AsDateTime = 0 then
              ...
              begin
                ...
                FDiagnostic.MemD_Reservations.FieldByName('Raison').AsString := copy(si,2);
              ...
              // Pour SKISET, on ne peut pas archiver les réservations, toutes les lignes seront donc ENABLED à False (0)
...
procedure TFmainRim.p_maj_etat(setat : string);
begin
...
procedure TFmainRim.FindeSessionWindows(var aMsg: TMessage);
var oMsg : Tform ;
begin
  Try
...
Procedure TFmainRim.SetDatabase(sDatabase : string);
...
begin
  ...
  else if paramstr(1) = '/V' then
  begin
    Self.Caption := RS_RESERVATION_CAPTION_MODE_VERIF;
    ologfile.Addtext(RS_RESERVATION_START+' Verification');
    trclog('T'+RS_RESERVATION_START+' Verification');
    FAuto := True ;
  end
  else
  begin
  ...
  begin
    Try
      ...
      Try
        ...
        begin
          trclog('TParamètre '+inttostr(i)+' : '+paramstr(i));
        end;
    ...
    Finally
      ...
      if (FDiagnostic.bdonotshow) or FModeVerif then
        trclog('TDoNotShow=true')
      else
        trclog('TDoNotShow=false');

      if FModeVerif then
      begin
        trclog('TMode Verif, kill de RIM');
        TerminateProcess(GetCurrentProcess, 0);
      end;

      if (not FDiagnostic.bdonotshow) and (FDiagnostic.MemD_Reservations.RecordCount>0) and (not FModeVerif) then
      begin
        oLOgfile.Free ;
        oLOgfile := nil ;
...
Procedure TFmainRim.ExecuteTraitementReservation;
...
begin
  ...
  try
    try
      Dm_Reservation.MemD_Rap.Close;
      Dm_Reservation.MemD_Rap.Open;
      ...
      begin
        ...
        iId_centrale:=FListCentrale.Items[i].MTY_ID;
        ...
        if (SameText(sNom_centrale, CINTERSPORT)) then
          bISF := True
        else
          bISF := False;
        ...
        case FListCentrale.Items[i].MTY_MULTI of
          0: // poste référant uniquement
          begin
            bAutorisation := (FPostID = Dm_Reservation.GetPostReferantId);
          end;

          1: // magasin uniquement
          begin
            bAutorisation := Dm_Reservation.IsMagAutorisation(FListCentrale.Items[i].MTY_ID,FMagID, FPostID);
          end;
        end;
        ...
        begin
          ...
          // Mode API (Skiset, Netski, GoSport, Skimium)
          if (FListCentrale.Items[i].MTY_CODE='RS7')
            or (FListCentrale.Items[i].MTY_CODE='RNS')
            or (FListCentrale.Items[i].MTY_CODE='RGS')
            or (FListCentrale.Items[i].MTY_CODE='RSKAPI') then
          begin
            trclog('TTraitement WebService');
            bok_API := True;
            bAPI := True;

            trclog('TLecture du fichier '+extractFilePath(application.exename)+'RIM.INI');
            //Normalement le fichier INI existe touours car si inexistant il est créé au démarrage de GINKOIA
            s := extractFilePath(application.exename)+'RIM.INI';
            if fileexists(s) then
            begin
              RimIni := TIniFile.Create(s);

              if RimIni.SectionExists('SKISET') then
              begin
                sdate_begin:=trim(lowercase(RimIni.ReadString('SKISET','date_begin','')));
                sdate_end:=trim(lowercase(RimIni.ReadString('SKISET','date_end','')));
                RimIni.Free;

                if (sdate_begin<>'') and (sdate_end<>'') then
                begin
                  ...
                  //date_begin
                  if pos('today',sdate_begin)>0 then
                  begin
                    ddate_begin:=now;
                    if pos('today+',sdate_begin)>0 then
                    begin
            ...
            begin
              ...
              Dm_Reservation.CreateResaSki7(FListCentrale.Items[i], ddate_begin, ddate_end, FModeVerif);
            end;
          end
          else
          begin
            ...
            trclog('TVérification des mails de la centrale en cours (n°='+inttostr(AnsiIndexStr(FListCentrale.Items[i].MTY_NOM,[CTWINNER,CINTERSPORT,CSKIMIUM,CSPORT2K,CGENE1,CGENE2,CGENE3]))+')');
...
//              trclog('Q-'); //pour forcer à ne pas afficher la fenêtre de diagnostic
            ...
            begin
              ...
              begin
                ...
                case AnsiIndexStr(FListCentrale.Items[i].MTY_NOM,[CTWINNER,CINTERSPORT,CSKIMIUM,CSPORT2K,CGENE1,CGENE2,CGENE3]) of
                  0,2,3,4,5,6: bResaOk := Dm_Reservation.CreateResa(FListCentrale.Items[i]); //Twinner, Skimium, Sport2k, Gen1,Gen2, Gen3
                ...
                case AnsiIndexStr(FListCentrale.Items[i].MTY_NOM,[CTWINNER,CINTERSPORT,CSKIMIUM,CSPORT2K,CGENE1,CGENE2,CGENE3]) of
                 //PDB - A noter : PAS D'ANNULATION pour Twinner et InterSport
                  2,3,4,5,6: bAnnulOk := Dm_Reservation.AnnulResa; // Sport2000, Gen1, Gen2, Gen3,...
                else
                  bAnnulOk := True;
                end; // case annulation
              ...
              begin
                if (main.bcreation_oc) and (not bcreation_init_oc) then
                  trclog('ACréation d''OC complémentaires')
                else
                  trclog('TLe traitement des réservations s''est mal déroulé, pas d''intégration possible');
              end;
            end
            else
            begin
...
//                trclog('Q-'); //pour forcer à ne pas afficher la fenêtre de diagnostic
        ...
        begin
          ...
          ShowmessageRS(sNom_centrale + ' : Pas d''autorisation. Traitement abandonné.', 'Erreur');
...
procedure TFmainRim.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  canclose := false ;
  uLog.Log.Close;  //ULOG
...
procedure TFmainRim.FormCreate(Sender: TObject);
var sId : string ;
begin
    Chargement_Langue();
    Decimalseparator := '.' ;
    ...
    // si c'est ce paramètre, c'est le mode ou on vérifie toutes les offres centrales pour les 120 jours à venir
    //et on les tag en base si pas présentes ou pas associées à une offre locale
    FModeVerif := (Paramstr(1) = '/V');
    ...
    // Init de bSKISET
    bAPI := False;
    vDiagnostic := TResaSkisetDiagnostic.Create;
...
Procedure  TFmainRim.ArchivMail(MaCentrale : TGENTYPEMAIL;MaCFG : TCFGMail; reprejet : string = '');
const
  Retry = 3;
var
...
procedure TFmainRim.UpdateGauge;
begin
...
Procedure TFmainRim.InitGauge(sMessage : string ;MaxValue: Integer);
begin
//    if Fauto = True then exit ;
...
procedure TFmainRim.ResetGauge;
begin
  //  if Fauto = True then exit ;
    Gge_A.Progress := 0 ;
    Gge_A.MaxValue := 0 ;
...
procedure TFmainRim.ShowmessageRS(sMessage, sTitle: string);
...
procedure TFmainRim.ShowmessageRS(sMessage, sTitle: string; bDetail: boolean);
var bNeedToDisplay : boolean;
begin
 //if Fauto = True then exit ;
