...
begin
  ...
  try
    ...
    try
      ...
      //----------------------------------------------
      main.trclog('1Web service SKISET');
      main.trclog('1');
      main.trclog('11-- Lecture des paramètres d''accès au webservice');
      ...
      cpt_trc:=60;
      main.trclog('1 - URL : '+surl);
      main.trclog('1 - Clé : '+scle);
      main.trclog('1 - URI liste des magasins : '+suri_listmag);
      main.trclog('1 - URI liste des offres : '+suri_listoffre);
      main.trclog('1 - URI liste des réservations : '+suri_listresa);
      main.trclog('1 - URI liste des status : '+suri_liststatus);
      main.trclog('1');
      ...
      //-----------------------------------
      main.trclog('1');
      main.trclog('12--Récupération de la liste des status');
      ...
      if nroot<>nil then begin
        cpt_trc:=100;
        main.trclog('1'+inttostr(nroot.GetNodeList.length)+' status disponibles');
        node:=nroot.getFirstChild;
        while node<>nil do begin
          soc_id:=status_xml.ValueTag(node,'id');
          soc_nom:=status_xml.ValueTag(node,'name');
          main.trclog('1  '+soc_id+'-'+soc_nom);
          node:=node.nextSibling;
        end;
      end
      else main.trclog('1Noeud xml racine inexistant ("statuses"). Traitement abandonné.');
      main.trclog('1Fin traitement des status');
      main.trclog('1');
      ...
      //------------------
      main.trclog('1');
      main.trclog('13--Traitement des offres');
      ...
      if nroot<>nil then begin
        main.trclog('1'+inttostr(nroot.GetNodeList.length)+' offres disponibles');
        ...
        begin
          ...
          ParamCheck := True;
          ParamByName('PMtyId').AsInteger := MaCentrale.MTY_ID;
          trclog('1Ouverture des offres avec PMtyId:='+inttostr(MaCentrale.MTY_ID));
          Open;
          //trclog('1'+SQL.text);
          trclog('1Nbre. d''offres dans la base='+inttostr(recordcount));
        end;
        cpt_trc:=150;

        main.trclog('1Boucle sur les offres');
        cpt_offer:=0;
        node:=nroot.getFirstChild;
        while node<>nil do begin
          inc(cpt_offer);
          ...
          main.trclog('1  n° '+inttostr(cpt_offer)+' '+soc_id+'-'+soc_nom+'   ->'+ss);

          node:=node.nextSibling;
        end;
      end
      else main.trclog('1Noeud xml racine inexistant ("offertypes"). Traitement abandonné.');
      main.trclog('1Fin traitement des offres');
      main.trclog('1');
      ...
      //-----------------------------
      main.trclog('1');
      main.trclog('14--Récupération des magasins disponibles');
      ...
      if nroot<>nil then begin
        main.trclog('1'+inttostr(nroot.GetNodeList.length)+' magasin(s) disponibles');
        node:=nroot.getFirstChild;
        i:=0;
        while node<>nil do begin
          ...
          main.trclog('1> '+smag_id+'-'+smag_nom+'-'+smag_active+'-'+smag_resortid+'-'+smag_resort);
          ...
          main.trclog('1  Recherche du magasin "'+smag_nom+'" (adh='+smag_id+')');
          Que_TmpNoEvent.Open;

          if Que_TmpNoEvent.Recordcount > 0 then begin
            main.trclog('1 Ce magasin existe et sera pris en compte');
            inc(i);
            setlength(tlistmag,i);
            tlistmag[i-1]:=smag_id;
          end
          else main.trclog('1 Ce magasin n''existe pas dans la base et sera ignoré');
          *)

          node:=node.nextSibling;
        end;
      end
      else main.trclog('1Noeud xml racine inexistant ("shops"). Traitement abandonné.');

      main.trclog('1Fin traitement des magasins disponibles');
      main.trclog('1');

      cpt_trc:=215;

      // Récupération de la liste des magasins à traiter
      trclog('1Récupération des magasins à traiter dans le module');
        ...
        while not dm_reservation.Que_IdentMagExist.EOF do begin
          if ((MaCentrale.MTY_MULTI <> 0) and (dm_reservation.POSID = dm_reservation.Que_IdentMagExist.FieldByName('IDM_POSID').AsInteger)) Or
            ...
            for i := 0 to lstRep.Count - 1 do begin
              ...
              trclog('1Identifiant "'+tlistmag[cpt_mag-1]+'"');
            end;
          end;
      ...
      dm_reservation.Que_IdentMagExist.Close;
      main.trclog('1'+inttostr(cpt_mag)+' magasin(s) identifié(s)');
      main.trclog('1Fin traitement des magasins à traiter');
      main.trclog('1');

      if length(tlistmag)=0 then begin
        main.trclog('1> Aucun magasin à traiter, traitement abandonné');
        exit
      end;
      ...
      main.trclog('1');
      main.trclog('15--Récupération des réservations du '+datetostr(ddate_begin)+' au '+datetostr(ddate_end));
      ...
      main.trclog('1Vérification de la correspondance avec les OC');
                      ...
                      if node5<>nil then begin
                        ...
                        if not IsOCParamExist(MaCentrale.MTY_ID,StrToIntDef(sIdArt,-1),iRLOOption) then begin
                          ...
                          //  RS_TXT_RESCMN_ERREUR) ;
                           ...
                           //Plus de détail
                           main.trclog('X  '+MaCentrale.MTY_NOM+'  id='+sbookingid+' du magasin "'+tlistmag[i-1]+'" en date du '+sdate+' n° article '+inttostr(cpt_art)+' -> '+soffertype_id+'-'+soffertype_name+' -> PAS OK!');

                        end
                        else main.trclog('1  id='+sbookingid+' du magasin "'+tlistmag[i-1]+'" en date du '+sdate+' n° article '+inttostr(cpt_art)+' -> '+soffertype_id+'-'+soffertype_name+' -> OK');
      ...
      if not bcheckoc then begin
        main.trclog('1> Erreur de correspondance avec les OC, traitement abandonné');
        exit;
      end;

      main.trclog('1Correspondance OK avec les OC');
      main.trclog('1> Intégration des réservations');
      ...
      while dcurdate<=ddate_end do begin
        ...
        sdate:=datetostr(dcurdate);
        main.trclog('1');
        main.trclog('1> En date du : '+sdate);
        ...
        for i := 1 to length(tlistmag) do begin
          ...
          cpt_trc:=230;
          main.trclog('1');
          main.trclog('1> Pour le magasin '+tlistmag[i-1]);
          ...
          if nroot<>nil then begin
            cpt_trc:=270;
            main.trclog('1'+inttostr(nroot.GetNodeList.length)+' réservations disponibles');
            ...
            while node<>nil do begin
              inc(cpt_resa);
              main.trclog('1Lecture de l''xml :');
              cpt_trc:=280;
              node2:=booking_xml.FindTag(node,'head');
              if node2<>nil then begin

                sorigin:=booking_xml.ValueTag(node2,'origin');             //main.trclog('1  origin='+sorigin);
                smemberid:=booking_xml.ValueTag(node2,'memberid');         //main.trclog('1  memberid='+smemberid);
                sresortid:=booking_xml.ValueTag(node2,'resortid');         //main.trclog('1  resortid='+sresortid);
                sshopid:=booking_xml.ValueTag(node2,'shopid');             //main.trclog('1  shopid='+sshopid);

                sbookingid:=booking_xml.ValueTag(node2,'bookingid');             main.trclog('1>>RESERVATION n°'+inttostr(cpt_resa)+' bookingid='+sbookingid);
                screationdate:=booking_xml.ValueTag(node2,'creationdate');       main.trclog('1  creationdate='+screationdate);
                supdatedate:=booking_xml.ValueTag(node2,'updatedate');           main.trclog('1  updatedate='+supdatedate);
                sstatus:=booking_xml.ValueTag(node2,'status');                   main.trclog('1    status='+sstatus);
                sfirstday:=booking_xml.ValueTag(node2,'firstday');               main.trclog('1    firstday='+sfirstday);
                sduration:=booking_xml.ValueTag(node2,'duration');               main.trclog('1    duration='+sduration);
                node3:=booking_xml.FindTag(node2,'customer');
                //Passage en uppercase car pas de cohérence dans l'XML
                sfirstname:=uppercase(booking_xml.ValueTag(node3,'firstname'));   main.trclog('1    firstname='+sfirstname);
                slastname:=uppercase(booking_xml.ValueTag(node3,'lastname'));     main.trclog('1    lastname='+slastname);
                sadress:=booking_xml.ValueTag(node3,'adress');                    main.trclog('1    adress='+sadress);
                szipcode:=booking_xml.ValueTag(node3,'zipcode');                  main.trclog('1    zipcode='+szipcode);
                scity:=booking_xml.ValueTag(node3,'city');                        main.trclog('1    city='+scity);
                scountry:=booking_xml.ValueTag(node3,'country');                  main.trclog('1    country='+scountry);
                sphonenumber:=booking_xml.ValueTag(node3,'phonenumber');          main.trclog('1    phonenumber='+sphonenumber);
                sdateofbirth:=booking_xml.ValueTag(node3,'dateofbirth');          main.trclog('1    dateofbirth='+sdateofbirth);

                //Traitement global de la réservation
                //-----------------------------------

                main.trclog('1');
                main.trclog('>>1INFORMATIONS DE SYNTHESE');
                sCentrale := MaCentrale.MTY_NOM;
                main.trclog('1> Centrale : '+sCentrale);
                ...
                if not TryStrToDateTime(screationdate, dDateResa, FSetting) then begin
                  dDateResa:=0;
                  main.trclog('1> Date de création : ERREUR DE CONVERSION de '+screationdate);
                end
                else main.trclog('1> Date de création : '+datetimetostr(dDateResa));
                //Date de début
                if not TryStrToDate(sfirstday, dDateDebut, FSetting) then begin
                  dDateDebut:=0;
                  main.trclog('1> Date de début : ERREUR DE CONVERSION de '+sfirstday);
                end
                else main.trclog('1> Date de début : '+datetostr(dDateDebut));
                //Durée
                iNbJours := StrToIntDef(sduration,0);
                main.trclog('1> Durée : '+inttostr(iNbJours));
                //Calcul de la date de fin
                if (dDateDebut>0) and (iNbJours>0) then begin
                  dDateFin   := dDateDebut + iNbJours -1;
                  main.trclog('1> Date de fin : '+datetostr(dDateFin));
                end
                else main.trclog('1> Date de fin : ERREUR DE CALCUL');
                ...
                if supdatedate<>'' then begin
                  ...
                  if not TryStrToDate(supdatedate, dDateMaj, FSetting) then begin
                    dDateMaj:=0;
                    main.trclog('1> Date de mise à jour : ERREUR DE CONVERSION de '+supdatedate);
                    main.trclog('1> -> La demande de mise à jour n''est pas valable -> ignorer la resa');
                  end
                  else begin
                    main.trclog('1> Date de mise à jour : '+datetostr(dDateMaj));
                    main.trclog('1> Vérification du status');
                    if sstatus='22' then begin
                      main.trclog('1> Status de mise à jour : '+sstatus);
                      bmajok:=true;
                      main.trclog('1> -> La demande de mise à jour est valable -> poursuivre avec la resa');
                    end
                    else begin
                      main.trclog('1> Status de mise à jour : '+sstatus);
                      main.trclog('1> -> La demande de mise à jour n''est pas valable');
                    end;
                  end;
                end;
                ...
                if (sstatus='30') or (sstatus='31') then begin
                  bresaannul:=true;
                  main.trclog('1> Demande d''annulation');
                  if bresaexist then begin
                    main.trclog('1> Status d''annulation : '+sstatus);
                    main.trclog('   Récupération du status "Annulé" dans la base');
                    with Que_TmpNoEvent do begin
                      Close;
                      sql.Clear;
                    ...
                    ss:=inttostr(iIdAdr);
                    main.trclog('   Status "Annulé" = '+ss);

                    if iIdAdr>0 then begin

                      main.trclog('   Mise à jour de l''état de réservation à  = '+ss);
                      ...
                      main.trclog('   Status d''annulation non renseigné dans la base -> annulation de la resa ignorée');
                    end;
                    iIdAdr:=0;
                  end
                  else begin
                    main.trclog('   La réservation renseignée n''existe pas -> annulation de la resa ignorée');
                  end;
                end;
                ...
                if (not bresaannul) and (bresaexist) and (bmajresa) and (bmajok) then begin
                  ...
                  cpt_trc:=310;
                  main.trclog('1> La réservation "'+sbookingid+ '" existe déjà -> validation des autres éléments indiquant sa mise à jour effective');
                  Que_TmpLoc.Close;
                  Que_TmpLoc.SQL.Clear;
                  ...
                  cpt_trc:=320;
                  Que_TmpLoc.ExecSQL;
                  main.trclog('   Déjà transféré en BL : '+inttostr(Que_TmpLoc.fieldbyname('RVS_BL').asinteger));
                  main.trclog('   Date de dernière màj : '+datetimetostr(Que_TmpLoc.fieldbyname('RVS_PAYMENTTIME').asdatetime));
                  ...
                  if bmajok then begin
                    ...
                    main.trclog('   > Toutes les conditions sont réunies pour traiter la resa en màj');

                    try

                    dm_main.StartTransaction;

                    main.trclog('1>   suppression des sous-lignes');
                    ...
                    //main.trclog('1  Sélection sous-lignes SQL='+Que_Tmp.SQL.text);
                    cpt_trc:=320;
                    Que_TmpLoc.ExecSQL;
                    ...
                    main.trclog('1>   suppression des lignes');
                    ...
                    main.trclog('1>   suppression de la réservation');
                    ...
                    main.trclog('1> Recherche de la resa dans genimport '+ss);
                    Que_TmpNoEvent.Open;

                    if Que_TmpNoEvent.Recordcount <> 0 then begin
                      cpt_trc:=384;
                      iIdGenImp := Que_TmpNoEvent.FieldByName('imp_id').AsInteger;
                      main.trclog('1> Trouvé idgenimport='+inttostr(iIdGenImp)+', on supprime');
                    ...
                    else main.trclog('1> Pas trouvé, on ignore...');
                    ...
                    Que_TmpNoEvent.SQL.Add('  and imp_ref='+inttostr(MaCentrale.MTY_ID)); //prende en compte l'ID de la centrale
                    main.trclog('1> Recherche du client dans genimport '+sFullname);
                    Que_TmpNoEvent.Open;
                    ...
                    if Que_TmpNoEvent.Recordcount > 0 then begin
                      ...
                      if Que_Client.Recordcount > 0 then begin
                        ...
                        main.trclog('1> Trouvé adr_id='+inttostr(iIdAdr)+', on supprime');
                    ...
                    cpt_trc:=400;
                      ...
                      on e:exception do begin
                        main.trclog('X> Exception lors de la suppression de la réservation : '+e.Message);
                        Dm_Main.Rollback;
                      end;
                    end;

                  end
                  else main.trclog('   > Toutes les conditions ne sont pas réunies pour traiter la resa en màj!');
                ...
                if (not bresaannul) and (not bresaexist) or ( (bresaexist) and (bmajresa) and (bmajok) ) then begin
                  ...
                  main.trclog('1> Traitement du client + Addresse');
                  cpt_trc:=410;
                  ...
                  begin
                    ...
                    main.trclog('1> Recherche du client '+sfullname);
                    Open;
                  ...
                  *)
                  ...
                  //ON FORCE L'AJOUT
                  FlagInsert := true;
                  main.trclog('1> On force l''ajout d''un nouveau client dans tous les cas');
                  ...
                  if FlagInsert then  begin
                    ...
                    cpt_trc:=430;
                    main.trclog('1> Le client '+sfullname+ ' n''existe pas encore -> création');

                    //D'abord ajout de son adresse (si elle existe dans l'xml, au moins le pays)
                    main.trclog('1> Traitement de l''adresse');

                    //On créé toujours l'adresse même vide
                    //iIdAdr:=0;
                    //if (scountry<>'') then begin

                      //main.trclog('1Il y a au moins le pays de renseigné-> création de l''adresse');
                    ...
                    //end
                    //else main.trclog('1Il n''y a pas au minimum le pays de renseigné-> pas de création de l''adresse');

                    cpt_trc:=440;
                    main.trclog('1> Traitement du client');
                    ...
                    With Que_Client do begin
                      ...
                      trclog('1>iMagId='+inttostr(iMagId));
                      ...
                      *)
                      ...
                      main.trclog('>1CLT_ID créé='+inttostr(iIdClient));
                    end;
                    ...
                    main.trclog('1Répercution dans GENIMPORT');
                    ...
                    main.trclog('1> Le client '+sfullname+ ' existe déjà ->màj de l''adresse');
                    ...
                    main.trclog('>1CLT_ID existant='+inttostr(iIdClient));
                  ...
                  main.trclog('1> Création de la réservation "'+sbookingid+ '"');
                  ...
                  *)
                  ...
                  //Codebarre client
                  main.trclog('1> Récupération du codebarre client');
                  InsertCodeBarre(iIdClient);

                  cpt_trc:=480;
                  //Traitement de l'entête de la reservation
                  main.trclog('1> Récupération du chrono pour la réservation');
                  sNumChrono := GetProcChrono;
                  main.trclog('1Ajout de la réservation, chrono='+sNumChrono);
                  ...
                  main.trclog('1RVS_ID créé='+inttostr(iIdResa));

                  cpt_trc:=485;
                  main.trclog('1  Mise à jour de la date de traitement');
                  Que_Resa.Edit;
                  Que_Resa.fieldbyname('RVS_PAYMENTTIME').asdatetime:=now;
                  Que_Resa.Post;
                  main.trclog('1  Fixée au '+datetimetostr(Que_Resa.fieldbyname('RVS_PAYMENTTIME').asdatetime));
                  ...
                  //-------------
                  main.trclog('1Traitement des articles de la réservation');
                  ...
                  if node2<>nil then begin
                    ...
                    main.trclog('1  '+inttostr(node2.GetNodeList.length)+' articles');
                    node3:=node2.getFirstChild;
                    cpt_art:=0;

                    main.trclog('1  Parcours des articles');
                    ...
                    while node3<>nil do begin
                      ...
                      inc(cpt_art);
                      main.trclog('1');
                      main.trclog('1  Article n°'+inttostr(cpt_art)+')');

                      //Infos sur la personne
                      main.trclog('1    Infos sur la personne :');
                      node4:=booking_xml.FindTag(node3,'person');
                      if node4<>nil then begin

                        sart_firstname:=booking_xml.ValueTag(node4,'first_name');    main.trclog('1      first_name='+sart_firstname);
                        sart_lastname:=booking_xml.ValueTag(node4,'last_name');      main.trclog('1      last_name='+sart_lastname);

                        node5:=booking_xml.FindTag(node4,'weight');
                        if node5<>nil then begin
                          sart_weight_value:=booking_xml.ValueTag(node5,'value');    main.trclog('1      weight_value='+sart_weight_value);
                          sart_weight_unit:=booking_xml.ValueTag(node5,'unit');      main.trclog('1      weight_unit='+sart_weight_unit);
                        end
                        else main.trclog('1      Noeud "weight" absent -> poids ignoré! -> poursuite de l''article.');

                        node5:=booking_xml.FindTag(node4,'height');
                        if node5<>nil then begin
                          sart_height_value:=booking_xml.ValueTag(node5,'value');    main.trclog('1      height_value='+sart_height_value);
                          sart_height_unit:=booking_xml.ValueTag(node5,'unit');      main.trclog('1      height_unit='+sart_height_unit);
                        end
                        else main.trclog('1      Noeud "height" absent -> taille ignorée! -> poursuite de l''article.');

                        node5:=booking_xml.FindTag(node4,'shoesize');
                        if node5<>nil then begin
                          sart_shoesize_value:=booking_xml.ValueTag(node5,'value');    main.trclog('1      shoesize_value='+sart_shoesize_value);
                          sart_shoesize_unit:=booking_xml.ValueTag(node5,'unit');      main.trclog('1      shoesize_unit='+sart_shoesize_unit);
                        end
                        else main.trclog('1      Noeud "shoesize" absent -> taille chaussure ignorée! -> poursuite de l''article.');

                        node5:=booking_xml.FindTag(node4,'helmetsize');
                        if node5<>nil then begin
                          sart_helmetsize_value:=booking_xml.ValueTag(node5,'value');    main.trclog('1      helmetsize_value='+sart_helmetsize_value);
                          sart_helmetsize_unit:=booking_xml.ValueTag(node5,'unit');      main.trclog('1      helmetsize_unit='+sart_helmetsize_unit);
                        end
                        else main.trclog('1      Noeud "helmetsize" absent -> taille casque ignorée! -> poursuite de l''article.');

                      end
                      else main.trclog('1      Noeud "person" absent -> réservation ignorée!');

                      //Infos sur Pack
                      main.trclog('1    Infos sur le pack :');
                      node4:=booking_xml.FindTag(node3,'pack');
                      if node4<>nil then begin
                        spack_withshoes:=booking_xml.ValueTag(node4,'withshoes');                main.trclog('1      withshoes='+spack_withshoes);
                        spack_withelmet:=booking_xml.ValueTag(node4,'withelmet');                main.trclog('1      withelmet='+spack_withelmet);
                        spack_skiertype:=booking_xml.ValueTag(node4,'skiertype');                main.trclog('1      skiertype='+spack_skiertype);
                        spack_skiersexe:=booking_xml.ValueTag(node4,'skiersexe');                main.trclog('1      skiersexe='+spack_skiersexe);
                        spack_snowposition:=booking_xml.ValueTag(node4,'snowposition');          main.trclog('1      snowposition='+spack_snowposition);
                        spack_insurancedamage:=booking_xml.ValueTag(node4,'insurancedamage');    main.trclog('1      insurancedamage='+spack_insurancedamage);
                        spack_skifit:=booking_xml.ValueTag(node4,'skifit');                      main.trclog('1      skifit='+spack_skifit);
                      end
                      else main.trclog('1    Noeud "pack" absent -> pack ignoré! -> poursuite de l''article.');

                      //Offer - Répétition du détail de l'offre (cf. liste des offres traitées en amont)
                      main.trclog('1    Infos répétées de l''offre :');
                      node5:=booking_xml.FindTag(node4,'offer');
                      if node5<>nil then begin
                        soffertype_id:=booking_xml.ValueTag(node5,'offertypeid');                main.trclog('1      offertypeid='+soffertype_id);
                        soffertype_name:=booking_xml.ValueTag(node5,'offertypename');            main.trclog('1      offertypename='+soffertype_name);
                        soffer_id:=booking_xml.ValueTag(node5,'offerid');                        main.trclog('1      offerid='+soffer_id);
                        soffer_name:=booking_xml.ValueTag(node5,'offername');                    main.trclog('1      offername='+soffer_name);
                        sequipment_id:=booking_xml.ValueTag(node5,'equipmentid');                main.trclog('1      equipmentid='+sequipment_id);
                        sequipment_name:=booking_xml.ValueTag(node5,'equipmentname');            main.trclog('1      equipmentname='+sequipment_name);
                        sage_groupid:=booking_xml.ValueTag(node5,'agegroupid');                  main.trclog('1      agegroupid='+sage_groupid);
                        sage_groupname:=booking_xml.ValueTag(node5,'agegroupname');              main.trclog('1      agegroupname='+sage_groupname);

                      end
                      else main.trclog('1    Noeud "offer" absent -> offre ignoré! -> poursuite de l''article.');

                      //Product - Détail du produit spécifique réservé
                      main.trclog('1    Infos sur le produit spécifique réservé :');
                      node5:=booking_xml.FindTag(node4,'product');
                      if node5<>nil then begin
                        sproduct_id:=booking_xml.ValueTag(node5,'id');               main.trclog('1      id='+sproduct_id);
                        sproduct_brand:=booking_xml.ValueTag(node5,'brand');         main.trclog('1      brand='+sproduct_brand);
                        sproduct_model:=booking_xml.ValueTag(node5,'model');         main.trclog('1      model='+sproduct_model);
                      end
                      else main.trclog('1    Noeud "product" absent -> produit ignoré! -> poursuite de l''article.');
                      ...
                      main.trclog('1    Vérification du lien avec l''OC');
          ...
          //              'Offre : ' +  MonXml.ValueTag(eArticleXml,('nom'))
            {*            InfoMessHP (ParamsStr(RS_ERR_RESADM_CANCELINTEG,VarArrayOf([MaCentrale.MTY_NOM,MemD_Mail.FieldByName('MailIdResa').AsString,MonXml.ValueTag(eArticleXml,('nom'))]))
                        ...
                        EXIT;
                      END;
                      main.trclog('1    La relation avec l''OC existe');

                      //Intégration de l'article
                      main.trclog('1    Ajout de l''article');
                      cpt_trc:=610;
                      iIdResaligne := InsertResaLigne(
                        ...
                        main.trclog('1    Traitement du commentaire');
                        if iIdAdr<>0 then begin
                          sTemp := '';
                        ...
                        else main.trclog('1    Pas d''adresse -> pas de commentaire');


                        cpt_trc:=630;
                        //Traitement des sous-lignes pour les réponses
                        main.trclog('1    Traitement des sous-lignes (réponses)');
                        ...
                        BEGIN
                          ...
                          //main.trclog('1Loop...'+Que_LOCTYPERELATION.sql.text);
                          //main.trclog('1PPrdID='+inttostr(Que_LOCOCRELATION.fieldbyname ('RLO_PRDID').asinteger));
                          //main.trclog('1PMtyId='+inttostr(MaCentrale.MTY_ID));
            ...
            //                if RecordCount > 0 then
                            begin
                              ...
                              begin
                                ...
                                //main.trclog('1      '+PChar(Format(' - Réponse "pointure". LceId = %d', [iLceId])));
                              ...
                              begin
                                ...
                                //main.trclog('1      '+PChar(Format(' - Réponse "taille". LceId = %d', [iLceId])));
                                InsertResaSousLigne(iIdResaligne,Que_LOCTYPERELATION.fieldbyname ('LTR_TCAID') .asinteger,iLceId);
                              end;
                              ...
                              begin
                                ...
                                //main.trclog('1      '+PChar(Format(' - Réponse "poids". LceId = %d', [iLceId])));
                                InsertResaSousLigne(iIdResaligne,Que_LOCTYPERELATION.fieldbyname ('LTR_TCAID') .asinteger,iLceId);
                              end;
                              ...
                              begin
                                ...
                                //main.trclog('1      '+PChar(Format(' - Réponse sans valeur. LceId = %d', [iLceId])));
                        ...
                        main.trclog('1    Fin de traitement de l''article -> au suivant');

                        cpt_trc:=640;
                      //Article suivant dans la réservation
                      node3:=node3.nextSibling;
                    end;
                    main.trclog('1 Tous les articles ont été traités');
                    ...
                    main.trclog('1  >> ETAT FINAL DES PAIEMENTS');
                    main.trclog('1  >> - RVS_ACCOMPTE = '+inttostr(Que_Resa.fieldbyname('RVS_ACCOMPTE').asinteger));
                    main.trclog('1  >> - RVS_MONTANTPREV = '+inttostr(Que_Resa.fieldbyname('RVS_MONTANTPREV').asinteger));
                    main.trclog('1  >> - RVS_COMMI = '+inttostr(Que_Resa.fieldbyname('RVS_COMMI').asinteger));


                  end
                  else main.trclog('1Noeud "article" absent -> réservation ignorée!');

                  //----------------------------------------

                end
                else main.trclog('   > Resa ignorée!');

              end
              else main.trclog('1Noeud d''entête "head" absent -> réservation ignorée!');
              cpt_trc:=660;
          ...
          //Absence de réservation pour le couple magasin+date
            ...
            if nroot<>nil
              then main.trclog('1Aucune réservation ce jour. Traitement terminé.')
              else main.trclog('1Noeud xml racine inexistant ("bookings"). Traitement abandonné.');
          end;
          resget.size:=0;
        ...
        //For... magasin suivant
          ...
          main.trclog('1> Aucun magasin à traiter');
        end;
      ...
      cpt_trc:=700;
      main.trclog('1');
      main.trclog('16--Commit des caches');
      TRY
        Dm_Main.StartTransaction();
      ...
      main.trclog('1');
      main.trclog('1Fin traitement des réservations');
      main.trclog('1');
      ...
      begin
        ...
        {$ENDIF}
        main.trclog('XException réservation SkiSet (trace_id='+inttostr(cpt_trc)+'): '+E.ClassName+':'+E.Message);
      end;
    end;
...
begin
  ...
  begin
    ...
    main.trclog('id='+inttostr(id));
    main.trclog('snom='+snom);
    Open;

    main.trclog('Recordcount='+inttostr(recordcount));
    if RecordCount = 0 then
    begin
      Append;
      ...
      Post;
      main.trclog('1>>>>Ajout '+sNom+' id='+inttostr(Id)+' << '+inttostr(FieldByName('lce_id').AsInteger));
    end;
  ...
  *)
  ...
  //main.trclog('1id='+inttostr(id));
  //main.trclog('1snom='+snom);
  //main.trclog('1Recordcount='+inttostr(Que_TmpNoEvent.recordcount));
  if Que_TmpNoEvent.Recordcount = 0 then begin
    With Que_LOCPARAMELT do
    begin
      Close;
...
begin
  ...
  begin
    ...
    (*
    main.trclog('1>>>>> '+sql.text);
    main.trclog('1  OCC_MTYID='+inttostr(OCC_MTYID));
    main.trclog('1  OCC_IDCENTRALE='+inttostr(OCC_IDCENTRALE));
    main.trclog('1  RLO_OPTION='+inttostr(RLO_OPTION));
    *)
...
//      FieldByName('RSL_COMENT').asstring := 'Prix Brut : ' + sResaPrix;
    ...
    case AnsiIndexStr(MaCentrale.MTY_CODE,['RTW','RSK','R2K','RIS','RG1','RG2','RG3','RGS','RS7']) of
      // Twinner, Sport2k, Gen1,Gen2,Gen3
      0,2,4,5,6,7,8 : FieldByName('RSL_PXNET').AsCurrency := cPrix;
      // Skimium
...
begin
  ...
  begin
    ...
    trclog('  >>Identification magasin :');
    //trclog('  '+SQL.TEXT);
    trclog('   PMtId='+inttostr(iMtyid));
    trclog('   PPresta='+'%' + sIdPresta + '%');
    Open;
