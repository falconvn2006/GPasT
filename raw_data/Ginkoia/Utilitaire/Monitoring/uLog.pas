...
const
    {$IFDEF WIN32}
    cKey = 'SOFTWARE\Algol\Ginkoia';
    {$ENDIF}
    {$IFDEF WIN64}
    cKey = 'SOFTWARE\Wow6432Node\Algol\Ginkoia';
    {$ENDIF}
    cField = 'HostEnvironment';
var
    iniFile     : TIniFile ;
    ia          : integer ;
    iNbSrv      : integer ;
...
begin
    ...
    try
        ...
        try
            ...
            if vReg.keyexists(cKey) then
            begin
                vReg.OpenKeyReadOnly(cKey);
                if vReg.ValueExists(cField) then
                    vRegValue := vReg.readinteger(cField);
            end;
        finally
            vReg.Free;
        end;
        ...
        case vRegValue of
            1:
            begin
                vServerName := 'ginkoiamonitoringweuppd';
                vServerHost := 'ginkoiamonitoringweuppd.azurewebsites.net';
                vServerPort := 80
            end;
        else
            vServerName := 'logs';
            vServerHost := 'logs.ginkoia.eu';
            vServerPort := 8080;
        end;

        for ia := 1 to iNbSrv do
        begin
            Servers[ia-1].name := iniFile.ReadString(LOG_SECTION, 'serverName_' + IntToStr(ia), vServerName) ;
            Servers[ia-1].host := iniFile.ReadString(LOG_SECTION, 'serverHost_' + IntToStr(ia), vServerHost) ;
            Servers[ia-1].port := iniFile.ReadInteger(LOG_SECTION, 'serverPort_' + IntToStr(ia), vServerPort) ;
        end;
