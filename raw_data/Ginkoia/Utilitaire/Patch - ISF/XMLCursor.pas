...
VAR
    ...
    OleResult    : HRESULT;
    StringStdXMLTLB  : string;

    VStdXMLTypeLib  : ITypeLib;
BEGIN
{$IFDEF MSWINDOWS}
   ...
   // Check if StdXML.tlb is in the application directory
        ...
        StringStdXMLTLB := 'StdXML.tlb';
        VStdXMLTypeLib  := nil;

        OleResult := LoadTypeLib(PWideChar(WideString(StringStdXMLTLB)), VStdXMLTypeLib);
        OleCheck(OleResult);
    END;
    Result := VStdXMLTypeLib;
{$ENDIF}
END;
