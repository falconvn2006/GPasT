...
constructor TCustomSearching.Create;
begin
    ...
    if FindFirst(Directory + FileExt, faAnyFile, SearchFileRec) = 0 then
      begin
        if (SearchFileRec.Name <> '.') and (SearchFileRec.Name <> '..') and
          ...
          AppendInfoSearch(SearchFileRec, Directory);
...
begin
  ...
  for i:= 0 to High(TabInfoSearch) do
    begin
      if Trim(TabInfoSearch[i].PathAndName) <> '' then
        Result:= Result + TabInfoSearch[i].PathAndName + #13#10;
    end;
end;
...
begin
  if Length(TabInfoSearch) <> 0 then
    begin
      vSL:= TStringList.Create;
      vSearch:= TCustomSearching.Create;
      try
