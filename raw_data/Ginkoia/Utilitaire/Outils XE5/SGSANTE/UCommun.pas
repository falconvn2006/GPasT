...
uses
  ...
  ShellAPi,inifiles,Tlhelp32, WinSvc, Forms,GestionLog;
...
// SELECT * FROM Win32_PerfFormattedData_PerfProc_Process WHERE Name='ibserver' //
// SELECT * FROM Win32_LogicalDisk Where DriveType=3 //
// SELECT Compressed, DeviceID, FileSystem, FreeSpace, LastErrorCode, Size FROM Win32_LogicalDisk Where DriveType=3
        ...
        if (bquery>=1) then
          begin
            rows := MagWmiGetInfoEx ('.', 'root\CIMV2', '','',
                      Format('SELECT KernelModeTime, PeakPageFileUsage, PeakVirtualSize, PeakWorkingSetSize, Priority, ProcessID, ' +
                      'ThreadCount, UserModeTime, VirtualSize, WorkingSetSize, WriteOperationCount  FROM Win32_Process WHERE Caption=''%s''',[aExeName]), WmiResults, instances, errstr) ;
            if rows > 0 then
            begin
            ...
            if (bquery=2) and (result.PID<>'') then
              begin   // SELECT HandleCount, Name, PageFileBytesPeak, PercentProcessorTime, WorkingSet, WorkingSetPeak, WorkingSetPrivate FROM  Win32_PerfFormattedData_PerfProc_Process
                rows := MagWmiGetInfoEx ('.', 'root\CIMV2', '','',
                          Format('SELECT ElapsedTime, HandleCount, Name, PageFileBytesPeak, PercentProcessorTime, WorkingSet, WorkingSetPeak %s FROM  Win32_PerfFormattedData_PerfProc_Process WHERE IDProcess=%s',[wsp,result.PID]), WmiResults, instances, errstr) ;
                if rows > 0 then
                  begin
...
begin
    ...
    try
        rows := MagWmiGetInfoEx ('.', 'root\CIMV2', '','',
              'SELECT Compressed, DeviceID, FileSystem, FreeSpace, LastErrorCode, Size FROM Win32_LogicalDisk Where DriveType=3', WmiResults, instances, errstr) ;
        if (rows>0) then
          begin
...
begin
    ...
    try
       rows := MagWmiGetInfoEx ('.', 'root\CIMV2', '','',
                  'SELECT BuildNumber, Caption, CSDVersion, CSName, FreePhysicalMemory, FreeSpaceInPagingFiles, FreeVirtualMemory, ' +
                  ' LastBootUptime, LocalDateTime, LastBootUptime, LocalDateTime , Version ' +
                  ' FROM Win32_OperatingSystem', WmiResults, instances, errstr) ;
        if rows > 0 then
        begin
