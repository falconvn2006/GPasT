//$Log:
// 2    Utilitaires1.1         27/06/2018 12:22:52    Python Benoit  
//      Utilisation de la constante pour le nom de fichier
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            Deletefile(Ginkoia + 'LiveUpdate\' + Nom_Outil_Maj + '.EXE');
            ...
            FileClose(Handle);
            Handle := FileCreate(Ginkoia + 'LiveUpdate\' + Nom_Outil_Maj + '.ZIP');
            FileWrite(Handle, P[0], FSize - Taille);
            FileClose(Handle);
            FreeMem(p, FSize - Taille + 10);
            UnZip.ZipName := Ginkoia + 'LiveUpdate\' + Nom_Outil_Maj + '.ZIP';
            UnZip.DestDir := Ginkoia + 'LiveUpdate\';
            Unzip.DoAll := true;
            UnZip.UnZip;
        END;

        IF NOT fileexists(Ginkoia + 'LiveUpdate\' + Nom_Outil_Maj + '.GIN') THEN
        BEGIN
        ...
        BEGIN
            IF NOT (fileexists(Ginkoia + 'LiveUpdate\' + Nom_Outil_Maj + '.EXE')) THEN
            BEGIN
        ...
        for i := 1 to paramcount do
           S := S+' '+paramstr(i) ;
        Winexec(Pchar(Ginkoia + 'LiveUpdate\' + Nom_Outil_Maj + '.EXE'+S), 0);
        Close;
    END;
END;
