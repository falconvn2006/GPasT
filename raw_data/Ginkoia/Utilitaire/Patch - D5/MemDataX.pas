//$Log:
// 3    Utilitaires1.2         06/06/2019 17:58:51    St?phane MATHIS
...
//     application.messagebox(pchar(s),'param',mb_ok) ;
   ...
   //application.messagebox(pchar(LaPage + 'Query=' + S),'REQUETE',mb_ok) ;
   //application.ProcessMessages;

   try
      ms := TMemoryStream.create;
      tIDHTTP1 := TIdHTTP.create(nil);
      tIDHTTP1.Get(LaPage + 'Query=' + S, ms);
      freeandnil(tIDHTTP1);
      ms.Position := 0;
      //ms.SaveToFile('c:\test2.txt');
      SetString(content, pchar(ms.Memory), ms.Size div sizeof(char));
      ms.free;
   ...
   begin
      ...
      Http.SaveToFile(LaPage + 'Query=' + S, 'c:\Test.txt');
   end
   else
   begin
      result.add('ERREUR connexion au serveur');
      //application.messagebox(pchar('ERREUR connexion au serveur'),'ERROR',mb_ok) ;
   END;
   //application.ProcessMessages;
   if ok then
   begin
...
begin
   ...
   Close;
   ListeAFaire.clear;
   //application.messagebox(pchar('Avant : LanceRequete'),'OK',mb_ok);

   tsl := LanceRequete;
   //application.messagebox(pchar('Après : LanceRequete, nb lignes = ' + inttostr(tsl.count)),'OK',mb_ok);
   //application.ProcessMessages;
   ...
   begin
      DisableControls;
      try
         //application.messagebox(pchar('Avant : remplitLesChamps'),'OK',mb_ok);
         //application.ProcessMessages;
         remplitLesChamps(tsl);
         //application.messagebox(pchar('Après : remplitLesChamps'),'OK',mb_ok);
         //application.ProcessMessages;
         inherited Open;
         ...
         begin
            ...
            if j = 0 then Append;
            //application.messagebox(pchar('test1 ('+inttostr(i)+','+inttostr(j)+','+leschamps[j]+')'),'OK',mb_ok);
        ...
        on E : exception do
        begin
          application.messagebox(pchar(E.message),'OK',mb_ok);
        END;
      end;
      First;
      EnableControls;
