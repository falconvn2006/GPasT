...
function GetInfosBase(Server, FileName, UserName, Password : string; Port : integer; out Version, Nom, Centrale, GUID, Sender : string; out DateVersion : TDateTime; out Generateur : integer; out Recalcul : boolean; out error :string) : boolean;
    ...
    try
      ...
      begin
        try
          ...
          // recup du nom
          try
            Query.SQL.Text := 'select bas_nompournous, max(bas_centrale) as bas_centrale, count(*) as nb from genbases join k on k_id = bas_id and k_enabled = 1 where bas_id != 0 group by bas_nompournous order by 3 desc rows 1;';
            Query.Open();
            if Query.Eof then
              Exit
            else
            begin
              ...
              Centrale := Trim(Query.FieldByName('bas_centrale').AsString);
            end;
          finally
            Query.Close();
          end;
