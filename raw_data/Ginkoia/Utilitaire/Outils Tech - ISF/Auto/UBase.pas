...
uses
   ...
   StdCtrls, ComCtrls, Db, IBCustomDataSet, IBQuery, IBSQL, IBDatabase;
...
begin
  ...
  begin
     ...
     begin
        ...
        ForceDirectories('C:\GINKOIA\A_MAJ\');
        Crc32 := TCrc32.create;
        ...
        begin
           ...
           S := 'C:\Ginkoia\' + tc.UneValeur(Tc_Fic, 'fichier', i_fic);
           CRC := IntToStr(crc32.FileCRC32(S));
           if CRC <> tc.UneValeur(Tc_Fic, 'crc', i_fic) then
           begin
              if FileExists('C:\GINKOIA\A_MAJ\' + tc.UneValeur(Tc_Fic, 'fichier', i_fic)) then
                 CRC := IntToStr(crc32.FileCRC32('C:\GINKOIA\A_MAJ\' + tc.UneValeur(Tc_Fic, 'fichier', i_fic)));
           end;
           if CRC <> tc.UneValeur(Tc_Fic, 'crc', i_fic) then
           begin
              _Message := 'récupération de ' + tc.UneValeur(Tc_Fic, 'fichier', i_fic);
              Fich := 'C:\GINKOIA\A_MAJ\' + tc.UneValeur(Tc_Fic, 'fichier', i_fic);
              repertoire := IncludeTrailingBackslash(extractfilepath(Fich));
              ForceDirectories(repertoire);
              Nomrecup := internet.traitechaine('http://lame2.no-ip.com/maj/' + NomVer + '/' + tc.UneValeur(Tc_Fic, 'fichier', i_fic));
              ...
              begin
                 ...
                 repertoire := IncludeTrailingBackslash(extractfilepath('C:\GINKOIA\' + tc.UneValeur(Tc_Fic, 'fichier', i_fic)));
                 ForceDirectories(repertoire);
                 if CopyFile(Pchar(fich), Pchar('C:\GINKOIA\' + tc.UneValeur(Tc_Fic, 'fichier', i_fic)), false) then
                    DeleteFile(fich);
              end
              else
              begin
  ...
  begin
    ...
    // vérification de la version du client
        ...
        begin
           ...
           begin
              ForceDirectories('C:\GINKOIA\A_Patch\');
              ...
              begin
                 ...
                 fichier := internet.traitechaine('http://lame2.no-ip.com/maj/patch/' + NomVer + '/script' + Inttostr(i) + '.SQL');
                 tsl.text := Internet.DownloadStr(fichier);
                 tsl.SaveToFile('C:\GINKOIA\A_Patch\script' + Inttostr(i) + '.SQL');
                 S := tsl.Text;
                 while s <> '' do
                 begin
                    if pos('<---->', S) > 0 then
                 ...
                 tc.ordre('Insert into histo (id_cli, ladate, action, actionstr, str1, str2, str3) ' +
                    'values (' + tc.UneValeur(tc_c, 'id') + ', "' + tc.DateTime(Now) + '", ' +
                    '4, "script' + Inttostr(i) + '.SQL","c:\ginkoia\data\ginkoia.ib","","")');
           ...
           begin
              ...
              begin
                 ...
                 fichier := internet.traitechaine('http://lame2.no-ip.com/maj/patch/' + GUID + '/script' + Inttostr(i) + '.SQL');
                 tsl.text := Internet.DownloadStr(fichier);
                 tsl.SaveToFile('C:\GINKOIA\A_Patch\Spe' + Inttostr(i) + '.SQL');
                 S := tsl.Text;
                 while s <> '' do
                 begin
                    if pos('<---->', S) > 0 then
                 ...
                 tc.ordre('Insert into histo (id_cli, ladate, action, actionstr, str1, str2, str3) ' +
                    'values (' + tc.UneValeur(tc_c, 'id') + ', "' + tc.DateTime(Now) + '", ' +
                    '7, "script' + Inttostr(i) + '.SQL","C:\Ginkoia\Data\Ginkoia.IB","","")');
