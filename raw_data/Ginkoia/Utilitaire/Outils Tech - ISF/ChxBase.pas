...
begin
  ...
  if FileExists(sListFilePath) then
...
begin
   ...
   begin
      ...
      // Si on a demandé \\SYMREPLIC2\EAI\INTERSPORT\MOY\DATA\
      // Tsl contient par exemple :
      // \\SYMREPLIC2\EAI\INTERSPORT\MOY\DATA\;DIR;
      // \\SYMREPLIC2\EAI\INTERSPORT\MOY\DATA\Backup\;DIR;Backup
      // \\SYMREPLIC2\EAI\INTERSPORT\MOY\DATA\GINKOIA.IB\;FIC;GINKOIA.IB
      ...
      begin
         ...
         // Retire la fin du chemin jusqu'a obtenir le dossier parent.
      ...
      begin
         s := tsl[i];
         // Exemple : \\SYMREPLIC2\EAI\INTERSPORT\MOY\DATA\GINKOIA.IB\;FIC;GINKOIA.IB


         reel := copy(s, 1, pos(';', s) - 1);
         delete(s, 1, pos(';', s));
         // reel contient : \\SYMREPLIC2\EAI\INTERSPORT\MOY\DATA\GINKOIA.IB\
         // s contient : FIC;GINKOIA.IB

         quoi := copy(s, 1, pos(';', s) - 1);
         delete(s, 1, pos(';', s));
         // quoi contient : FIC
         // s contient : GINKOIA.IB

         fichier := s;
         // fichier contient : GINKOIA.IB
...
begin
  ...
  begin
    ...
    base := copy(base,1,length(base)-1) ; // On delete le \ de fin
    Modalresult := Mrok ;
  END ;
end;
