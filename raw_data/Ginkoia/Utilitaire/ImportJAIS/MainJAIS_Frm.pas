  ...
  begin
    ...
    begin
      ...
      try
        ...
        With GenParam do
        begin
          PRM_TYPE   := CGENPTYPE; // 12
          PRM_MAGID  := 0;
          PRM_POS    := 0;
          PRM_FLOAT  := 0;
          PRM_CODE   := CGENPCODEEXC; // 4
...
begin
  ...
  try
    ...
    Chk_CONVERT.Visible := ReadBool('CONVERT','DOCONVERT',False);
  finally
    free;
  end;
...
begin
  ...
  try
    EnableObj(False);
    BINPROGRESS := True;
    BDOCONVERT := Chk_CONVERT.Checked; // Si Checked Autorise la convertion des fichiers Fic en XML
    ...
    begin
      ...
      begin
        ...
        if BCONNECTED or OpenDataBase(GFILEDEST) then
        begin
          // 2- Récupération de la liste des tables à traiter
          AddToMemo(mmLogs,'2- Récupération de la liste des tables à traiter');

          // Récupération du code JA encours
          TableJA := GetTableList(GPATHDATA + 'TableJA.txt');
          TableJA.Lab := Lab_Progress;
          TableJA.Memo := mmLogs;
          TableJA.Progress := ProgressBar1;
          TableJA.lstCommande := Lbx_cmd;

          ConvertTbHF(TableJA);
          CheckFieldList(TableJA);
          TbJa := TableJA.GetTable('JA');
          TbJa.CDataSet.Last;
          GJADIR := Trim(TbJa.NFieldByName('JACODE').AsString);

          // Récupération des autres tables
          TableList := GetTableList(GPATHDATA + 'tablelist.txt');
