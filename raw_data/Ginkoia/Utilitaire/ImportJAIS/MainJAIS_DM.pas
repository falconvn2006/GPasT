...
type
  TDM_MainJAIS = class(TDataModule)
    ...
    VCLZip1: TVCLZip;
    ibs_ArtChrono: TIBOStoredProc;
    ibs_cmdChrono: TIBOStoredProc;
    ibs_NewK: TIBOStoredProc;
  private
...
//      TbJA.CDataSet.Last; // Positionnement sur le dernier enregistrement (Il semble que parfois il y ait tous les JA dans la table mais il sont dans l'ordre)
      sFileName := GPATHARCHIVEZIP + GJADIR + FormatDateTime('_YYYYMMDDhhmmssnn',Now);
  ...
  try
    ...
    begin
      ...
      Try
        ...
        XMLDoc.LoadFromFile(GPATHFILETMP + Copy(ListTbHF.Items[i].TableName,iPos,Length(ListTbHF.Items[i].TableName)) + '.xml');
        XMLDoc.Active := True;
      Except on E:Exception do
        begin
...
//    raise Exception.Create('Table CMDMAGCAD inexistante');
  ...
  Try
    ListTbHF.Lab.Caption := 'Génération des articles (0 / ' + IntToStr(CDataSet.RecordCount) + ')';
    CDataSet.First;
    iOldArt := 0;
    i := 1;
...
//      sTotal := sTotal + 'GRE : ' + IntToStr(itmp) + ' - ';
      ...
      begin
        ...
        if IdArticle = 0 then
        begin
          if Trim(TbArticleMOD.NFieldByName('CTMNOMMOD').AsString) = '' then
            ListTbHF.Memo.Lines.Add(NFieldByName('CDEMODPCES').AsString);
...
//      sTotal := sTotal + 'PX : ' + IntToStr(itmp) + ' - ';
      ...
      CDataSet.Next;
      ListTbHF.Lab.Caption := 'Génération des articles (' + IntToStr(i) + ' / ' + IntToStr(CDataSet.RecordCount) + ')';
...
//      AddToMemo(ListTbHF.Memo,'Total Ligne : ' + IntToStr(iTotal));
 ...
 // récupération des données des commandes
  ...
  Try
    ...
    // 2limination des code adhérant non existant dans la base de données
    TbCDEMAG.CDataSet.First;
    while not TbCDEMAG.CDataSet.EOF do
    begin
      MAG_ID := GetMagasinId(TbCDEMAG.NFieldByName('CMGSOCCODE').AsString);
      if MAG_ID <> -1 then
        TbCDEMAG.CDataSet.Next
      else begin
          AddToMemo(ListTbHF.Memo,'Code adhérant non traité : ' + TbCDEMAG.NFieldByName('CMGSOCCODE').AsString + #13#10 +
            'Commande : ' + TbCDEMAG.NFieldByName('CMGIDCDE').AsString);
         TbCDEMAG.CDataSet.Delete;
      end;
    end;

    ListTbHF.Lab.Caption := 'Initialisation des commandes';
    while not CDataSet.Eof do
    begin
...
//        if MAG_ID = -1 then
//        begin
//          AddToMemo(ListTbHF.Memo,'Code adhérant non traité : ' + TbCDEMAG.NFieldByName('CMGSOCCODE').AsString
//          + #13#10 + 'Commande : ' + TbCDE.NFieldByName('CDELIB').AsString);
//          while (MAG_ID = -1) and TbCDEMAG.NLocate('CMGIDCDE',NFieldByName('CDEID').AsString,[loCaseInsensitive]) do
//          begin
//            TbCDEMAG.CDataSet.Delete;
//            MAG_ID := GetMagasinId(TbCDEMAG.NFieldByName('CMGSOCCODE').AsString);
//          end;
//        end;
  ...
  //                  if TbCDEMAG.NFieldByName('CMGMAGIDCDE').AsString = NFieldByName('CDEID').AsString then
                   // begin
  ...
  //                    if TbCDEAMC.NLocate('AMCCDEID;AMCMAGCDEID',VarArrayof([NFieldByName('CDEID').AsString,TbCDEMAGCAD.NFieldByName('CMCMAGADIDCDE').AsString]),[loCaseInsensitive]) then
                      if TbCDEAMC.NLocate('AMCCDEID;AMCMAGCOD',VarArrayOf([NFieldByName('CDEID').AsString,TbCDEMAG.NFieldByName('CMGCODEMAG').AsString]),[loCaseInsensitive]) then
                      begin
                        while not TbCDEAMC.CDataSet.Eof do
                        begin
                          if (TbCDEAMC.NFieldByName('AMCCDEID').AsString = NFieldByName('CDEID').AsString)
                              and (TbCDEAMC.NFieldByName('AMCMAGCOD').AsString = TbCDEMAG.NFieldByName('CMGCODEMAG').AsString)  then
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    begin
                                      ...
                                      CDL_LIVRAISON := XmlStrToDate(TbCDEAMC.NFieldByName('AMCDTECAD').AsString);
  ...
  //            end; // if TbCDEMAGCAD
      ...
      ListTbHF.Progress.Position := CDataSet.RecNo * 100 Div CDataSet.RecordCount;
      Application.ProcessMessages;
      CDataSet.Next;
    end; // while
    ...
    begin
      ...
      begin
        ...
        AddToMemo(ListTbHF.Memo,'Chrono Commande : ' + sChronoCMD + ' / ' + ListCMD[i].CDE_COMENT);
      end; // if IMP_ID = 0
...
begin
  ...
  try
    ...
    SQL.Add('Where Upper(ART_NOM) = :PARTNOM');
...
begin
  ...
  try
    ...
    begin
      ...
      ParamByName('PARFCHRONO').AsString     := sChrono;
      ...
      AddToMemo(TableList.Memo,'Nouvel Article : ' + sChrono);
    end else
...
begin
  ...
  begin
    Close;
    ParamCheck := True;
    ParamByName('TABLENAME').AsString := UpperCase(sTableName);
    Open;
...
begin
  ...
  begin
    ...
    SQL.Add('  Join UILGRPGINKOIAMAG on UGM_UGGID = UGG_ID');
  ...
  // TVA 1,12
  Result.TVA_ID := GetGenParam(CGENPCODETVA,CGENPTYPE).PRM_INTEGER;

  // Garantie 2,12
  Result.GAR_ID := GetGenParam(CGENPCODEGAR,CGENPTYPE).PRM_INTEGER;

  // type comptable 3,12
  Result.TCT_ID := GetGenParam(CGENPCODETCP,CGENPTYPE).PRM_INTEGER;

  // fournisseur intersport 5,12
  Result.FOU_ID := GetGenParam(CGENPCODEFOU,CGENPTYPE).PRM_INTEGER;

  // collection obligatoire 7,12
  Result.Collection := (GetGenParam(CGENPCODECOL,CGENPTYPE).PRM_INTEGER = 1);
end;
...
begin
  ...
  // Verrou 6,12
  GenParam := GetGenParam(CGENPCODEVER,CGENPTYPE);
...
begin
      ...
      begin
        ...
        while not EOF do
        begin
          // Exercice commercial 4,12
          GenParam := GetGenParamByMAGID(CGENPCODEEXC,CGENPTYPE,FieldByName('SOC_ID').AsInteger);
          Edit;
      ...
      begin
        ...
        begin
          // Acheteur 8,12
          GenParam := GetGenParamByMAGID(CGENPCODEACH,CGENPTYPE,FieldByName('MAG_ID').AsInteger);
          cdsMagasin.Edit;
