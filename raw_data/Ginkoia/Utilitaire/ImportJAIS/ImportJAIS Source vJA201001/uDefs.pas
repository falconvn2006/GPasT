...
var
  ...
  function GetTableList : TListTableHF;
...
begin
  ...
  try
    try
      lstTable.LoadFromFile(GPATHDATA + 'tablelist.txt');
...
begin
  ...
  sDirDestination := GPATHARCHIVEJA + TbJA.NFieldByName('JACODE').AsString + '\';
    ...
    try
      ...
      begin
        ...
        begin
          ...
          With ShellLink do
          begin
            SetDescription(PChar('Raccourcis Vers JA ' + TbJA.NFieldByName('JACODE').AsString)); //Description du raccourcis (visible dans les propriétés du raccourcis)
            ...
            (ShellLink as IPersistFile).Save(StringToOleStr(sDirTmp +'JA ' + TbJA.NFieldByName('JACODE').AsString + '.lnk'), true); //creation du raccourcis sur le bureau
          end;
        end;
      end;
    finally
