...
TYPE
    ...
    PUBLIC
        ...
        PROCEDURE scriptGinkoia(scr: STRING);
...
BEGIN
    ...
    BEGIN
        TRY
            WITH StdConst DO
            BEGIN
                Database.DatabaseName := UpperCase(iniCtrl.readString('DATABASE', 'PATH', ''));
                StdConst.PathBase := Database.DatabaseName;
                Database.SQLDialect :=
                    iniCtrl.readInteger('DATABASE', 'DIALECTSQL', 3);
            END;
    ...
    BEGIN
        ...
        Application.Terminate;
    END;
    StdConst.PathRapport := ChemWin(ExtractFilePath(StdConst.PathBase)) + 'Rapport\';
END;
...
BEGIN
    ...
    script := 'SET generator system_id to ' + result;
    scriptGinkoia(script);
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            ELSE
                flag := true;
            script := script + IbQ_TableField.FieldByName('KFLD_NAME').AsString;
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        script := script + IbQ_TableField.FieldByName('KFLD_NAME').AsString + ' = ' +
                            inttostr(DataSet.FieldByName(IbQ_TableField.FieldByName('KFLD_NAME'
                            ).AsString).AsInteger);
                    ...
                    BEGIN
                        ...
                        script := script + IbQ_TableField.FieldByName('KFLD_NAME').AsString + ' = ' + #39 +
                            DataSet.FieldByName(IbQ_TableField.FieldByName('KFLD_NAME'
                            ).AsString).AsString + #39;
                    ...
                    BEGIN
                        ...
                        script := script + IbQ_TableField.FieldByName('KFLD_NAME').AsString + ' = ' + #39 +
                            floattostr(DataSet.FieldByName(IbQ_TableField.FieldByName('KFLD_NAME'
                            ).AsString).AsFloat) + #39;
                    END;
                END;
                IbQ_TableField.Next;
...
BEGIN
    script := 'INSERT INTO K (K_ID,  KRH_ID,  KTB_ID,  K_VERSION,  K_ENABLED,  KSE_OWNER_ID,  KSE_INSERT_ID,' +
        'KSE_UPDATE_ID,  KSE_DELETE_ID,  K_INSERTED,  K_UPDATED,  K_DELETED,  KSE_LOCK_ID,  KMA_LOCK_ID)'
        + 'VALUES (';
    ...
    IbC_InsertK.ParamByName('KRH_ID').AsString := KRH_ID;
    script := script + krh_id + ',';
    IbC_InsertK.ParamByName('KTB_ID').AsString := KTB_ID;
    script := script + ktb_id + ',';

    IF FVersionSys = 0 THEN
    BEGIN
        IbC_InsertK.ParamByName('K_VERSION').AsString := GenVersion;
        script := script + GenVersion + ',';
    END
    ELSE
    BEGIN
        IbC_InsertK.ParamByName('K_VERSION').AsInteger := FVersionSys;
        script := script + inttostr(FVersionSys) + ',';
    END;
    script := script + '1,';
    IbC_InsertK.ParamByName('KSE_OWNER_ID').AsString := KSE_Owner;
    script := script + KSE_Owner + ',';
    IbC_InsertK.ParamByName('KSE_INSERT_ID').AsString := KSE_Insert;
    script := script + KSE_Insert + ',';
    IbC_InsertK.ParamByName('KSE_UPDATE_ID').AsString := '0';
    script := script + '0,';
    IbC_InsertK.ParamByName('KSE_DELETE_ID').AsString := '0';
    script := script + '0,';
    IbC_InsertK.ParamByName('K_INSERTED').AsString := S1;
    script := script + #39 + Dtscript + #39 + ',';
    IbC_InsertK.ParamByName('K_UPDATED').AsString := S1;
    script := script + #39 + Dtscript + #39 + ',';
    IbC_InsertK.ParamByName('K_DELETED').AsString := S2;
    script := script + #39 + '12/30/1899' + #39 + ',';
    IbC_InsertK.ParamByName('KSE_LOCK_ID').AsString := '0';
    script := script + '0,';
    IbC_InsertK.ParamByName('KMA_LOCK_ID').AsString := '0';
    script := script + '0);';
...
BEGIN
    ...
    script := script + ' K_ENABLED = 0, KSE_DELETE_ID = ' + KSE_DELETE + ', K_DELETED = current_date '+
        'WHERE K.K_ID = ' + K_ID ;
...
BEGIN
    ...
    Ibc_IntegrityChk.Open;
    Ibc_IntegrityChk.SQL.SaveToFile('c:\integr.txt');
    Result := Ibc_IntegrityChk.Eof;
...
PROCEDURE TDm_Main.scriptGinkoia(scr: STRING);
VAR
    f: textfile;
    ind: integer;
    T: Tstrings;
BEGIN
    IF FileExists('c:\developpement\ginkoia\Modifsbase.txt') then
       AssignFile(F, 'c:\developpement\ginkoia\Modifsbase.txt')
    else
    begin
         T := Tstrings.Create;
         T.add('Creation du fichier');
         T.SaveToFile( ExtractFilePath(StdConst.PathBase)+'Modifsbase.txt');
         T.Free;
         AssignFile(F, ExtractFilePath(StdConst.PathBase)+'Modifsbase.txt')
    END;
    Append(f);

    IF insertscript = 0 THEN
    BEGIN //premièr insert dans script donc petit commentaire
        writeln(f, ' ');
        writeln(f, '/* ------------------------Début Modifs du ' + datetostr(date) + '-------------------------- */');
        writeln(f, ' ');
        insertscript := 1;
    END;

    IF scr = 'F' THEN
    BEGIN //Balise de fin de modif
        writeln(f, ' ');
        writeln(f, '/* ------------------------Fin Modifs du ' + datetostr(date) + '-------------------------- */');
        writeln(f, ' ');
    END
    ELSE
    BEGIN
        ind := strtoint(release) + 1;
        release := Conv(ind, 4);
        Writeln(f, '<RELEASE>' + version + '.' + release);
        Writeln(f, scr);
    END;
    Writeln(f, ' ');
