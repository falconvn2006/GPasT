...
uses
  ...
  Vcl.Forms,Vcl.Dialogs,Vcl.Graphics, Classes, SciSupport;
...

{
  ...
  private
   ...
   // notifiedIDs: TStringList;
    toolbarsetup: boolean;
...
type TtoolbarIconsWithDarkMode = record
	hToolbarBmp: HBITMAP;
	hToolbarIcon: HICON;
...
begin
  inherited;
  self.toolbarsetup:=false;
  //notifiedIDs:=TStringList.Create;
end;
...
begin
  Result := nppPChar(self.PluginName);
    ...
    {
function TNppPlugin.isUnicode: boolean;
begin
  Result := true;
end;  }
...
begin
 ...
 // notifiedIDs.Add(IntToStr(sn^.nmhdr.code));

    //Nasty hack: Only ever seem to receive the NPPN_TB_MODIFICATION notification when debugging with npp process.
    //As 1002 is always first to be fired anyway just make sure to call it ASAP regardless of message.
    if(self.toolbarsetup = false) then begin
      if (HWND(sn^.nmhdr.hwndFrom) = self.NppData.NppHandle) then// and ((sn^.nmhdr.code = NPPN_TB_MODIFICATION) or (sn^.nmhdr.code = NPPN_BUFFERACTIVATED)) then
      begin
        self.DoNppnToolbarModification;
        self.toolbarsetup:=true;
      end;
    end;
  //end;
...
begin
  ...
  SetLength(S,Size + 1);
  try
    Sci_Send(SCI_GETSELTEXT, 0, LPARAM(PWideChar(S)));
    SetLength(S,Size);
    Result := WideString(S);
  finally
    SetLength(S,0);
...
function TNppPlugin.GetSelStart: Integer;
begin
    Result := Sci_Send(SCI_GETSELECTIONSTART, 0, 0);
end;

function TNppPlugin.GetSelEnd: Integer;
begin
    Result := Sci_Send(SCI_GETSELECTIONEND, 0, 0);
end;

function TNppPlugin.GetSelLength: Integer;
begin
    Result := GetSelEnd - GetSelStart;
end;
