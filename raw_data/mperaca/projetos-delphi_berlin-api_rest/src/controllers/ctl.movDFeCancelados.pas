...
uses models.movOrcamentos, prv.dataModuleConexao, dat.movDFeCancelados;


procedure ListaTodosCancelados(Req: THorseRequest; Res: THorseResponse; Next: TProc);
...
begin
  try
    ...
    wlista := dat.movDFeCancelados.RetornaListaCancelados(Req.Query,widempresa,wlimit,woffset);
...
procedure RetornaCancelado(Req: THorseRequest; Res: THorseResponse; Next: TProc);
var wcancelado,werro: TJSONObject;
    wid: integer;
    wval: string;
begin
  try
    wid         := Req.Params['id'].ToInteger; // recupera o id do orçamento
    wcancelado  := TJSONObject.Create;
    wcancelado  := Req.Body<TJSONObject>;
    wcancelado  := dat.movDFeCancelados.RetornaCancelado(wid);
//    if wconsultado.TryGetValue('status',wval) then
    wcancelado.TryGetValue('status',wval);
    if wval='500' then
       Res.Send<TJSONObject>(wcancelado).Status(400)
    else
       Res.Send<TJSONObject>(wcancelado).Status(200);
  except
    On E: Exception do
    begin
      werro := TJSONObject.Create;
...
procedure RetornaTotalCancelados(Req: THorseRequest; Res: THorseResponse; Next: TProc);
var wtotal,werro: TJSONObject;
    wval: string;
begin
  try
    wtotal := TJSONObject.Create;
    wtotal := dat.movDFeCancelados.RetornaTotalCancelados(Req.Query);
...
begin
// Método Get
  THorse.Get('/trabinapi/movimentos/dfe/cancelados',ListaTodosCancelados);
  THorse.Get('/trabinapi/movimentos/dfe/cancelados/total',RetornaTotalCancelados);
  THorse.Get('/trabinapi/movimentos/dfe/cancelados/:id',RetornaCancelado);
