...
uses models.movOrcamentos, prv.dataModuleConexao, dat.movDFeCiencias;


procedure ListaTodasCiencias(Req: THorseRequest; Res: THorseResponse; Next: TProc);
...
begin
  try
    ...
    wlista := dat.movDFeCiencias.RetornaListaCiencias(Req.Query,widempresa,wlimit,woffset);
...
procedure RetornaCiencia(Req: THorseRequest; Res: THorseResponse; Next: TProc);
var wciencia,werro: TJSONObject;
    wid: integer;
    wval: string;
begin
  try
    wid      := Req.Params['id'].ToInteger; // recupera o id do orçamento
    wciencia := TJSONObject.Create;
    wciencia := Req.Body<TJSONObject>;
    wciencia := dat.movDFeCiencias.RetornaCiencia(wid);
//    if wconsultado.TryGetValue('status',wval) then
    wciencia.TryGetValue('status',wval);
    if wval='500' then
       Res.Send<TJSONObject>(wciencia).Status(400)
    else
       Res.Send<TJSONObject>(wciencia).Status(200);
  except
    On E: Exception do
    begin
      werro := TJSONObject.Create;
...
procedure RetornaTotalCiencias(Req: THorseRequest; Res: THorseResponse; Next: TProc);
var wtotal,werro: TJSONObject;
    wval: string;
begin
  try
    wtotal := TJSONObject.Create;
    wtotal := dat.movDFeCiencias.RetornaTotalCiencias(Req.Query);
...
begin
// Método Get
  THorse.Get('/trabinapi/movimentos/dfe/ciencias',ListaTodasCiencias);
  THorse.Get('/trabinapi/movimentos/dfe/ciencias/total',RetornaTotalCiencias);
  THorse.Get('/trabinapi/movimentos/dfe/ciencias/:id',RetornaCiencia);
