...
uses prv.dataModuleConexao, dat.cadValidacoes;

procedure ListaTodasValidacoes(Req: THorseRequest; Res: THorseResponse; Next: TProc);
...
begin
  try
    ...
    wlista := dat.cadValidacoes.RetornaListaValidacoes(Req.Query,wlimit,woffset);
...
procedure RetornaValidacao(Req: THorseRequest; Res: THorseResponse; Next: TProc);
var wvalidacao,werro: TJSONObject;
    wid: integer;
    wval: string;
begin
  try
    wid       := Req.Params['id'].ToInteger; // recupera o id do cliente
    wvalidacao := TJSONObject.Create;
    wvalidacao := dat.cadValidacoes.RetornaValidacao(wid);
    if wvalidacao.TryGetValue('status',wval) then
       Res.Send<TJSONObject>(wvalidacao).Status(400)
    else
       Res.Send<TJSONObject>(wvalidacao).Status(200);
  except
    On E: Exception do
    begin
      werro := TJSONObject.Create;
...
procedure RetornaTotalValidacoes(Req: THorseRequest; Res: THorseResponse; Next: TProc);
var wtotal,werro: TJSONObject;
    wval: string;
begin
  try
    wtotal := TJSONObject.Create;
    wtotal := dat.cadValidacoes.RetornaTotalValidacoes(Req.Query);
...
begin
// Método Get
  THorse.Get('/trabinapi/cadastros/validacoes',ListaTodasValidacoes);
  THorse.Get('/trabinapi/cadastros/validacoes/:id',RetornaValidacao);
  THorse.Get('/trabinapi/cadastros/validacoes/total',RetornaTotalValidacoes);
