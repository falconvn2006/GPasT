...
uses prv.dataModuleConexao, dat.cadPessoasRevendas;
...
begin
  try
    ...
    wlista := dat.cadPessoasRevendas.RetornaListaRevendas(Req.Query,widpessoa);
...
begin
  try
    ...
    else if dat.cadPessoasRevendas.VerificaRequisicao(wRevenda) then
       begin
         wnewRevenda := dat.cadPessoasRevendas.IncluiRevenda(wRevenda,widpessoa);
...
begin
  try
    ...
    wRevenda    := Req.Body<TJSONObject>;
    wnewRevenda := dat.cadPessoasRevendas.AlteraRevenda(wid,wRevenda);
    if wnewRevenda.TryGetValue('idrevendapessoa',wval) then
       begin
  ...
  try
    ...
    if wid>0 then
       begin
         ...
         wret := dat.cadPessoasRevendas.ApagaRevenda(wid);
...
begin
  try
    ...
    wRevenda := TJSONObject.Create;
    wRevenda := Req.Body<TJSONObject>;
    wRevenda := dat.cadPessoasRevendas.RetornaRevenda(wid);
...
begin
// Método Get
  THorse.Get('/trabinapi/cadastros/pessoas/:idpessoa/revendas',ListaTodasRevendas);
  THorse.Get('/trabinapi/cadastros/pessoas/:idpessoa/revendas/:id',RetornaRevenda);

// Método Post
  THorse.Post('/trabinapi/cadastros/pessoas/:idpessoa/revendas',CriaRevenda);

  // Método Put
  THorse.Put('/trabinapi/cadastros/pessoas/:idpessoa/revendas/:id',AlteraRevenda);

// Método Delete
  THorse.Delete('/trabinapi/cadastros/pessoas/:idpessoa/revendas/:id',ExcluiRevenda);
end;
