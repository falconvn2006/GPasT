...
uses prv.dataModuleConexao, dat.cadTabelasDescontos;
...
begin
  try
    ...
    wlista := dat.cadTabelasDescontos.RetornaListaTabelas(Req.Query);
...
begin
  try
    ...
    else if dat.cadTabelasDescontos.VerificaRequisicao(wTabela) then
       begin
         wnewTabela := dat.cadTabelasDescontos.IncluiTabela(wTabela);
...
begin
  try
    ...
    wTabela    := Req.Body<TJSONObject>;
    wnewTabela := dat.cadTabelasDescontos.AlteraTabela(wid,wTabela);
    if wnewTabela.TryGetValue('id',wval) then
    ...
    else if (wnewTabela.TryGetValue('status',wval)) and (wval='500') then
       begin
         wnewTabela.TryGetValue('description',wdescerro);
       ...
       begin
         ...
         wresp.AddPair('description','Tabela Desconto não encontrada');
         wresp.AddPair('datetime',formatdatetime('yyyy-mm-dd hh:nn:ss',now));
  ...
  try
    ...
    if wid>0 then
       begin
         ...
         wret := dat.cadTabelasDescontos.ApagaTabela(wid);
...
begin
  try
    ...
    wTabela := TJSONObject.Create;
    wTabela := Req.Body<TJSONObject>;
    wTabela := dat.cadTabelasDescontos.RetornaTabela(wid);
...
begin
// Método Get
  THorse.Get('/trabinapi/cadastros/tabelasdescontos',ListaTodasTabelas);
  THorse.Get('/trabinapi/cadastros/tabelasdescontos/:id',RetornaTabela);

// Método Post
  THorse.Post('/trabinapi/cadastros/tabelasdescontos',CriaTabela);

  // Método Put
  THorse.Put('/trabinapi/cadastros/tabelasdescontos/:id',AlteraTabela);

  // Método Delete
  THorse.Delete('/trabinapi/cadastros/tabelasdescontos/:id',ExcluiTabela);
end;
