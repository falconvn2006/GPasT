...
uses models.movOrcamentos, prv.dataModuleConexao, dat.movDFeUltNSU;
...
procedure RetornaUltimoNSU(Req: THorseRequest; Res: THorseResponse; Next: TProc);
var wultimo,werro: TJSONObject;
    wval: string;
begin
  try
    wultimo := TJSONObject.Create;
    wultimo := Req.Body<TJSONObject>;
    wultimo := dat.movDFeUltNSU.RetornaUltimoNSU;
    wultimo.TryGetValue('status',wval);
    if wval='500' then
       Res.Send<TJSONObject>(wultimo).Status(400)
    else
       Res.Send<TJSONObject>(wultimo).Status(200);
  except
    On E: Exception do
    begin
      werro := TJSONObject.Create;
...
begin
// Método Get
  THorse.Get('/trabinapi/movimentos/dfe/ultnsu',RetornaUltimoNSU);
