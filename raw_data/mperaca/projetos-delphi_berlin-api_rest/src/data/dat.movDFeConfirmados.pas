...
function RetornaConfirmado(XId: integer): TJSONObject;
function RetornaListaConfirmados(const XQuery: TDictionary<string, string>; XEmpresa,XLimit,XOffset: integer): TJSONArray;
function RetornaTotalConfirmados(const XQuery: TDictionary<string, string>): TJSONObject;
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('       "ProtocoloNFeDistribuicao"  as protocolo,');
         SQL.Add('       "AtorDistribuicao"          as ator ');
         SQL.Add('from "DistribuicaoDocumentoFiscal" where "CodigoEmpresaDistribuicao"=:xempresa and "StatusDistribuicao" in (''3'',''8'')  ');
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('Where "StatusDistribuicao" in (''3'',''8'') ');
         if XQuery.ContainsKey('dataemissao') then // filtro por data emissão
            begin
              SQL.Add('and "DataEmissaoDistribuicao" >=:xemissao ');
              ParamByName('xemissao').AsDate := strtodate(XQuery.Items['dataemissao']);
