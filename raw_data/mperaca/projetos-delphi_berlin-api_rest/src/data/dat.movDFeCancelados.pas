...
function RetornaCancelado(XId: integer): TJSONObject;
function RetornaTotalCancelados(const XQuery: TDictionary<string, string>): TJSONObject;
function RetornaListaCancelados(const XQuery: TDictionary<string, string>; XEmpresa,XLimit,XOffset: integer): TJSONArray;
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('       "ProtocoloNFeDistribuicao"  as protocolo,');
         SQL.Add('       "AtorDistribuicao"          as ator ');
         SQL.Add('from "DistribuicaoDocumentoFiscal" where "CodigoEmpresaDistribuicao"=:xempresa and "StatusDistribuicao"=''4''  ');
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('Where "StatusDistribuicao"=''4'' ');
         if XQuery.ContainsKey('dataemissao') then // filtro por data emissão
            begin
              SQL.Add('and "DataEmissaoDistribuicao" >=:xemissao ');
              ParamByName('xemissao').AsDate := strtodate(XQuery.Items['dataemissao']);
