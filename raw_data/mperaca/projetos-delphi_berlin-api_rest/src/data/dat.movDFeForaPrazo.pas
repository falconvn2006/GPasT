...
function RetornaForaPrazo(XId: integer): TJSONObject;
function RetornaListaForaPrazo(const XQuery: TDictionary<string, string>; XEmpresa,XLimit,XOffset: integer): TJSONArray;
function RetornaTotalForaPrazo(const XQuery: TDictionary<string, string>): TJSONObject;
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('       "ProtocoloNFeDistribuicao"  as protocolo,');
         SQL.Add('       "AtorDistribuicao"          as ator ');
         SQL.Add('from "DistribuicaoDocumentoFiscal" where "CodigoEmpresaDistribuicao"=:xempresa and "StatusDistribuicao" = ''0''  ');
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('Where "StatusDistribuicao" = ''0'' ');
         if XQuery.ContainsKey('dataemissao') then // filtro por data emissão
            begin
              SQL.Add('and "DataEmissaoDistribuicao" >=:xemissao ');
              ParamByName('xemissao').AsDate := strtodate(XQuery.Items['dataemissao']);
