...
function RetornaOrcamentoItem(XOrcamento,XId: integer): TJSONObject;
function RetornaTotalItens(const XQuery: TDictionary<string, string>; XIdOrcamento: integer): TJSONObject;
function ApagaOrcamentoItem(XOrcamento,XId: integer): TJSONObject;
...
function RetornaOrcamentoItens(const XQuery: TDictionary<string, string>; XOrcamento,XLimit,XOffset: integer): TJSONArray;
function IncluiOrcamentoItem(XOrcamentoItem: TJSONObject; XIdOrcamento,XIdEmpresa: integer): TJSONObject;
function AlteraOrcamentoItem(XIdItem: integer; XItem: TJSONObject): TJSONObject;
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('from "OrcamentoItem" where "CodigoOrcamentoItem"=:xorcamento and "CodigoInternoItem"=:xid ');
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Clear;
         Params.Clear;
         SQL.Add('delete from "OrcamentoItem" where "CodigoOrcamentoItem"=:xorcamento and "CodigoInternoItem"=:xid ');
         ParamByName('xorcamento').AsInteger := XOrcamento;
         ParamByName('xid').AsInteger  := XId;
         ExecSQL;
         EnableControls;
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('       cast((select "PercentualPISConfigura" from "ConfiguracaoTSCompras" where "CodigoEmpresaConfigura"=:xidempresa) as numeric) as xpercpis,');
         SQL.Add('       cast((select "PercentualCofinsConfigura" from "ConfiguracaoTSCompras" where "CodigoEmpresaConfigura"=:xidempresa) as numeric) as xperccofins ');
         SQL.Add('from "OrcamentoItem" where "CodigoOrcamentoItem"=:xorcamento ');
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('from "OrcamentoItem" where "CodigoOrcamentoItem"=:xidorcamento ');
...
begin
  try
    ...
    if XItem.TryGetValue('idaliquota',wval) then
       begin
         if wcampos='' then
            wcampos := '"CodigoAliquotaICMItem"=:xidaliquota'
         else
            wcampos := wcampos+',"CodigoAliquotaICMItem"=:xidaliquota';
       end;
    ...
    if XItem.TryGetValue('quantidade',wval) then
       begin
         if wcampos='' then
            wcampos := '"QuantidadeProdutoItem"=:xquantidade '
         else
            wcampos := wcampos+',"QuantidadeProdutoItem"=:xquantidade ';
       end;
    ...
    if XItem.TryGetValue('baseicms',wval) then
       begin
         if wcampos='' then
    ...
    if XItem.TryGetValue('basest',wval) then
       begin
         if wcampos='' then
    ...
    if XItem.TryGetValue('valorst',wval) then
       begin
         if wcampos='' then
    ...
    if XItem.TryGetValue('valorpis',wval) then
       begin
         if wcampos='' then
            wcampos := '"ValorPISItem"=:xvalorpis '
         else
            wcampos := wcampos+',"ValorPISItem"=:xvalorpis ';
       end;
    if XItem.TryGetValue('valorcofins',wval) then
       begin
         if wcampos='' then
            wcampos := '"ValorCOFINSItem"=:xvalorcofins '
         else
            wcampos := wcampos+',"ValorCOFINSItem"=:xvalorcofins ';
       end;
    if XItem.TryGetValue('cstpis',wval) then
       begin
         if wcampos='' then
    ...
    if XItem.TryGetValue('cstcofins',wval) then
       begin
         if wcampos='' then
    ...
    if wconexao.EstabeleceConexaoDB then
       begin
         ...
         begin
           ...
           SQL.Clear;
           Params.Clear;
           SQL.Add('Update "OrcamentoItem" set '+wcampos+' ');
           ...
           if XItem.TryGetValue('idaliquota',wval) then
              ParamByName('xidaliquota').AsInteger  := strtoint(XItem.GetValue('idaliquota').Value);
           if XItem.TryGetValue('idvendedor',wval) then
           ...
           if XItem.TryGetValue('baseicms',wval) then
              ParamByName('xbaseicms').AsFloat := strtofloat(XItem.GetValue('baseicms').Value);
           if XItem.TryGetValue('valoricms',wval) then
              ParamByName('xvaloricms').AsFloat := strtofloat(XItem.GetValue('valoricms').Value);
           if XItem.TryGetValue('basest',wval) then
              ParamByName('xbasest').AsFloat := strtofloat(XItem.GetValue('basest').Value);
           if XItem.TryGetValue('valorst',wval) then
              ParamByName('xvalorst').AsFloat := strtofloat(XItem.GetValue('valorst').Value);
           if XItem.TryGetValue('percentualpis',wval) then
           ...
           if XItem.TryGetValue('cstpis',wval) then
              ParamByName('xcstpis').AsString := XItem.GetValue('cstpis').Value;
           if XItem.TryGetValue('cstcofins',wval) then
              ParamByName('xcstcofins').AsString := XItem.GetValue('cstcofins').Value;
           ExecSQL;
           EnableControls;
           wnum := RowsAffected;
         end;
