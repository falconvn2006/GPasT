...
function RetornaArquivado(XId: integer): TJSONObject;
function RetornaListaArquivados(const XQuery: TDictionary<string, string>; XEmpresa,XLimit,XOffset: integer): TJSONArray;
function RetornaTotalArquivados(const XQuery: TDictionary<string, string>): TJSONObject;
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('       "ProtocoloNFeDistribuicao"  as protocolo,');
         SQL.Add('       "AtorDistribuicao"          as ator ');
         SQL.Add('from "DistribuicaoDocumentoFiscal" where "CodigoEmpresaDistribuicao"=:xempresa and "StatusDistribuicao" in (''7'',''8'')  ');
...
// cria provider de conexão com BD
    ...
    if wconexao.EstabeleceConexaoDB then
       ...
       begin
         ...
         SQL.Add('Where "StatusDistribuicao" in (''7'',''8'') ');
         if XQuery.ContainsKey('dataemissao') then // filtro por data emissão
            begin
              SQL.Add('and "DataEmissaoDistribuicao" >=:xemissao ');
              ParamByName('xemissao').AsDate := strtodate(XQuery.Items['dataemissao']);
